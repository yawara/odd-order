...
- [x] Add the explicit-parameter (14.6) dispatcher: Clifford case (a) is refuted by the
      prime contradiction; case (b) returns the transported field model.
...
The dispatcher keeps `c = 1`, `q = 3`, `u = (p-1)^2/4`, and
`TypeIOverNormalizerData` explicit.  It therefore does not hide the issue-0116 analytic
producer or strengthen the generic S15 hypothesis.  The new leaf is additive and consumed
downstream by S16; no b/c-owned declaration is modified.
...
- `S15.sSide_galoisField_repr_of_parameters_and_typeIOverNormalizerData`
  dispatches the proven Clifford dichotomy; case (a) is eliminated by issue 1038,
  while case (b) returns the transported model.
- A direct probe of the existing c-owned `S16.s_side_field_repr` target succeeds with
  ...
  Hub/c therefore only needs to supply the canonical `TypeIOverNormalizerData` and the
  explicit (13.12)/(13.13) parameter equalities; no target or proof-term adaptation is needed.
