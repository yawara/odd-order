...
- **✅ `exists_orderQ_le_mf_normal_in_M_of_not_fittingIsTI` の `q = p` ケース完成** (S15_MF、旧 8229 sorry)。`#print axioms` = `[propext, Classical.choice, Quot.sound]` で **witness 全体が axiom-clean**。`Z := Ω₁(Z(O_p(M_F)))` を **既存 `OddOrder.BG.Ch3.S10.omega1CenterInG P p`** (= `(Ω₁(Z(↥P))).map P.subtype`) で構成 (自前 plumbing 不要)。`|Z| = p` を rank 論法で:
  - **`X₁ ⊄ Z` (crux、Coq `not_sX1Z0`)**: `X₁ ≤ Z ⊆ Z(P)` ⟹ 各 `x∈X₁` は `P` の中心元 ⟹ `P ≤ C_G(X₁)` ⟹ `P ≤ C₁ = M_F ⊓ C_G(X₁)` (abelian = `isMulCommutative_mf_inf_centralizer_of_not_le`) ⟹ `P` abelian、新 helper `opiCore_singleton_not_isMulCommutative_of_witness` (= Coq `not_cPP`、conjunct B prefix を共有 lemma 化) と矛盾。
  - **`X₁ ⊓ Z = ⊥`**: `|X₁|=p` 素数 ⟹ `X₁⊓Z ∈ {⊥, X₁}`、`X₁⊄Z` ゆえ `⊥` (`Nat.dvd_prime` + `eq_of_le_of_card_ge`)。
  - **rank bound (Sylow-of-G 不要)**: `B = X₁ ⊔ Z` elem abelian (`isElementaryAbelian_sup_of_le_centralizer`)、`|B| = p·|Z|` (`card_HK_mul_card_inf_eq_card_mul_card` + `coe_mul_of_left_le_normalizer_right`)、`B ≤ C₁` ゆえ `log_p|B| ≤ pRank C₁ ≤ rank C₁ < 3` (`le_pRank`+`pRank_le_rank`、`subgroupOf C₁` 経由)。`|Z|=p^d` (elem abelian) と合わせ `d+1 ≤ 2` ⟹ `d ≤ 1`、`Z ≠ ⊥` (P 非自明 p-群 ⟹ `IsPGroup.center_nontrivial` + Cauchy `exists_prime_orderOf_dvd_card'`) ⟹ `d = 1`、`|Z| = p`。
  - **M-normality**: `M ≤ N(P)` (`le_normalizer_opiCoreInG_of_le_normalizer`) → `M ≤ N(Z)` (既存 `OddOrder.BG.Ch3.S10.normalizer_le_normalizer_omega1CenterInG`)。
- **新 helper `opiCore_singleton_not_isMulCommutative_of_witness` (S15_MF、sorry-free+axiom-clean)**: 非-TI witness data + 非 abelian `M_F` ⟹ `O_p(M_F)` 非 abelian (Coq `not_cPP`)。conjunct B (`typeF_nonabelian_cyclic_opiCore_compl`) の prefix を抽出した共有 lemma。両者 AxiomsCheck 登録。
- **下流効果**: `isTypeI_of_isTypeF` (`hFI` bridge) の **2 ゲートのうち「非-TI residual = BG Thm 15.7(e)」が解消**。残ゲートは `typeFData_of_kappa_eq_bot` 経由の **sorried Theorem A** (`theoremA_maximal_structure` standalone、5 deep theorems の最後) のみ ⟹ `isTypeI_of_isTypeF` は `[propext, sorryAx, Classical.choice, Quot.sound]` (Theorem A 由来の sorryAx)。AxiomsCheck の hFI 注記を訂正済。
- **▶ 次フロンティア**: hFI 完全 axiom-clean 化には Theorem A standalone (`theoremA_maximal_structure`) の faithfulness 修正のみ (faithful 版 `theoremA_maximal_structure_faithful` は cont.² 完成済 → standalone を faithful 版に置換 or 配線)。他 6 bridge (`hP1neIIIIV`/`hP1eqV`/逆方向 4 本) は別 deep / carrier-gated。
...
- **✅ S16:1391 配線完成 + witness q≠p 完成 (commit `9e668543`)**: `isTypeI_of_isTypeF` の非abelian枝を `refine Or.inr (Or.inr ⟨conjunct A (witness+engine), conjunct B⟩)` で **sorry-free 化** (両エンジン consume)。witness lemma `exists_orderQ_le_mf_normal_in_M_of_not_fittingIsTI` (S15_MF): 各 q∈π(M_F) で order-q 部分群 Z≤M_F が M-normal。
  - **q≠p DONE** (sorry-free): O_q(M_F)≤O_{p'}(M_F) cyclic (`oPiCore_mono`+`Subgroup.isCyclic_of_le`)、Cauchy 元 zpowers の order-q 部分群は cyclic ゆえ char (`Ch04.characteristic_of_subgroup_of_isCyclic`)、M≤N(O_q) (`le_normalizer_opiCoreInG_of_le_normalizer`+`maxNilpotentNormalHall_le_normalizer`) → M≤N(Z) (`Ch07.normalizer_le_normalizer_map_of_characteristic`+`map_subgroupOf_eq_of_le`)。
- **✅ 残 (c) = witness の q=p ケース → cont.¹⁵ で完成** (下記)。`Z_p = Ω₁(Z(O_p(M_F)))` を既存 `omega1CenterInG` で構成、`|Z_p|=p` を rank 論法で締結。witness 全体 axiom-clean。
...
**② conjunct A (∀q exp(U)∣q-1)** — Frobenius semiregular (Coq 1156-1164, FmaxM 枝)。**残**:
  ...
  **✅ de-risk 解決 + crux landed**: `regular_norm_dvd_pred` 相当を usable な形で着地 = `card_dvd_sub_one_of_isFrobeniusAction` (S15, commit `ceecb7c8`、= `IsFrobeniusAction A N → |A|∣|N|-1`、`IsFrobeniusAction.card_modEq_one` + `Nat.modEq_iff_dvd'`)。Z_q に `IsFrobeniusAction U₀ Z_q` を立てれば |U₀|∣q-1。残 plumbing = Z_q=Ω₁(Z(O_q(M_σ))) 構成 (order q、q≠p は cyclic O_{p'} 依存) + `MulDistribMulAction U₀ Z_q` (conjugation) setup + Frobenius FPF from `frobenius_HU0` + |U₀|∣q-1 ⟹ exp(U)=exp(U₀)∣q-1。
...
Prop 16.1 完了: AxiomsCheck.lean:6205 に `proposition_type_classification` の assert 登録 (axiom-clean;
検証 2026-07-02)。theoremA 残件は BG 側で凍結 (do-not-prove-as-is)。
