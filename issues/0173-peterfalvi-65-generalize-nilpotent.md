...
- [ ] 債務の出所 `six_three_of_imageData` ((6.3)) を読み、冪零性が**本質的に使われている箇所**を
      特定する (単なる利便か、`K/M` へ持ち上げられるか)。
      ⚠ 「threading されている」≠「依存している」— 証明本体を grep して実使用を数えること
...
(6.5)(a)(b)(c) が書籍どおり `K/M` 冪零の仮定で述べられ、旧 `K` 冪零版がその特殊化として
