...
### |W| = 3 側の設計 (2026-07-26 導出、部品 3 本 landed)

目標は同じ **`3 ∣ |Ab(R₂⟨s⟩)|`** (`R₂ = R₁` なので `= 3 ∣ |Ab(N_G(RΣ))|`)。
`K := ⁅R₁⟨s⟩, R₁⟨s⟩⁆ ≤ R₁` なので **`3 ∣ |Ab(R₁⟨s⟩)| ⟺ K ≠ R₁`**。

書籍の `R̄₁` 商計算を経由せず、次の 2 段で出せる (導出済):
1. **`A := R₁^ab` の `s`-固定部分が非自明**。`s` は `ΣP` を中心化するので、
   `ΣP` の `A` における像が非自明なら十分 ⟹ **`ΣP ⊄ ⁅R₁,R₁⁆`** が要る。
   これは landed: `not_sigma_sup_P_le_commutator_sylowThree`
   (`Z₁ ≤ ⁅R₁,R₁⁆` と `Z₁ ⊓ ΣP = 1` から、`ΣP ≤ ⁅R₁,R₁⁆` なら位数 27 の
   `Z₁ΣP` が入り `|⁅R₁,R₁⁆| ≤ 9` に矛盾)。
   - 部品も landed: `zpowers_inf_sigma_sup_P_eq_bot` (`Z₁ ⊓ ΣP = 1`、強実性から) /
     `zpowers_le_commutator_sylowThree` (`Z₁ ≤ ⁅R₁,R₁⁆`)。
2. **`φ : A →* A, a ↦ a⁻¹·a^s` は単射でない** (核が `A^s ≠ 1` を含む) ⟹ 有限性から
...
### ✅ |W| = 3 側 完了 (2026-07-26)
`three_dvd_card_abelianization_of_card_W_eq_three` : **3 ∣ |Ab(R₂⟨s⟩)|** (|W| = 3)。
σ-1 論法の実装ポイント:
- `φ := σ / MonoidHom.id` (hom の除法) にすると `map_mul` を自前で証明せずに済む。
- `F := π⁻¹(range φ)` の `R₁⟨s⟩`-正規性は生成元 2 種で確認 (R₁ 側 = 内部自己同型が
  abelianization 上自明、s 側 = `σ∘φ = φ∘σ`)。
- `⁅R₁⟨s⟩,R₁⟨s⟩⁆ ≤ F` は `commutator_le_of_generators` で 4 ケース
  (R₁×R₁ は像 1 / `⁅a,s⁆ ↦ φ(π a)⁻¹` / `⁅s,b⁆` はその逆元 / `⁅s,s⁆ = 1`)。
- `index_range_eq_card_ker` + `ker φ ⊇ π(ΣP) ≠ 1` ⟹ `|F| ≤ 3⁴` ⟹ `|⁅H,H⁆| ≤ 3⁴`
  ⟹ `|Ab(H)| = 2·3^(5-j)` が 3 で割れる。

⚠ 1500 行超過のため **prefix-split**: 前半 (generic + 弱閉性 + `N_G(R₂) = R₂⟨s⟩`) を
