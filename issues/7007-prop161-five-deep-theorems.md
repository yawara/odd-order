...
**✅✅ (a) `kappa_conj_smul` DONE (cont.¹¹ session 末、commit `e6bd440e`, sorry-free + axiom-clean, AxiomsCheck 登録, full build 3881)**: `kappa (conj g•M) = kappa M` + 系 `sigmaComplementPrimes_conj_smul`/`isTypeP_conj_smul`/`isTypeP1_conj_smul`/`isTypeP2_conj_smul`（全 S14, `OddOrder.BG.Ch4.S14`）。一方向 (∀ a N) + 対称性。`MaximalSubgroupTypeConj` toolkit (`conjSubgroupEquiv`/`pRank_eq_of_mulEquiv`/`centralizer_pointwise_smul`/`pointwise_smul_eq_bot_iff`/`coe_pointwise_smul`) + 既存 `conj_smul_mem_elemAbelianOfRank`/`sigma_conj_smul_eq`/`Msigma_conj_smul` で構築（S14 に `import MaximalSubgroupTypeConj` 追加、S14 は `OddOrder.GroupTheory` を open 済ゆえ toolkit は bare）。⟹ **sK_FD case-2a の blocker 解消**。**▶ 次 = (b) sK_FD 本体**: step-1 infra (s'H_K/D=E-setup/uniqMst) を貼り戻し → Lemma 14.11 適用 → case 分析（case 1=uniqMst+τ₂⊄σ; case 2a=`isTypeP1_conj_smul`+`not_isTypeP1_and_isTypeP2`; case 2b=`sigma_conj_smul_eq`+`kappa_subset_sigmaCompl`）。残 (c)(d)(e) は不変。~150-180 行、1.5 session。
