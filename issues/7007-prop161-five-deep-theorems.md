...
- [~] Thm 15.7 `fitting_not_ti_cases` — **residual = type-F の `M'=F(M)` 一点まで還元** (`dc2fe378`/`7eeb933b`/`1d5e6ff0`/`4fc9125a`, 2026-06-21)。(a)=`fittingIsTI_of_isTypeP2`+三分律 / (b)=`mf_eq_msigma_of_not_fittingIsTI` / ∃X=巡回非自明 X≤MF は Mσ order-q 元 (Lean 文 X 非 pin=弱化) + prime p∈σ-β は disjointness + disjunct (a)。**`M'=F(M)` の type-P1 case は実証明** (`4fc9125a`: U=⊥ inline [κ=σ'] → M'=Mσ via Lem15.1b、K≠⊥ via `card_kappaHall_ne_one`、Mσ=MF nilpotent → Mσ≤F(M)、clause d F(M)≤M')。**残 = type-F の M'=F(M) のみ**。**2026-06-21 精密再構成** (BG の「(c)(d) follow from Cor 15.5 + Lem 12.1」を分解):
  - **M'≤F(M) は導出可能** (次セッション ~40 行で landable): `subgroupESetup_of_isHall_kappa_eq_bot` (type-F E-setup) → `derivedInG_eq_Msigma_sup_derivedInG_complement` で `M'=Mσ⊔E'` → **Lem 12.19** (`S12_E:269`, E' が Mσ の Hall β'-部分群を中心化; π(Mσ)∩β=∅ [=`piSet_mf_inf_beta_disjoint`] ゆえ Mσ 全体) で `E'≤C_G(Mσ)` → `M'=Mσ⊔E'≤Mσ⊔(C_G(Mσ)⊓M)=F(M)` (fitting_decomposition clause 5)。
  - **F(M)≤M' が真の深い core = `C_E(Mσ)=1`** (type-F が Frobenius、補元**全体**が FPF)。`F(M)=Mσ⊔C_M(Mσ)`、`C_M(Mσ)=Z(Mσ)·C_E(Mσ)`、`C_E(Mσ)≤M'⟺C_E(Mσ)≤E'⟺C_E(Mσ)=1`。Lem 15.1(e) `typeP_hall_frobenius_factor` は Frobenius **factor** U0 のみ (全補元でない)、`fittingInAmbient_le_Msigma_of_le_Msigma` は σ-uniqueness gap `C_M(Q)⊆Mσ` 要求 = **Theorem B 級**。⟹ ChatGPT 相談 or Theorem B 連動の multi-session
