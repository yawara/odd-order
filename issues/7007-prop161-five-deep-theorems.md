...
- [~] Thm 15.7 `fitting_not_ti_cases` — **✅✅✅ 2026-06-22 重大発見: BG 印刷版 conjunct (c) `M'=F(M)` は overstatement、faithful 版は `M'≤F(M)`** (ChatGPT GPT-5 Pro 相談 + MathComp 検証で確定):
  ...
  - **authoritative MathComp odd-order 形式化 (`BGsection15`/`nonTI_Fitting_structure`) が conjunct (c) を `M^'(1) ⊆ 'F(M)` (包含) ∧ `Mσ × O_σ('F(M)) = 'F(M)` と形式化、等式でない** → BG 印刷版が overstatement と確定。詳細 `notes/bg/s15_7_typeF_chatgpt_prompt.md`。
