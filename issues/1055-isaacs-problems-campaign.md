...
### 5B.2 の探索記録 (2026-07-26)

**得られた鍵の事実 (landing 済、`Dietzmann.lean`)**:
`X` 共役閉、`x ∈ X` ⇒ **`x` による共役は `X \ {x}` を保つ** (`conj_mem_sdiff_singleton`;
`g y g⁻¹ = g ⟺ y = g` から)。したがって
**`⟨X \ {x}⟩` は `⟨X⟩` の中で正規** (`closure_le_normalizer_closure_sdiff`) で、
`⟨X⟩ / ⟨X \ {x}⟩` は `x` の像で生成される巡回群 ⇒ `|⟨X⟩| ≤ n · |⟨X \ {x}⟩|`。
(m = 2 のときはこれだけで完結する: `X = {a,b}` 正規集合なら `a⁻¹ b a = b` が強制され
`|⟨X⟩| = 4 = n²`。)

**まだ閉じない理由**: 帰納を回すには `X \ {x}` にも同じ仮説が要るが、
`X \ {x}` は**自分自身の元による共役で閉じない** (S₃ で `(13)⁻¹(23)(13) = (12) ∉ X\{(12)}`)。
- 「共役類ごと除く」方式 (`X \ C` は共役閉、`⟨C⟩ ⊴ G`) は `m` を落とせるが、
  **単一共役類 `C` で `|⟨C⟩| ≤ n^{|C|}`** に帰着するだけで前進しない。
- 「`x ∉ ⟨X\{x}⟩` なら帰納が回る」が、S₃ ではどの `x` でも `x ∈ ⟨X\{x}⟩` で停止。

⟹ 次は **CLAUDE.md の [[feedback-ask-chatgpt-for-elided-gaps]] に従い外部相談**するか、
文献の Dietzmann 補題の sharp form の証明を当たる。
