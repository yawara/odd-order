...
### 5E.1 (Itô) の設計 — **部分的にしか固まっていない** (2026-07-27 の率直な状況報告)

5E.1 は §5E の他 2 問より**明確に重い** (Itô の「極小非 `p`-冪零群」の分類そのもの)。
この iteration で**確定できた部分**と**未確定の部分**を分けて記録する。

**確定できた部分**:

1. **入口**: `G` が正規 `p`-補群を持たないので Frobenius Thm 5.26 の対偶より、
   `N_G(X)` が正規 `p`-補群を持たない非自明 `p`-部分群 `X` が存在する。仮定 (真部分群は
   すべて正規 `p`-補群をもつ) から `N_G(X) = ⊤`、すなわち `X ◁ G` ⟹ **`O_p(G) ≠ 1`**。
2. ⭐ **`Ḡ := G/O_p(G)` は必ず `p`-冪零**。もし違えば `Ḡ` にも 1 が適用でき
   `O_p(Ḡ) ≠ 1`、つまり `O_p(G) < M ◁ G` で `M/O_p(G)` が `p`-群となる `M` が取れるが、
   そのとき `M` 自体が正規 `p`-部分群になり `O_p(G)` の最大性に矛盾する。
   ⟹ **`G` は `p`-可解** (`1 ≤ O_p(G) ≤ K ≤ G` が `p`, `p'`, `p` 因子の列)。
3. 2 から Schur–Zassenhaus で `K = O_p(G) ⋊ L` (`L` は `G` の Hall `p'`-部分群)、
   Frattini で `G = O_p(G)·N_G(L)`。`N_G(L) = ⊤` なら `L` が正規 `p`-補群になって矛盾ゆえ
   `N := N_G(L)` は真部分群で `p`-冪零、その正規 `p`-補群はちょうど `L`。

**未確定の部分 (次 iteration の課題)**:

* 上の `G = O_p(G)·N` から **`P ◁ G`** (結論 (a)) を出す最後の一歩が詰められていない。
  `P := O_p(G)·P_N` (`P_N ∈ Syl_p(N)`) が Sylow `p`-部分群になることまでは位数計算で出るが、
  `L` が `P` を正規化することが未証明。
* `O_{p'}(G) = 1` を示す (または迂回する) 筋も未確定。`O_{p'}(G) ≠ 1` のとき `G/O_{p'}(G)` へ
  帰納しても、(a) の持ち上げ (`Y ⋊ P₁` の `P₁` の正規性) が従わない。
* 結論 (b) (`|G|` の `p` 以外の素因数がちょうど 1 個) も (a) 経由の予定。

⚠ **次 iteration の方針**: 教科書 (Isaacs) は演習として省略しているので、
**Huppert III.5.2 / IV.5.4 (Itô の定理) か Gorenstein の該当箇所を読んで行間を埋める**
(CLAUDE.md: BG の行間補完のため Gorenstein 原文参照は可)。`coq/` submodule に
対応物があるかも確認する。設計が固まるまで Lean を書き始めない。
