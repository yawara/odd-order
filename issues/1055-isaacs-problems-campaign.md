...
| perfect | **進行中**。✅ 核心の恒等式 `LinearMap.commutator_transvection` (2026-08-03): `f` が両中心を殺し `g` が自分の中心 `w` を殺すなら `⁅t_{v,f}, t_{w,g}⁆ = t_{w, -(g v) • f}`。⟹ `g v = 1` に取れば**中心 `w` の任意の transvection が交換子**。残りは存在部分 2 つ: (i) `dim ≥ 3` なら `v ∈ ker h`, `v ≠ 0`, `v ≠ w` が取れる (`dim ker h ≥ dim W − 1 ≥ 2`)、(ii) `v, w` 独立なら `g w = 0`, `g v = 1` なる `g` が取れる (`W ⧸ span{w}` へ落として双対で分離)。そのあと `commutator` の正規性 + `T v` 同士の共役性 + `is_generator` で `commutator = ⊤` |
