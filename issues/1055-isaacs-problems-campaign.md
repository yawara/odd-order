...
**残り (8A.14)**: 後半「`H` がどの点安定化群も含まないなら軌道数 ≤ `m/2`」。
二重剰余類 `HgG_α` が単一の右剰余類 `Hg` に等しいのは `g G_α g⁻¹ ⊆ H` のとき, すなわち
`H` が点安定化群 `G_{g•α}` を含むとき — これを禁じると各二重剰余類が 2 つ以上の右剰余類を
含むので軌道数 ≤ `m/2`。形式化は上の全射の**ファイバーが常に 2 元以上**であることを示す形。
