...
- [ ] Ch.10 More Transfer — **⚠ 実測で §10C は 6 問 (旧インベントリの「10C(1)」は誤り)**。
      **§10A (7 問) 🎉 完済 (2026-08-03)**: 10A.3 前半+後半 と **10A.4** が landing。
      §10B (2 問): 10B.2 は骨格のみ (Maschke 待ち)、**10B.1 未**。
      **§10C (6 問) は全部未着手**
...
| 8B.6 (残: `D₂ₚ` 明示同型) | lane a 単独セッション (2026-07-28 再開) | 2026-07-28 00:0x | **🎉 完了 2026-07-28** |
| 8C.6 (残: 初等可換 2-群 + 「⟹」) | lane a 単独セッション (2026-07-28 再開) | 2026-07-28 00:0x | **繰延** (下記 scope 発見により文書順で後ろ) |
| Ch.1 §1E (1E.1–1E.8) | lane a 単独セッション | 2026-07-28 01:0x | 着手 |
...
## ⚠ scope 発見 (2026-07-28 実測): Ch.1 §1E–§1G / Ch.3 §3C–§3F の演習が丸ごと未形式化

本 campaign は Ch.1 §1A → §1B → §1C → §1D と進んだ後 **§1E で止まらず Ch.2 へ移っており**,
同様に Ch.3 も §3A / §3B までで §3C 以降に入っていなかった。checklist の Ch.1 / Ch.3 が
未チェックのままだった理由がこれ。**8C.6 に進む前に実測で確認**した結果:

### 教科書側 (実測)

`grep -nE "Problems [1-4][A-G]" references/isaacs/finite-group-theory.pdftotext.txt` と
問題番号の直接 grep より:

* **Ch.1 = §1A–§1G** (Problems 1A 〜 Problems 1G, 後者は text L2219)。
  **§1E は 1E.1–1E.8, §1F / §1G も演習あり** (1F.1–, 1G.1–1G.4)。
* Ch.2 = §2A–§2D (repo は 2A.1–10 / 2B.1–6 / 2C.1 / 2D.1–2 で被覆済)。
* **Ch.3 = §3A–§3F**。repo は **§3A / §3B のみ**で **§3C (3C.1–3C.8) / §3D (3D.2–3D.5) /
  §3E (3E.1–3E.5) / §3F (3F.1–3F.5) が未着手**。
* Ch.4 = §4A–§4D (repo は 4A.1–13 / 4B.1–5 / 4C.1–3 / 4D.1–7 で被覆済)。

⚠ repo の `3C.1` / `3E.x` grep hit は **演習ではない** —
`Ch03_SplitExtensions/Basic.lean:1323` の `3C.1` は **定理番号 (Hall D-定理, Wielandt)**、
`Ch04_Commutators/ForwardFromCh03.lean` の `§3E.x` は **節見出し**。

### 帰結 (作業順)

CLAUDE.md の **上流優先 + 文書順**より, 真のフロンティアは **Ch.1 §1E** であって
Ch.8 §8C.6 ではない。順序は **§1E → §1F → §1G → §3C → §3D → §3E → §3F → 8C.6 (Ch.8 の残り)**。

### §1E の問題 (pdftotext L1958–, 書籍 p. 37–38)

⚠ pdftotext は上付きを落とす (`2 4 -3 2` = `2⁴·3²`) が、**積の値と一致するので確定できる**
(144 = 2⁴·3², 336 = 2⁴·3·7, 180 = 2²·3²·5, 240 = 2⁴·3·5, 252 = 2²·3²·7, 315 = 3²·5·7)。

* **1E.1**: `|G| = p²q²` (`p < q` 素数) なら `n_q(G) = 1`, ただし `|G| = 36` は例外。
  Note: `|G| = 36` では Sylow 3 が正規でないことがあるが、その場合 Sylow 2 が正規。
* **1E.2**: `|G| = pqr` (`p < q < r` 素数) なら `n_r(G) = 1`。
  Hint: そうでなければ元の数え上げで Sylow q が正規、位数 `pr` の商を見る。
* **1E.3**: 位数 `315 = 3²·5·7` の単純群は無い。
* **1E.4**: `|G| = 144 = 2⁴·3²` なら単純でない。
* **1E.5**: `|G| = 336 = 2⁴·3·7` なら単純でない。Hint: 単純とすると `n₇(G)` を計算し **1C.5** を使う。
...
### 10A.3 (前半) / 10A.6 完了 (2026-07-29)

* **10A.3 前半 ✅** `exists_injective_hom_regularWreath_of_center_isComplement`。
  `Q := K` (=`Z(P)` の `A` 内補元) を `P` に押し出せば `|P:Q| = p²`, `Q ∩ Z(P) = 1` で
  **10A.5 に帰着**。指数計算だけ。
  ⏳ **後半「`A` は基本可換」は未解決** (前掲のとおり `A = Z(P) × K` だけでは出ない)。
* **10A.6 ✅** `isTwoTransitive_iff_exists_doubleCoset`。2-推移性を
  「`∀ a b c d, a ≠ b → c ≠ d → ∃ g, g•a = c ∧ g•b = d`」の初等形で述べ、
  `H ∪ HgH` を「`∀ x, x ∈ H ∨ ∃ h₁ h₂ ∈ H, x = h₁ g h₂`」で述べた。
  `mathlib` の `IsMultiplyPretransitive` (= `IsPretransitive G (Fin n ↪ α)`) は
  `Fin 2 ↪ Ω` の取り回しが煩雑なので使わなかった。
  ⚠ `Ω` が 1 点の場合 (`H = G`, `g = 1`) を別扱いする必要がある。
...
## 9D.4 の未解決 2 点を解決 — Fable 5 単独・紙の証明 (2026-07-30)

前回 deferred の 2 点 (「`N` 非可換ケース」「`S` も非可換単純」) の証明が立った。
形式化は下記の紙の証明を正とする。

### (I) `N` 非可換ケース: `[S, N] = 1` の導出 (想定筋の実現)

設定: 極小反例 `(G, S)`、`N` 極小正規 (`< G`)、`S ∩ N = 1`, `SN = G` (skeleton 1-4 済)、
`N` 非可換 (可換なら `O^p` 論法で済 — 既記録)。`N = T₁ × ⋯ × T_k`, `Tᵢ` 非可換単純。

任意の素数 `p ∣ |S|` について:

1. `S_p ∈ Syl_p(S)` を取る。`S_p` は `Syl_p(N)` に共役で作用し `|Syl_p(N)| ≡ 1 (mod p)`
   なので **`S_p`-不変な `N_p ∈ Syl_p(N)`** が取れる (p-群作用の不動点)。
2. `P := S_p N_p` は部分群 (`S_p ≤ N_G(N_p)`) で `|P| = |S|_p |N|_p = |G|_p`
   (`S_p ∩ N_p ≤ S ∩ N = 1`) ⟹ `P ∈ Syl_p(G)`。
3. **Kegel 仮説を `P^n` (`n ∈ N`) に適用**: `P^n ∩ S ∈ Syl_p(S)`。一方、射影
   `π_S : G = S ⋉ N → S` で `P^n ∩ S ⊆ π_S(P^n) = π_S(P)^{π_S(n)} = S_p`。
   位数が `|S_p|` なので **`P^n ∩ S = S_p`、すなわち `S_p ≤ P^n` が全 `n ∈ N` で成立**。
4. `R := ⟨S_p^N⟩` (`N`-共役たちの生成)。step 3 から各 `n S_p n⁻¹ ≤ P` なので
   `R ≤ P` は **`p`-群**。構成から `N` は `R` を正規化する ⟹ `R ◁ NR` ⟹
   `[N, R] ≤ N ∩ R` かつ `N ∩ R ◁ N`。
5. 半単純 (非可換単純の直積) `N` の正規部分群は**因子の部分積のみ** (Isaacs §9A/
   `Semisimple.lean` 圏の標準事実; 各 `Z(Tᵢ) = 1`)。`N ∩ R` は `p`-群なので部分積としては
   **自明**: `N ∩ R = 1`。
6. ⟹ `[N, S_p] ≤ [N, R] = 1`。これを `|S|` の全素数 `p` で回すと
   `S = ⟨S_p : p⟩ ≤ C_G(N)`、つまり **`[S, N] = 1`**。
7. `S ∩ N = 1` かつ `N` が `S` を中心化するので `S ◁ SN = G` ⟹ `S ◁◁ G` で反例に矛盾。∎

⚠ step 5 が `N` 非可換を使う唯一の箇所 (可換 `N = q`-基本可換だと `p = q` のとき
`N ∩ R ≠ 1` があり得る — だから可換ケースは従来どおり `O^q(G) ≤ S` 論法で別処理)。
⚠ 前回検討した「`C_G(N) = 1` を先に出す」「相異なる極小正規 2 つの解析」は**不要**になった。

### (II) `S` は非可換単純

1. **`S` の真 subnormal は `G`-subnormal**: `T ◁◁ S`, `T < S` なら `(G, T)` も Kegel
   仮説を満たす (`P ∩ T = (P ∩ S) ∩ T` と Lemma 9.31 を `S` 内で適用) ので
   `|T| + |G| < |S| + |G|` の極小性から `T ◁◁ G`。
2. `T ◁ S` 真正規, `T ≠ 1` とし `R := T^G ◁ G` (`≠ 1`)。**(F2) 商降下** + 極小性で
   `SR/R ◁◁ G/R` ⟹ `SR ◁◁ G`。
3. `SR < G` なら (F1) + 極小性で `S ◁◁ SR`、繋いで `S ◁◁ G` で矛盾 ⟹ **`G = S·T^G`**。
4. すると `T^G = ⟨T^{sr} : s ∈ S, r ∈ T^G⟩ = ⟨(T^s)^r⟩ = ⟨T^r : r ∈ T^G⟩ = T^{T^G}`
   (**`T ◁ S` なので `T^s = T`**)。真の subnormal 部分群の正規閉包列は真に降下する
   (`T ◁◁ K`, `T < K` ⟹ `T^K < K`: 極小長の鎖 `T ≤ K_{m-1} ◁ K`, `K_{m-1} < K` で
   `T^K ≤ K_{m-1}`) ので、停滞 `T^{T^G} = T^G` は **`T = T^G ◁ G`** を強制。
5. `T ≠ 1` ⟹ 極小正規 `M ≤ T ≤ S`。skeleton step 4 は任意の極小正規で `S ∩ M = 1`
   を与えるので `M = S ∩ M = 1` で矛盾 (`G` が単純なら `T ◁◁ G` 真 ⟹ `T = 1` は直ちに出る
   ので step 2-5 は不要)。⟹ `S` の真正規部分群は `1` のみ = **`S` は単純** (`S ≠ 1` は
   skeleton step 1)。
6. **非可換**: `|S| = q` 素数なら Kegel から `S = P ∩ S ≤ P` が全 `P ∈ Syl_q(G)` で成立
   ⟹ `S^g ≤ P` (∀g, 固定 P) ⟹ `⟨S^G⟩` は正規 `q`-部分群 ⟹ `S ≤ O_q(G)`。
   `O_q(G)` は冪零正規なので `S ◁◁ O_q(G) ◁ G` ⟹ subnormal で矛盾。∎

### 形式化順序 (残り)

(F2) 商降下 → skeleton 1-4 (含 chain-shortening = 正規閉包の真降下補題) →
...
### (I) step 5 の簡略化 — Fitting 部分群で `N ∩ R = 1` (2026-08-03)

記録済の紙の証明は step 5 で「半単純 `N` の正規部分群 = 因子の部分積」を使っていたが、
**Fitting 部分群 1 本で済む**:

`N ∩ R` は `N` の正規な `p`-部分群 (冪零) なので `N ∩ R ≤ F(N)`。`F(N)` は `N` に
characteristic で `N ◁ G` だから `F(N) ◁ G`、しかも `F(N) ≤ N`。`N` の極小性から
`F(N) = 1` か `F(N) = N`。後者なら `N` は冪零で、極小正規 + 冪零 ⟹ 可換
(`Z(N)` char `N ◁ G` かつ冪零非自明なら `Z(N) ≠ 1` ⟹ 極小性で `Z(N) = N`) となり
`N` 非可換に矛盾。⟹ `F(N) = 1` ⟹ **`N ∩ R = 1`**。

必要部品: (a) `H char K`, `K ◁ G` ⟹ `H.map K.subtype ◁ G`、
...
⟹ (i) `P ↪ C_p ≀ C_p` (済) **かつ (ii) `A` は基本可換**。

(ii) の証明 (前回の `φ : a ↦ a⁻¹uau⁻¹`, `ker φ = Z(P)`, `im φ = P'` の解析は**不要**だった):

1. `P` は自動的に非可換 (`P` 可換なら `Z(P) = P ≤ A` が `|P:A| = p` に矛盾)。
   `u ∈ P − A` を取ると `A` は極大 (指数 `p`) なので `⟨A, u⟩ = P`、ゆえに
   **`C_A(u) = Z(P)`** (`c ∈ C_A(u)` は `A` とも `u` とも可換 ⟹ 中心的; 逆は
   `Z(P) ≤ A` (直積因子) から)。
2. `A^p := {a^p | a ∈ A}` (可換なので `p` 乗写像の像 = 部分群)。`A = Z(P) × K`,
   `|Z(P)| = p` から `(zk)^p = z^p k^p = k^p` ⟹ **`A^p ≤ K`**。
3. `A` 基本可換でない ⟺ `A^p ≠ 1` とする。`A ⊴ P` (p-群の指数 p) で `p` 乗と共役は可換
   なので `A^p` は `u`-共役不変。`p`-群 `⟨u⟩` の非自明 `p`-群 `A^p` への作用は
   **非自明な固定点** `z ≠ 1` をもつ (|Fix| ≡ |A^p| ≡ 0 mod p, 1 ∈ Fix)。
4. `z ∈ C_A(u) = Z(P)` (step 1) かつ `z ∈ A^p ≤ K` (step 2) ⟹ `z ∈ Z(P) ⊓ K = 1`
   で矛盾。∎
...
| `is_generator` | ✅ **2026-08-03**: `LinearMap.iSup_transvectionSubgroup_eq_top` (`Transvection.lean`)。mathlib の Gauss 消去 `Matrix.diagonal_transvection_induction_of_det_ne_zero` を `P N := ∀ g', toMatrix B B g' = N → g' ∈ ⨆ v, T v` で回す。対角の場合は `diagonal_eq_one_of_det_ne_zero` (𝔽₂ では可逆対角 = 単位)、transvection の場合は `toMatrix_transvection`、積の場合は `Matrix.toLinearEquiv` で因子の equiv を作って `g' = (g' * gA'⁻¹) * gA'` と分ける。補助: `toMatrix_injective` / `toMatrix_mul_equiv` / `det_toMatrix_ne_zero` |
| perfect | ✅ **2026-08-03**: `commutator_linearEquiv_eq_top` (`LinearGroupSimple.lean`)。`t_{w,h}` に対し `v := exists_mem_ker_ne_zero_ne h`、`g := exists_dual_eq_zero_eq_one`、`commutator_transvection` で `t_{w,h} = ⁅t_{v,h}, t_{w,g}⁆` (𝔽₂ で `-1 = 1`)。`is_generator` と合わせて `commutator = ⊤` |
| **🎯 `GL(V)` 単純** | ✅ **2026-08-03**: `isSimpleGroup_linearEquiv` — mathlib の `IwasawaStructure.isSimpleGroup` に `linearEquivIwasawaStructure` を食わせる。`dim V ≥ 3` + 2 元体 |
