...
正しい次 frontier は Coq の theorem が本来そうであるように **arbitrary-σ 版 (11.8)** を
現行 T-side carrier 上で組むこと。具体的には上流順に:

1. `exists_coherent_extension_h114_of_orthogonal` の arbitrary-grid 版
   (`h114 : tau(mu0-zeta) = sum grid_i0 - nu(zeta)`);
2. (11.8.2)+(11.8.5) の rowwise alpha-image を arbitrary grid へ一般化
...
引数として使う proof generalization であり、次 iteration は (1) から進める。
