...
- [ ] **98: `(5.8)` + `(5.13.b,c,d)` (一般化分解数の直交関係) + block orthogonality**
  ...
  **設計 (2026-08-04、(5.2) landing 直後に確定)**。(5.8) は「`χ ∈ Irr(B)` なら `d^x_{χφ}` の台は
  ...
  - [ ] **段 98 の残り = (5.10)/(5.11)/(5.12)/(5.13)**。`Irr(B)` の**族**と類関数の
    ...
    - [ ] **(5.11) block orthogonality**: `g_p` と `h_p` が非共役なら
          `Σ_{χ∈Irr(B)} χ(g) χ(h⁻¹) = 0`。(5.10) を `θ = Σ_χ χ(h⁻¹) χ` に適用し、
          第二直交関係で `θ` が `S(g_p)` 上消えることを言う。
          必要な追加物: `θ` の `Irr(G)` 展開の係数が `χ(h⁻¹)` であること
          (`eq_ordinaryCoeff` で出る) と第二直交関係 (`sum_character_inv_mul_character`、既存)。
    - [ ] (5.12)/(5.13) はさらに `p`-section 上の類代表の全数え上げが要る (後回し可)。
