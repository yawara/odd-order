...
1. **`U` を実際に構成する** (段 204-206 の結合係数から): `𝓑 = {ε_j χ_j⁰ : j ≠ j₀}` に対し
   `μ = Σ_φ u_{μφ} η_φ` を書き下す。`μ` は `Σ_{χ∈Irr(B₀)} a_χ χ⁰` (段 206) で、各 `χ⁰` は
   `𝓑` の整数結合 (段 195-196 の `D_𝓑`) なので合成すればよい。
2. `C_𝓑 = D_𝓑ᵗ D_𝓑 = 1 + δ` を書き下す (段 195-196 から機械的)。
3. **(7.6)** (`P ⊴ G` が `p`-部分群で `G/C_G(P)` が `p`-群 ⟹ `Bl(G/P) ≃ Bl(G)`,
   `IBr` 一致, `C_B = |P| C_B̄`) — 未着手の最大の塊。
4. その後 BS 本証明 pp.141-146 の残り (`Q₈` の指標表 / `Aut(Q₈)` 相当 / 二面体群)。
 - **整数性**は Navarro **Lemma (3.16)** (「各 `φ ∈ IBr(B)` は `{χ⁰ : χ∈Irr(B)}` の整数結合」)
   を要する。⚠ **repo に無い** (実測: `(3.16)` の grep は BG の別番号しかヒットしない)。
   ただし `C_𝓑 = UᵗCU` の等式自体は `K` 上で十分なので、整数性が本当に要るのは
   「`D^t_i` が整数列」を言う所 — そちらは `t` が対合であることからの別ルート (段 178 型) が
   使える見込み。
⟹ 次の作業単位: (i) Cartan 正則性の実測と `l(B₀) = 3`、(ii) `U` の `K` 上の構成、
   (iii) `C_𝓑 = 1 + δ`。その後 (7.6)。
