...
⟹ これは単独で複数 session 規模の塊。

##### 分裂は 2 つの独立な条件に分解する (2026-08-06 の整理)

`K[G] ≅ ∏ Matrix_{n_i}(D_i)` (Wedderburn, 標数 0 なので半単純) から出発すると、
`K[G] ≅ ∏ Matrix(K)` は
**(i) 各単純成分の中心 `Z(D_i)` が `K`** かつ **(ii) 各 `D_i = Z(D_i)` (Schur 指数 1)**
と同値。

- **(i)** は `Z(K[G]) ≅ ∏ Z(D_i)` と「`Z(D_i)` は指標値が生成する」から、
  `ζ_{exp G} ∈ K` で片付く (指標値は `m` 乗根の和)。
- **(ii)** は 2 通りの道がある:
  * **(ii-a) Brauer 帰納定理 + Schur 指数**: 古典的な道。repo に帰納定理は在るが
    Schur 指数の機構はゼロ。
  * **(ii-b) `Br(Frac B) = 0`**: `Frac(𝕎(𝔽̄_p))` もその有限拡大も
    「剰余体が代数閉な完備離散付値体」なので Lang の定理 (C₁) より Brauer 群が自明。
    ⟹ 有限次元斜体は存在せず `D_i = Z(D_i)`。付値の斜体への延長 + 剰余斜体が
    代数閉体上有限次元 ⟹ 自明、という道でも良い。
...
##### ❌❌ (iii) は**死んだ** — API の都合でなく**数学的に偽** (2026-08-06、実施して判明)

段 309-310 で移送補題そのものは完成し (`eq_zero_of_vecMul_map` +
`eq_zero_of_vecMul_brauerCharacterMatrix{,_fractionRing}`、`BrauerBasis` の binder は
`[IsFractionRing 𝒪 K]` → `[FaithfulSMul 𝒪 K]` に弱まった)、
続けて 53 file の binder 掃除を実施したところ **`LatticeRepresentation.lean` で止まった**。
そこで初めて `IsFractionRing` の**本当の**用途が判明した。

**止まった箇所**: `Module.Basis.extendOfIsLattice`
(mathlib `Algebra/Module/Lattice.lean:138`) が `[IsFractionRing R K]` を要求する。
そしてこれは**外せない — 仮定を落とすと定理が偽**:

> 反例: `R = ℤ`, `K = ℝ`, `V = ℝ`, `L = ℤ + ℤ√2 ⊆ ℝ`。
> `L` は有限生成で `span_ℝ L = ℝ` なので mathlib の `Submodule.IsLattice ℝ L` を**満たす**が、
> その ℤ-基底 `(1, √2)` は `ℝ` 上一次従属。

**より重要な、一段深い理由 (これが決定打)**。`F := Frac 𝒪`、`K ⊇ F`、`V` を `dim_K V = n` の
`K[G]`-加群とする。`V` の `𝒪`-形 (= `G`-安定で `𝒪`-階数 `n` の束 `L`) が存在するなら、
`L` の `𝒪`-基底は `n` 個で `V` を `K` 上張るので **`K`-基底**であり、
`V ≅ K ⊗_F (F ⊗_𝒪 L)` — つまり **`V` は `F` 上実現可能**。逆も明らか。

⟹ **`𝒪`-形が全ての `K`-既約に存在する ⟺ `F = Frac 𝒪` が分裂体**。
`K` を `Frac 𝒪` から切り離しても、**分裂体であることの要求は `F` 側に残る**だけで
何も回避できない。

**具体的な破綻**: `𝒪 = 𝕎(𝔽̄_p)` (`F` は `p'`-乗根を全部持つが `ζ_p` を持たない)、
`G = ℤ/p`、`χ : G → K^×` を忠実な線型指標 (`χ(g) = ζ_p`)、`V = K` とすると、
`0 ≠ v ∈ V` を含む `G`-安定な有限生成 `𝒪`-部分加群は必ず `𝒪[ζ_p]·v` を含み、
これは階数 `p−1 > 1 = dim_K V`。⟹ **形が無く、還元 (= 分解行列の行) が定義できない**。

⟹ **(iii) は恒久的に閉じる**。「係数体を代数閉に取れば分裂が無償」という道は無い。
`Frac 𝒪` が分裂体であること自体を示すしかなく、**残る選択肢は (ii-a) / (ii-b) のみ**。

⚠ 段 309-310 は**破棄しない** — `eq_zero_of_vecMul_map` は独立に正しい一般補題で、
`BrauerBasis` は「`𝒪` が埋まる任意の体上で Brauer 指標表は一次独立」という真に一般化された
...
### 📖 原文 pp.141-143 の実読 (2026-08-06、ページ画像で確定)

⚠ **これ以前の p.142 の記述 (2026-08-05 の要約) は不正確だった** — `u_j` は 3 本あり、
符号パターンは `(−,−) / (−,+) / (+,−)`。以下が画像で確定した内容。

#### p.141 "Analysis at t"

`P` は `C_G(t)` の Sylow-2 ⟹ `C_G(t)/⟨t⟩` の Sylow-2 は `ℤ₂×ℤ₂`。`P` の位数 4 の元が
`C_G(t)`-共役ゆえ `C_G(t)/⟨t⟩` の対合は全て共役 ⟹ **Cor (7.4) の仮定が満たされる**。
`b̄₀` の basic set `{1 = ψ₀, ψ₁, ψ₂}` (**奇数次数**) で Cartan は `c_ij = 1 + δ_ij`。
(7.6) の remark でこれは `b₀ = B₀(C_G(t))` の basic set でもあり、**Cartan は 2 倍**。

  `χ_i(tu) = ∑_{j=0}^{2} d^t_{χ_i ψ_j} ψ_j(u)`   (`u ∈ C_G(t)⁰`)
  列形: `χ(tu) = D^t_0 + ψ_1(u) D^t_1 + ψ_2(u) D^t_2`

* **`d^t_{00} = 1`, `d^t_{01} = 0`, `d^t_{02} = 0`** (一般化分解数の一意性; `χ_0 = 1_G`)
* `D^t_j` は**整数列** (`t` が対合)
* (3) `(D^t_i, D^t_j) = 2(1 + δ_ij)` / (4) `(D^y_0, D^t_j) = 0` / (5) `(χ(1), D^t_j) = 0`
* `τ` が `P` の既約指標なら `τ(t) ≡ τ(y) mod 2` ⟹ **`ξ(t) ≡ ξ(y) mod 2`** (全ての `ξ ∈ Irr(G)`)

#### p.142

`ψ_i(1)` が奇数なので
`D^t_0+D^t_1+D^t_2+D^y_0 ≡ D^t_0+ψ_1(1)D^t_1+ψ_2(1)D^t_2+D^y_0 = χ(t)+χ(y) ≡ 0 mod 2`。
⟹ **3 本の整数列**が定義できる:

```
2u_1 = D^y_0 + D^t_0 − D^t_1 − D^t_2
2u_2 = D^y_0 + D^t_0 − D^t_1 + D^t_2
2u_3 = D^y_0 + D^t_0 + D^t_1 − D^t_2
```

(1)-(5) から `(χ(1), u_j) = 0`, `(D^y_0, u_j) = 2`, `(u_i, u_j) = 1 + 2δ_ij`。
⚠ 検算に `(D^y_0, D^y_0) = 4` (= "analysis at y" の Cartan `(4)`) を使う。
各 `u_i` の第 1 成分は 1、`(u_i,u_i) = 3` ⟹ **非零成分ちょうど 3 個**、値は `±1`。
`(χ(1),u_i) = 0` と `χ_0(1) = 1` ⟹ **`{1,1,−1}`**。
記号を選んで `(u_1)_3 = (1, δ_1, δ_2)ᵀ` (`δ_1δ_2 = −1`) とし、
**`(u_j)_3 = (1,0,0)ᵀ` (`j = 2,3`)** を示す (`(u_1,u_2) = 1` から `δ_1ε_1 + δ_2ε_2 = 0`、
`δ_1δ_2 = −1` で `ε_1 = ε_2`、`u_2` の非零成分が `{1,1,−1}` ゆえ `ε_i = 0`)。

#### p.143

`(D^y_0)_3 = (1, f_1, f_2)ᵀ` (`f_i ∈ {0,±1}`)、`(D^y_0,u_1) = 2` から
`1 + δ_1(f_1 − f_2) = 2` ⟹ 記号を選んで `(D^y_0)_3 = (1, δ_1, 0)ᵀ`。
`2(u_3 − u_1) = 2D^t_1`, `2(u_2 − u_1) = 2D^t_2` より
**`(D^t_1)_3 = (0, −δ_1, −δ_2)ᵀ = (D^t_2)_3`**、
`(D^t_0)_3 = (2u_1 − D^y_0 + D^t_1 + D^t_2)_3 = (1, −δ_1, 0)ᵀ`。
`χ(t) = D^t_0 + ψ_1(1)D^t_1 + ψ_2(1)D^t_2` から

* `χ_1(t) = −δ_1(1 + ψ_1(1) + ψ_2(1))`, `χ_2(t) = −δ_2(ψ_1(1) + ψ_2(1))`
* **(6) `1 + δ_1χ_1(t) − δ_2χ_2(t) = 0`**
* **(7) `1 + δ_1χ_1(1) + δ_2χ_2(1) = 0`** (`(χ(1),u_1) = 0` から)

そのあと群論に戻る: **「2 つの対合の積は奇位数」** — `o(t_1t_2)` が偶なら
`⟨t_1,t_2⟩` は位数 `2o(s)` の二面体群で `G` が位数 4 の基本アーベル部分群を持つが、
`P = Q₈` はそれを持たない。⚠ **pp.144-146 は未読**。

### ✅ 段 324 完了 (2026-08-06) — p.142 の整数列の中核

`Algebra/ThreeNormColumn.exists_pair_of_sum_sq_eq_three` — **表現論を含まない純粋な整数の補題**
として切り出した: ノルム 3 の整数列で `i₀` 成分が `1`、次数の列 (すべて `≥ 1`、`i₀` で `= 1`)
と直交するものは、非零成分がちょうど 3 個で `1, 1, −1`、しかも `deg j = 1 + deg i`。

併せて `SumSquaresFour` の「零を許す」2 本を**総和 `≤ 4` に一般化**
(`eq_zero_or_one_or_neg_one_of_sum_sq_le_four` / `card_filter_ne_zero_of_sum_sq_le_four`)。
`±2` の排除に要るのは「`4 + 1 > 総和`」だけで総和が 4 である必要はない。呼び出し 2 箇所更新済。

### ⏭ 段 325 の着手点 — 列 `u_j` の構成と内積計算

段 324 が「ノルム 3 + 直交 ⟹ `{1,1,−1}`」を与えたので、残りはその**仮説の供給**:

1. `2u_j = D^y_0 + D^t_0 ∓ D^t_1 ∓ D^t_2` が**整数列**であること
   (`D^t_0+D^t_1+D^t_2+D^y_0 ≡ 0 mod 2` から; その入力は `ξ(t) ≡ ξ(y) mod 2` = 段 289 と
   `ψ_i(1)` が奇数 = (7.4) の "odd degrees")。
   ⚠ **(7.4) の出力に「`ψ_i(1)` は奇数」が入っているか未実測** — `PrincipalBlockBasicSet` /
   `IntegralBasicSetMatrix` を確認すること。無ければそこを足すのが先。
2. `(u_i,u_j) = 1 + 2δ_ij` / `(χ(1),u_j) = 0` / `(D^y_0,u_j) = 2` の**内積計算**
   — 入力は (3)(4)(5) + `(D^y_0,D^y_0) = 4` + `(χ(1),D^y_0) = 0`。
   これも段 324 と同じく**純粋な双線型代数**として切り出せる (4 本の列と 1 本の次数列の
...
### 📖📖 原文 pp.144-145 の実読 (2026-08-06) — **BS の証明は p.145 で終わる。未読ページはもう無い**

⚠⚠ **重要**: p.145 後半の **(7.7)** と p.146 は **Z\*-定理**の証明用で、
**issue 0147 (Q₈ Brauer–Suzuki) のスコープ外**。⟹ BS 本体 = **pp.139-145** で、全て実読済。

#### p.144 — Burnside の類和公式で (8)(9) を出す

`K` = `t` の類、`M` = 2-特異元 `s` の類とすると、**「2 つの対合の積は奇位数」** (p.143) より
`|{(x,y) ∈ K×K | xy = s}| = 0`。Burnside の公式 (Isaacs *Characters* Prob (3.9)) で

  `∑_{χ ∈ Irr(G)} χ(s⁻¹)χ(t)²/χ(1) = 0`  (全ての 2-特異 `s`)

⟹ 類関数 `∑_χ (χ(t)²/χ(1))·χ` は `t` と `y` の 2-section で 0 ⟹ **(5.10)** で `Irr(B₀)` に制限:

  **(8)** `∑_{χ ∈ Irr(B₀)} χ(s⁻¹)χ(t)²/χ(1) = 0`  ⟺ **(9)** `(χ(s), χ(t)²/χ(1)) = 0`

`u_1 = ½(D^y_0 + D^t_0 − D^t_1 − D^t_2)` が 2-特異 `s` の列 `χ(s)` の **ℂ-一次結合**であること:
`D^y_0 = χ(y)`、`χ(tu) = D^t_0 + ψ_1(u)D^t_1 + ψ_2(u)D^t_2` なので、**3×3 行列**
`[[1,1,1],[ψ_1(1),ψ_1(w),ψ_1(v)],[ψ_2(1),ψ_2(w),ψ_2(v)]]` が可逆になる 2-正則 `w, v ∈ C_G(t)` を
取り (basic set の一次独立性から「elementary linear algebra」)、
`(α,β,γ)` を `[1,−1,−1]ᵀ` の解とすると `αχ(t) + βχ(tw) + γχ(tv) = D^t_0 − D^t_1 − D^t_2`。

#### p.145 — 最後の代入

(9) から `(u_1, χ(t)²/χ(1)) = 0` ⟹ `1 + δ_1χ_1(t)²/χ_1(1) + δ_2χ_2(t)²/χ_2(1) = 0` ⟹
**(10)** `χ_1(1)χ_2(1) + δ_1χ_1(t)²χ_2(1) + δ_2χ_2(t)²χ_1(1) = 0`。
(6)(7) を代入して `(χ_1(1) − χ_1(t))² = 0` ⟹ **`t ∈ ker χ_1`** ⟹ 証明終わり。

### ✅ 段 326 完了 (2026-08-06) — 最後の一歩

`Algebra/SignRelationSolution.eq_of_sign_relations` — (6)(7)(10) ⟹ `χ_1(1) = χ_1(t)`。
**群論を含まない 6 元の環等式**として切り出した (側条件は `δ_i² = 1` のみ)。
⚠ `linarith` は使えない (`R` は順序体でなく整域) — `sub_eq_zero.mp (pow_eq_zero_iff …)` で閉じる。

✅ **Burnside の類和公式は repo に在る (2026-08-06 実測)**:
`RepresentationTheory/ClassSumCoefficientFormula.lean:121`
`classSumCoeff_mul_centralizer_card_eq_sum_irreducibleCharacter` —
`a_{ijk}·|C_G(s)| = ∑_χ (|K_i|χ(x_i)·|K_j|χ(x_j)/χ(1))·χ(s⁻¹)`。
⚠ **ただし `ℂ` と repo の `IrreducibleCharacter G` で述べられている**。modular 鎖は
`K = ℂ_[p]` と `wedderburnRepresentation eG j` を使う ⟹ **係数体の橋渡しが設計課題**
(選択肢: (i) 公式を任意の標数 0 分裂体で証明し直す、(ii) `ℂ` ↔ `ℂ_[p]` の指標対応を作る、
(iii) 消費側を `ℂ` 版の類和で書く)。⚠ 着手時に `ClassSumCoefficientFormula` の証明を読んで
`ℂ` 固有の議論 (直交関係・代数的整数性) がどれだけ在るか実測すること。

### ⏭ 段 327 以降の残タスク (2026-08-06 時点の全量、**未読ページは無い**)

| # | 内容 | 状態 |
|---|---|---|
| A | p.143 の `(D^t_j)_3` 決定 → (6)(7) | 整数線型代数 + 「記号の選択」(添字の付け替え) |
| B | 「2 つの対合の積は奇位数」 | 群論。`Q₈` は Klein four 部分群を持たない (段 218 の decide 系が使える見込み) |
| C | Burnside → (8)(9)(10) | 公式は在る。**係数体の橋渡しが要る** |
| D | p.144 の 3×3 可逆行列 (basic set の一次独立性) | 「elementary linear algebra」だが repo に一次独立性の形が在るか要実測 |
| E | 段 324/325 の仮説供給 (`d^t_{00}=1` 等) | 一般化分解数の一意性から。要実測 |
| F | `𝓞_ℂ_[2]` での modular datum 一式の instantiation (`G` / `C_G(t)` / `C_G(y)` / `C_G(t)/⟨t⟩`) | **最大の塊**。段 321b が `hconv` を供給済 |
...
### 🎯🎯 残タスク D は**消滅した** (2026-08-06) — 3×3 可逆行列も basic set の一次独立性も要らない

原文 p.144 の「elementary linear algebra で 2-正則 `w, v` を選んで 3×3 行列
`[[1,1,1],[ψ_1(1),ψ_1(w),ψ_1(v)],[ψ_2(1),ψ_2(w),ψ_2(v)]]` を可逆にする」段は、
**`IBr` の一次独立性を使えば丸ごと不要**になる。

`Θ_χ := χ(t)²/χ(1)` と置く。原文が (9) から `(u_1, Θ) = 0` を出す道は:

1. Burnside + (5.10) ⟹ `θ := ∑_{χ∈Irr(B₀)} Θ_χ·χ` は `t` の 2-section 上で消える。
2. `θ(tu) = ∑_μ (∑_χ Θ_χ d^t_{χμ}) μ(u)` (一般化分解数の定義) が全ての 2-正則 `u` で 0。
3. **`IBr(C_G(t))` の一次独立性** (`BrauerBasis.eq_zero_of_sum_irreducibleBrauerCharacter_eq_zero`、
   **既存**) ⟹ **`∑_χ Θ_χ d^t_{χμ} = 0` が全ての `μ ∈ IBr(C_G(t))` で成立**。
4. basic set 列は `IBr` 列の `u`-結合 (`basicDecompositionNumber` の定義) なので
   **`(Θ, D^t_j) = 0` (`j = 0,1,2`)** — これは **段 323
   `sum_mul_basicDecompositionNumber_left_eq_zero` がそのまま与える形**。
5. `y` は 2-特異なので `(Θ, D^y_0) = (Θ, χ(y)) = 0` も同様。
6. `2(Θ,u_1) = (Θ,D^y_0) + (Θ,D^t_0) − (Θ,D^t_1) − (Θ,D^t_2) = 0` ⟹ `(Θ,u_1) = 0`
   — これは **段 325 `two_mul_sum_mul_of_halfSum` がそのまま与える形**。

⟹ 原文の `α, β, γ` を解く段は「3 本の方程式から 3 つの係数を決める」ための道具にすぎず、
`IBr` 独立性で**全ての `μ` について一気に 0 が出る**ので迂回できる。
**basic set の一次独立性 (repo に無い) も不要**。

⟹ **残タスク D は削除**。代わりに必要なのは:

* **D'** = 「`θ` が `x` の `p`-section 上で消えるなら `∑_χ c_χ d^x_{χμ} = 0` (∀`μ`)」の**抽出**。
  これは `BlockPartVanishing.blockPart_eq_zero_of_forall_pSection` (5.10) の**証明の中に既に在る**
  (`hexp : ∑_j (∑_i c_i d_{ij}) φ_j(y) = θ(xy)` + `IBr` 独立性)。
  ⟹ standalone な定理として切り出すだけ (新しい数学は無い)。

⚠ **これで p.144 の残りは Burnside (タスク C) だけになった**。
...
#### 段 352b の依存を 1 段掘った結果 (2026-08-06) — **さらに `|G|_p ∣ Φ_φ` が要る**

(3.4′) の非自明部分 (`f = ∑_{i∈Irr_S} e_i` が `𝒪` 係数) を詰めた。**係数の計算は綺麗に終わる**:

* `Irr_S := {i | ∃μ ∈ S, d_{iμ} ≠ 0}` と置くと、`S` が linking で閉じているので
  * `φ ∈ S` なら `d_{iφ} ≠ 0 ⟹ i ∈ Irr_S` (定義)、
  * `φ ∉ S` なら `i ∈ Irr_S` について `d_{iφ} = 0` (linking 閉包)。
* ⟹ `∑_{i∈Irr_S} χ_i(1)χ_i(g⁻¹) = ∑_φ φ(1)(∑_{i∈Irr_S} d_{iφ}χ_i(g⁻¹)) = ∑_{φ∈S} φ(1)Φ_φ(g⁻¹)`
  (`χ_i(1) = ∑_φ d_{iφ}φ(1)` は `1` が `p`-正則ゆえ)。
* `f_S` の `g` での係数 = `(1/|G|)∑_{φ∈S}φ(1)Φ_φ(g⁻¹)`。`p`-特異な `g` では
  `Φ_φ(g⁻¹) = 0` (`projectiveIndecomposableCharacter_eq_zero`、既存) で自動的に 0。
* `1/|G|_{p'} ∈ 𝒪` なので、**残る唯一の要件は `|G|_p ∣ Φ_φ(x)`**。

**`|G|_p ∣ Φ_φ` は repo に無い** (grep 実測)。古典的証明は「射影加群の Sylow への制限は射影 ⟹
`p`-群上の射影加群は自由 ⟹ 指標は正則指標の倍数」で、射影加群論 (射影被覆・制限の射影性・
`p`-群上の自由性) が要る = **これ自体が Navarro Ch.2 のもう 1 段**。

⟹ **依存の鎖**: `|G|_p ∣ Φ_φ` → (3.4′) → Problem (3.4) → (7.6) のブロック全単射 →
`hd` → `hT` / `hb0`-`hd0` → 組み立て。**Navarro Ch.2-3 の本文材料なので in-scope**、
ただし multi-session。次の着手点は **`|G|_p ∣ Φ_φ(x)` を最下段から**。

⚠ 他の近道も潰した: `B₀(C)` の basic set を商群を経由せず直接取る案は
`|IBr(B₀(C))| = 3` を要求し、これも同じ (7.6) の内容。`k[G]` の中で `f_S` を作る案は、
`k[G]` の中心原始冪等元がまさにブロック冪等元なので循環 (通常指標のある `𝒪` 側でしか作れない)。

#### ⚠ 訂正 (2026-08-06、同日) — **`|G|_p ∣ Φ_φ(x)` は偽**。段 352b の見積りを取り消す

前項で「残る唯一の要件は `|G|_p ∣ Φ_φ(x)`」と書いたが、**`x ≠ 1` では成り立たない**。

**反例**: `G = S₃`, `p = 3` (`|G|_3 = 3`)。主ブロックの分解行列は
`χ_triv ↦ (1,0)`, `χ_sgn ↦ (0,1)`, `χ_2 ↦ (1,1)` なので `Φ_{1} = χ_triv + χ_2`。
`Φ_1(1) = 1 + 2 = 3` は 3 で割れるが、互換 `t` (位数 2 = `3`-正則) では
`Φ_1(t) = 1 + 0 = 1` で **3 で割れない**。

正しい形は **`|C_G(x)|_p ∣ Φ_φ(x)`** (Navarro Ch.2)。しかしこれでは
`c_g = (1/|G|)∑_{φ∈S}φ(1)Φ_φ(g⁻¹)` の整数性は出ない
(`|C_G(g)|_p/|G|_p = 1/|K|_p`、`K` = `g` の類、が残る)。
⟹ **`f_S ∈ 𝒪[G]` は項別可除性では出ない**。linking 閉包性を本質的に使う議論が要る
(`S` = 全体なら列直交で `c_g = δ_{g,1}` になる、という形で閉包性が効く)。

#### 収穫: `hnil` からブロック冪等元の原始性は**即座に出る**

`hnil : blockCharacterPi z = 0 → IsNilpotent z` は
「`Z(k[G]) → ∏_B k` の核が nil」を意味する。冪等元は nil 核に沿って**一意に持ち上がる**ので、
**`Z(k[G])` の冪等元 = ブロックの部分集合**、したがって `e_B` は原始的。
⟹ Problem (3.4) の矛盾を出す側 (「`ω_μ(f̄) = 1`, `ω_τ(f̄) = 0` は同一ブロックでは不可能」) は
**repo の枠組みで既に自明**。難しいのは `𝒪` 側で `f_S` を**作る**方だけ。

#### ⟹ 段 352b の正しい着手点 (次のセッションはここから)

`Z(𝒪[G])` の原始冪等元 ↔ 「`ω_i ≡ ω_j mod 𝔪`」による `Irr(G)` の類、という
**order の連結成分**の議論 (Curtis–Reiner / Nagao–Tsushima 流) が本筋。
repo に `Z(𝒪[G])` の order 構造・`𝔪`-進完備性まわりの資産があるか実測してから設計すること
(`Algebra/BlockIdempotent.lean` / `Modular/BlockIdempotentLift.lean` /
`Algebra/AlgClosedIdempotentLift.lean` が候補)。
**`|G|_p ∣ Φ_φ` を作りに行くのは誤り** (偽なので)。

### 🎯 段 352b の設計確定 (2026-08-06、**原文 Ch.3 を実読**) — 前項の「訂正」を再訂正

`references/navarro/characters-and-blocks.pdftotext.txt` の Ch.3 (pp.52-55) を読んだ結果、
**必要な連鎖が全部原文に在り、しかも短い**ことが判明した。前項で「`|G|_p ∣ Φ_φ` は偽」と
書いたが、**原文が使うのは `x = 1` での `|G|_p ∣ Φ_φ(1)` だけ**で、これは真
(`S₃` の反例は `x ≠ 1` の話であり、ここでは不要)。**係数計算で `Φ` と `φ` の役割を
取り違えていた**のが誤りの原因。

#### 原文の連鎖 (行番号は pdftotext)

* **(3.6)** (line ~2360): `A` が Brauer グラフの連結成分の合併なら
  `∑_{χ∈A} χ(x)χ(y) = ∑_{φ∈S} Φ_φ(x)φ(y)` (`S` = `A` に対応する `IBr`)。
* **(3.7) Weak Block Orthogonality** (line 2393): (3.6) + `Φ` が `p`-特異元で消えること。
* **(3.8) COROLLARY (Osima)** (line 2427): 🎯 **`A` が連結成分の合併 ⟹ `f_A ∈ Z(𝒪G)`**、
  かつ `f_A(K) = 0` (`K` が `p`-正則でないとき)。
  証明 = 係数は `f_A(K) = (1/|G|)∑_{φ∈S} Φ_φ(1)·φ(x⁻¹)` で
  (**`Φ_φ(1)` と `φ(x⁻¹)`** — 私が逆にしていた)、`p`-特異なら `Φ` の消滅で 0、
  `p`-正則なら **Dickson の定理 `|G|_p ∣ Φ_φ(1)`** と `1/|G|_{p'} ∈ 𝒪` で整。
* **(3.9) THEOREM** (line 2466): 🎯 **`Irr(B)` は Brauer グラフのただ 1 つの連結成分**。
  証明 = Lemma (3.5) (`Irr(B)` は連結成分の合併) + (3.8) + 「`λ_χ` はブロックにしか依らない」。
* **(3.10)** = `D_B` は `(* 0; 0 *)` の形にならない = **Problem (3.4) 相当**。
* Ch.9 冒頭 (line 8121-) はこの (3.9)/(3.11) をブロックの被覆に使う。

#### ⟹ 形式化の順序 (bottom-up、各段は独立に landing 可能)

1. **Dickson: `|G|_p ∣ Φ_φ(1)`**。安い証明が在る: `Φ_φ` は `p`-特異元で消えるので
   Sylow `P` への制限は `1` の外で 0 ⟹ `[Φ_φ|_P, 1_P] = Φ_φ(1)/|P|`、
   そして `Φ_φ = ∑_i d_{iφ}χ_i` (`d ∈ ℤ`) ゆえ内積は整数。
   ⟹ **射影加群論は不要** (前々項の見積りは過剰だった)。
2. **(3.6)**: 連結成分の合併に対する二重和の等式 (分解行列の帳簿計算; linking 閉包性を使う)。
3. **(3.8) Osima**: `f_A ∈ Z(𝒪G)`。
4. **(3.9)**: `Irr(B)` = 単一連結成分。⟹ `IBr(B)` も単一連結成分 (= Problem (3.4))。
5. ⟹ (7.6) のブロック全単射 ⟹ `hd` ⟹ `hT` / `hb0`-`hd0` ⟹ 組み立て。
