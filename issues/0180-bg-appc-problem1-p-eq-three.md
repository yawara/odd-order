...
最終 assembly 監査 (effort max) の計 6 本すべて CONFIRMED・fatal 0)。witness 排除のケース木: 衝突あり ⟹
(B2)-elim / 非退化 loop ⟹ family capstone / 共謀 ⟹ (EX) ⟹ master formula ⟹
Δ=0・Σ̄=0・λ∉𝔽₃・𝔽₃ 残 4 候補がすべて初等的に矛盾 (使用仮説は χ(−1)=−1・e 奇・
e³-cube・K≠0・|T|≥4 のみ — equidistribution・鳩の巣すら不要)。残り = 統合証明文書 +
...
> 旧: 「衝突たちが `F` を張る」 → 「衝突 1 個で `Tr S ≠ 0`」 →
> **今: 「同じ剰余類に Paley 点が 2 個」または gcd 不等式 `gcd(E−1,n) ≥ 2·gcd(E²+E+1,n) + 1` 1 本**

まで弱まった。**トレースの計算も `δ` の平方判定も要らない。**

### 全 `q` で無条件に成り立つもの (すべて Lean 化済・axiom-clean)

| # | 内容 | Lean |
|---|---|---|
| 1 | **定理 1** — `e` が Frobenius 冪なら witness 無し | `false_of_centralizing` |
| 2 | **定理 2** — `e` が Frobenius 冪でなければ `G` は非可解 | `not_isSolvable_of_exp` |
| 3 | **トレース判定** — 衝突 1 個 (`Tr S ≠ 0`) で witness 無し | `false_of_collisionPair_trace_ne_zero` |
| 4 | `ker Tr` は唯一の Frobenius 安定超平面 (判定 3 が弱くて済む理由) | `ker_eq_ker_trace_of_frobenius_stable` |
...
* **奇素数 `q ≤ 43` は全て** — 定理 1 が `3,5,11,17,23,37,43`、トレース判定の証明書が
  `7,13,19,29,31,41`。
* **`q = 53`, `q = 79`, `q = 101`** — **gcd 判定 (Theorem A)** で探索なしに決着。
  `n = (3^q−1)/2` の分解でブロック列挙の完全性も検証済 (`q = 101` は全分解、`53`/`79` は
  余因子を分解して `≡ 1 mod 3` の素因数が無いことを確認)。
* **`q = 47`, `q = 73`** — 8 個の exotic 指数のうち 4 個 (単一ブロックの support のみ判定成立)。
  ブロック列挙の完全性は検証済。
* ⚠ ChatGPT は `q = 109, 137, 139, 149, 157, 163, 179` も判定成立と主張しているが、
  **ブロック列挙の完全性が未検証** (余因子が 43〜81 桁で分解が届かない) ので「決着」と書かない。

### 追記 (2026-08-12): `E ↔ E²` の共役を証明 — 未説明の経験則が 1 つ消えた

悉皆列挙で見つかっていた「`e` と `e²` でファイバー分布が**完全に**一致する (偶然ではありえない)」
は**未追跡**のまま残っていたが、初等的に決着した (`notes/bg/appC_problem1_partial_resolution.md`
「`E` と `E²` は共役」節、Lean 化 + AxiomsCheck 登録済)。`w = D(a)` として

```
w が平方   ⟹  b := a^E·w⁻¹ ,        w が非平方 ⟹  b := −(a+1)^E·w⁻¹
```

はどちらも `b ∈ T` かつ `D_{E²}(b) = (w⁻¹)^{E²}` (`w` のみに依存) ⟹ `D_E` のファイバーが
`D_{E²}` のファイバーへ全単射に写る。**⟹ (B1) は各対 `{E, E²}` の片方について示せば十分**。
⚠ 標数 3 は使わない (`char ≠ 2`・`|F| ≡ 3 mod 4`・`E` 奇数・`E³ ≡ 1` のみ)。
素体での数値検証 74 ケース (非自明な指数すべて、`p < 600`) で独立確認済。

### 追記 (2026-08-12): `δ ∈ U` は本物の条件ではなかった

判定法が要求していた「`δ = r^E − p^E` が平方」は、**順序を入れ替えれば必ず満たせる** —
`q` 奇数 ⟹ `−1` 非平方 ⟹ `δ` と `−δ` の平方類は必ず反転するので、順序対
`(p,r)` / `(r,p)` のちょうど一方が条件を満たす。⟹ **(B1) は「衝突が存在する」だけになった**
(平方性という 2 つ目の equidistribution 仮定が消えた)。悉皆列挙の
...
* **(B1) の一般証明** — 道具立ては Katz の総説
  ...
  古典 Weil が自明になる領域は [arXiv:2211.07739](https://arxiv.org/abs/2211.07739)。
* **行き止まりの記録**は `notes/bg/appC_problem1_partial_resolution.md` の
  「なぜ一般 `q` が難しいのか」「`H`-座標での言い換えと自明な繊維」
...
**数値測定** (q = 7/13 悉皆): N2 は q = 13 で trace の外側を 195/182 件殺す。q = 7 の
escape 21 衝突 (両端 trace 0) には**閉包実験** (pair 空間 `V` を C1/C2 で飽和、kill =
不動点 K1 / グラフ性違反 K2 / `Dom = F` K3) を実施 → **全 21 が最初の自己合成 1 回で死亡**
(escape の `V₀` は dim 6 = `ker Tr` 加群で、合成 1 歩が必ず加群の外に出る)。
⟹ **GF(3⁷) の全 140 衝突が有効証明書 = (B2) は q = 7 で経験的に完全消滅** (note §7)。
...
1. **点ごとの `E ↔ E²` 共役** — `Φ_E(p) = p^E/D_E(p)` で `λ(Φ_E(p)) = λ(p)^E`。
   本日こちらが独立に証明・Lean 化した `exists_paley_collision_pow_mul` と**同じ写像**
   (回答は値ごとの分布一致まで出しており、こちらの衝突レベルの全単射より強い)。
2. **Theorem A (gcd 判定)** — `a = gcd(E−1,n)`, `c = gcd(E²+E+1,n)` は `a·c = n` を満たし、
   **`a ≥ 2c+1` なら同一剰余類の衝突が必ず存在する** (鳩の巣: `Σ_C |T_C| = (a−1)/2`、
   `|T_L|` は偶数)。本日こちらが Lean 化した「同じ剰余類なら `D` 一定」の**存在保証版**。
3. **Theorem B (これが本命)** — 同一剰余類の衝突は `K(p) = K(r)` ゆえ `S = S′` となり、
   関係式が共役でなく**交換関係** `[a(t), b(St^E)] = 1` になる。
   `W = {s : [a(t), b(st^E)] = 1 ∀t}` は `F₃`-部分空間で `s ↦ s^{−E}` で閉じ、3 回反復して
   **反転で閉じる**。反転で閉じた加法部分群は Hua の恒等式で `s⁻¹W` が部分体になり、
   `q` 素数ゆえ `W = 𝔽₃` か `W = F`。前者なら `S = −1` で `Tr S = −q ≠ 0` (トレース障害)、
   後者なら `A` が `B` を中心化して `N` 可換 (定理 2 に矛盾)。
   ⟹ **同一剰余類の衝突が 1 つでもあれば witness は無い。(B2) は不要**。

⟹ **Theorem C = A + B**: `gcd(E−1,n) ≥ 2·gcd(E²+E+1,n) + 1` なら hypothesis (B) は不可能
(gcd と整数比較だけ、探索も (B2) も不要)。

### 検証状況 (こちら)

* Theorem A の組合せ論 = 素体 58 exotic 指数で全数検証、失敗 0。判定が成立する 18 例すべてで
  実際に衝突が存在。⚠ `|T_L|` の偶数性だけは**標数 3 に固有** (対合の不動点 `x = 2 = −1` が
  非平方) で素体では再現しない — 回答者の議論は正しく、こちらのテストの限界。
* Theorem B = 全ステップを手で検証、repo の `layerFieldHom` / `layerFieldHom_two_eq` の
  定義とも整合。Hua の恒等式も可換体で直接検算。**正しい**。
* **`q = 101` は一から完全検証** — 分解 `n = 33034273 · 465092326319 · 50316775668019759306202964023`
  (積 = `n`、3 つとも素数、`mod 3` は `1,2,2`) と明示 2 指数の全条件。⟹ **決着**。
* ⚠ 他の `q` (53, 79, 109, 137, 139, 149, 157, 163, 179) は**ブロック列挙の完全性が未検証**
  なので「決着」とは書かない。

### 次にやること

1. **Theorem A・B の Lean 化** (最優先)。B は (B2) を消すので判定法の質が変わる。
2. `q = 47` の残り 4 指数: Theorem A の鳩の巣は届かない (`a < c`) が、こちらの誕生日評価では
   `a² ≫ c` なので**同一剰余類の衝突はほぼ確実に存在**し、`√c ≈ 4.7×10⁵` サンプルで見つかる。
   見つかれば Theorem B で (B2) 無しに決着する。
3. 他の `q` は `n` の完全分解が要る。

## ChatGPT 4 回目 — (B1)+(B2) の一般証明専用 (2026-08-12、ユーザー指示)

上記の「再開するとしたら」をそのまま投げた。**Work サーフェスが使えないのでユーザー指示により
...
* 渡した等価形と行き止まり: APN 微分としての `D` / 自由な対合が `T` を外すこと / 偶関数 `H` 版 /
  `Fix` 部分群による構成的証明書 / Frobenius 同変性と唯一の不変超平面 /
  **効かない攻め筋** (Sidon・planar・APN 分類予想・素朴な数え上げ・合同枚挙・Gersten–Stallings・Itô)
  / `q = 13` の悉皆列挙データ (ファイバー分布が Poisson と 0.06% 一致)
* 訊いたこと: (A) **(B1) の一般証明** (道具立て = Katz の Weil 和モーメント / small subgroups、
  使うべき構造 = `E² = E⁻¹` かつ `ω` が Frobenius と可換な位数 3 の自己同型) /
  (B) **(B2) の証明**または (B2) を回避する定式化 / (C) 衝突を経由しない別の一般障害 /
  (D) witness の構成

⚠ 前回 (Chat, 2026-08-09) は 2.5 時間で network error → 再試行が 4 時間走って未完了。
**落ちる前提で 30 分間隔で監視**する。
...
3. ⚠ **`q` を 1 つずつ潰しても最終証明には到達しない** (奇素数は無限)。証明になるのは
   (B1) の一般証明だけ。**2026-08-12 の文献調査で (B1) の正体が確定**: `D(a) = (a+1)^E − a^E`
   は冪写像 `x^E` の方向 1 の差分そのもので、(B1) は
   「**`x^E` が APN (almost perfect nonlinear) でない**」+「その余分な衝突が Paley 集合に落ちる」。
   奇標数の APN 冪指数の (完全と予想されている) リストのうち `p = 3`, `n` 奇の候補
   `(3^q−1)/2 − 1`, `(3^q+1)/4 + (3^q−1)/2`, `3^q − 3` は**どれも乗法的位数 3 を持たない**
   (`q = 7, 13, 19` で実測) ⟹ **APN 分類予想を仮定すれば `x^E` は APN でない**。
   ⟹ (B1) は未解決予想に寄りかかる位置にあり、初等的な見落としではない。
   道具立て = Katz の総説 arXiv:1805.10452 / arXiv:2211.07739 (詳細は notes)。
   ⟹ **q = 47 以降の証明書は走らせない方針** (証明に寄与しないため)。
4. **一般 `q` は依然 open**。示すべきことは 2 段階に弱まった:
   **(B1)** 衝突が 1 つ存在する (= `A_E = {(u,u^E)}` が Sidon でない ⟸ `z ↦ z^E` の擬ランダム性。
   これが本丸、ChatGPT も未証明) / **(B2)** その `S` が `Tr S ≠ 0`。
   ⚠ **(B2) も未証明** — 実測では常に数個以内の衝突で当たる (18 ケースで平均 1.7 個) が、
...
## 状態 (2026-08-12 更新) — **q ≤ 43 決着**、判定法は Lean 化済
...
| **13, 19, 29** | ✅ **collision-span (rank 満杯、独立再現済) + トレース判定** |
| **31** | ✅ **トレース判定 / GAP** (両指数とも衝突 #1、2026-08-11) |
| **41** | ✅ **トレース判定 / C** (両指数とも衝突 #2、2026-08-12) |
| 47 以降 | 🟡 可解/有限奇位数は不可 (定理 2)。exotic 指数ごとにトレース判定を測れば個別決着するが、`q=47` は 1 指数あたり ~12 時間 × 8 指数 |

⟹ **`q ≤ 43` の全奇素数で hypothesis (B) は不可能** (`q = 37, 43` は exotic 指数 0 で定理 1)。
次の未決は **`q = 47`** で、探索コスト `≈ √(πQ/(8q)) = 1.5×10¹⁰` 歩 = 32 スレッドで
1 指数 ~12 時間 × exotic 指数 8 個 ≈ 4 日。⟹ 証明書方式はこの辺りが限界で、
その先は (B1) の一般証明が要る。
