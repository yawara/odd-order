...
## 📌 総括 — **部分解決で区切る** (2026-08-12)

⚠ **能動的な作業はここで一区切り**。以下が確定した内容で、これ以上は `q` を増やしても
最終証明には到達しない (奇素数は無限にあるため)。
...
**奇素数 `q ≤ 43` は全て**(定理 1 が 3,5,11,17,23,37,43 / トレース判定の証明書が 7,13,19,29,31,41)。
加えて `q = 47` は 8 個の exotic 指数のうち 4 個 (構造化指数の直接構成)。
