...
| **`e ∉ ⟨3⟩`** (`q ≥ 7` で `3 \| φ(n)`) | 🟡 **`G` は可解でない。有限奇位数群は不可** | **定理 2** (`q = 7,13,19,29,31,41` で検証) |
| `q = 7`, `e = 151, 941` | ✅ **完全決着** | 定理 2 + `[Γ_e : H] = 3` の実測 |

**⟹ 奇数位数定理を認めれば、`p = 3` の witness は有限奇位数群の中には存在しない**
(検証済の `q` について)。BG App.C が置かれている文脈そのものでは否定的に決着した。
...
* `e ∉ ⟨3⟩` なら加法性が壊れる。代わりに関係格子
  `L_e = span{ (s, s^e, s^{e²}) : s ∈ S } ≤ V³` を測ると **`L_e = V³` (フル)**。すると
...
1. **`L_e = V³ ⟺ e ∉ ⟨3⟩` を一般 `q` で証明する**。同値な言い換えは
   「`(λ,μ,ν) ≠ 0` で `Tr(λs + μs^e + νs^{e²}) = 0` (∀`s ∈ S`) となるものは無い」。
   `e` の次数が `~n/3` と巨大なので素朴な Weil 評価は効かない。**ここが本丸**。
2. `q = 13` 以降で `[Γ_e : H̄] = 3` を実測する (定理 2 と合わせれば完全決着)。
   ⚠ 関係子 `u^n` が長い (`q = 13` で `n ≈ 8·10⁵`)。生成元を素冪分解した表示にすると軽くなるはず。
3. 定理 2 の完全群 `N` をさらに潰す (`U` が `P` に既約に作用することを使う)。
