...
[`notes/bg/appC_problem1_centralizing_case.md`](../notes/bg/appC_problem1_centralizing_case.md))。

| 場合 | 状態 | 根拠 |
|---|---|---|
| `q = 3` (`e = 1, 3, 9` 全部) | ✅ **否定で確定** | 万能完備化 `Γ_e` の合同枚挙 (自前 GAP で再現) |
| **中心化作用** (`σ(P₀)^y` が `σ(U)` を中心化) — 全ての奇素数 `q` | ✅ **否定で確定** | **人手証明** (計算機不要) |
| ⟹ `3 ∤ φ((3^q−1)/2)` なる `q` (= 5, 11, 17, 23, 37, 43, …) | ✅ **否定で確定** | 上の系 (書籍 p.1094 の注により作用は自動的に中心化) |
| `3 \| φ((3^q−1)/2)` なる `q ≥ 7` の**非中心化作用** (= 7, 13, 19, 29, 31, 41, …) | ❌ **未解決** | `q = 7` は ChatGPT が主張するが未検証 |

**⟹ 問題そのものは依然 open だが、「`p = 3` は可能」を主張するには
`q ∈ {7, 13, 19, 29, 31, 41, …}` で `U` に位数 3 で作用する `g` を作るしかない**、
というところまで絞られた。
...
`Γ_e` を経由せず **`G` の中だけで完結する証明**に分解できた ⟹ 中心化作用の一般 `q` 解決。
...
1. **`e ≠ 1` (非中心化作用) の一般論**。中心化作用の証明は「`(gx)³ = 1` を `U` で共役して
   `(gs)³ = 1` (`s ∈ S`) を得る」ところだけで中心化を使う。`e ≠ 1` だと共役先が
   `(g·v^{1−e²}·x^v)³ = 1` になり、`s` ごとに別の位数 3 元が付くので加法性が壊れる。
   ⟹ **ここが唯一の残り穴**。`S₀ = C_U(g)` が大きければ同じ証明が通る (`|S₀| = gcd(e−1,n)`)。
2. `q = 7` の `e ∈ {151, 941}` を自前で決着させる (ChatGPT の主張は未検証)。
   `|Γ| ≈ 1.6·10¹⁰` なので自明群上の合同枚挙は不可。`H` 上の枚挙 (index `3⁸ = 6561`) は回るが
   忠実性が別途要る。
3. **Lean 化**: 中心化作用の定理は `G` の中だけで完結する有限群論の議論なので**形式化可能**。
   必要なのは (i) 語の恒等式 `[x^{-1}g,(x^{-1}g)^x] = (gx)³`、(ii) 補題 C の消去、
   (iii) 補題 B (Paley 型の張り生成)。**(iii) の一般 `q` は Weil 評価**を要するので、
   mathlib に無ければ `q` ごとの有限検証に落とすか、`q` を固定した形で入れる。
4. 旧項目「`D` の枚挙」は**破棄**。万能完備化に置き換わった。
