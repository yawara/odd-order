...
2. ~~走らせている計算~~ → **2026-08-11 完了**。`notes/meta/gap/verify_trace_obstruction.g` で
   **q = 7, 13, 19, 29, 31 の exotic 指数 16 個すべてが DECISIVE**。とくに **q = 31 が決着**
   (両指数とも衝突 #1 で `Tr S ≠ 0`、サンプル 1.24×10⁷ / 0.95×10⁷、所要 ≈ 105 分)。
   ⟹ **`q ≤ 31` の全奇素数で hypothesis (B) は不可能**。
...
## 状態 (2026-08-11 更新) — **q ≤ 31 決着**、判定法は Lean 化済
...
| **31** | ✅ **トレース判定** (両指数とも衝突 #1、2026-08-11) |
...
(`q = 37, 43` は exotic 指数 0 で定理 1 が片付ける)。探索コスト `≈ √(Q/q) = 2.3×10⁹` は
GAP では非現実的なので、以降は (B1) の一般証明か C 実装 + rho 法が要る。
