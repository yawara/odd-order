...
### (170) 段 (3) の格上げ道具 — `eq_of_mk_eq_of_mem_Q`

`π : U → U/Z(U)` は `Q ∩ U` 上単射 (`Z(U) ≤ D` + `Setup.Q_inf_D_eq_bot`)。
⟹ 段 (2) の商上の等式を `U` 内の等式へ格上げできる。

**⚠ 次の障害 (次セッションが解く)**: 段 (2) の `ζ` は**商の元**。書籍は `ζ ∈ C_W(P∩U)`
(`U` の元) として扱うので、`W(intrinsic) = π(W ∩ U)` が要る。
* 内在仮説の `W` は自身の `H`,`D` から `W = D ⊓ C(Q₀)` で定義される。
* `map_W_of_mulEquiv` は **MulEquiv 用**なので、射影 (全射準同型) には使えない。
...
### (173) 📖 p.134 = §4 の終盤 (Ch. IV の最後) — 構造を確定

ページ画像を読んだ。p.134 は 1 ページで §4 (と Ch. IV) が終わる:

* **(9)** `((ζ⁻¹ + a^{-2μ})^μ)/((1 + b²a^{-2μ})^μ) = (b² + ζ)/(1 + b²a^{-2μ})`
* `λ := (ζ⁻¹ + a^{-2μ²})/(b² + ζ) ∈ F` と置くと `λζ² + (λb² + a^{-2μ²})ζ + 1 = 0`。
  `ζ^{1+q} = 1` かつ `ζ ∉ F` から **`λ = 1`** および `b² + a^{-2μ²} = ζ + ζ⁻¹`。
  (9) の両辺の分母が等しくなり `b²a^{-2μ} = (ζ + ζ⁻¹ + a^{-2μ²})a^{-2μ}` は `μ` で不変。
* **(10)** `(ζ + ζ⁻¹ + X^μ)X = (ζ + ζ⁻¹ + X^{μ²})X^μ`  for `X ∈ F − {0, α^{2τ}}`
* `X ∈ F − {0, α^{2τ}, α^{2τ}+1}` に対し (10) を `X+1` でも書いて引くと **`X^{μ²} = X`**。
* ⟹ **`μ = 1`** (`μ` は奇位数で、`μ ≠ 1` なら `|F| ≥ 8`)。
  ⟹ **`η ∈ W`** かつ **`h(ω) ∈ W`** ⟹ §3 の Corollary 1 で終了。∎

**⟹ §4 の残り = (4)〜(8) の計算 (p.133 末〜134 冒頭) + 上記の終盤**。
`μ` は `η` (= `P` の生成元) が `Q/Q₀ = E` に半線形に作用するときの体自己同型
(Appendix I Prop 2)。

**着手しやすい独立部品** (次セッション候補):
> `μ ∈ RingAut F` が高々 3 点を除いて `μ² = id`、かつ `|F| ≥ 8`、かつ `Odd (orderOf μ)`
> ⟹ `μ = 1`
...
### (175) 📖 p.133 の (3)〜(8) を読了 + (10)→μ² の核が landing

**p.133 の構造** (段 (3) の Proposition の証明 + 計算 (3)〜(8)):

* 段 (3) の証明: PSU(3,ℓ) の構造から `(V∩U)/(P∩U)` が `C_{Q₀}(P)` を中心化 ⟹ Galois で
  `V∩U ⊂ PW`、`U ⊂ C_G(P)` ゆえ `V∩U ⊂ P × C_W(P)`。
  `|(V∩U)/(P∩U)| = (ℓ+1)/(ℓ+1,3) ≠ 1` (ℓ > 2)。`ζ₁ ∈ (V∩U) − (P∩U)`、
  `ζ ∈ C_W(P)` を `ζ₁ ∈ ζP` に取る。⟹ 我々の `exists_f_eq_conj_inv_residual` は
  この `ζ₁` 版で、書籍の `ζ ∈ C_W(P)` への精密化がこの段落 ((171) の裁定どおり)。
* `η` は `Q/Q₀ ≅ E` に半線形に作用し、`μ` はその体自己同型 (Appendix I Prop 2)。
* **(3)** `f(ωs^a)‾ = ζa⁻²·f(ωs^b)‾` (§2 の (2) から)
* **(4)** `a²·f(ωs^a)‾ = ζ⁻¹·(f(ωs^a)‾)^η + ω̄`
* **(5)** `ζ·f(ωs^b)‾ = a^{-2μ}·(f(ωs^b)‾)^η + ω̄` ((4) に (3) を代入)
* **(6)** `b²·f(ωs^b)‾ = ζ⁻¹·(f(ωs^b)‾)^η + ω̄` ((4) を `b` で)
* **(7)(8)** (5)(6) の線形結合で `f(ωs^b)‾` と `(f(ωs^b)‾)^η` を明示

**landing** (commit 済): `sq_apply_eq_of_relation` — (10) を `X` と `X+1` で書いて引く
と `μ²X = X`。⚠ 実際に計算すると標数の仮定は不要だった (可換環一般)。
...
### (214) ⚠ `ζ₁ ∈ V` は自動でない — step (3) に持ち上げ関係を露出

実測: `V = C_D(t)` なので `ζ₁ ∈ V` には `ζ₁` が `t` と可換であることが要る。
`ζ₁, t ∈ U` だけでは (`U` は非可換なので) 出ない。

書籍は `ζ₁ ∈ (V ∩ U) − (P ∩ U)` と**選ぶ**ので、正しい筋は
「prescribed な `ζ₁` の像として `ζ̄` を取り、step (3) が返す `z` が
`ζ̄` を持ち上げること (`z ∈ ζ₁·Z(U)`) から `z ∈ V`, `z ∉ P` を継承する」。
`Z(U) ⊆ P ≤ V` なので両方通る。

⟹ **step (3) の結論に `mk' z = ζ` を追加**した
(`exists_f_eq_conj_inv_residual`; 証明内では既に `hzζ` として在ったので露出のみ)。
