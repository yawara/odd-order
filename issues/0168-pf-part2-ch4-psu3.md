...
2. **§3 (3)**: 体側は ✅ 完了 (2026-08-01):
   ...
   * `sq_eq_of_one_div_eq` (2026-08-01) — `1/(a²+ζ⁻¹) = ζa⁻²/(b²+ζ⁻¹)` から
     `b² = ζ + ζ⁻¹ + a⁻²` (襷掛け)。
   * 🎯 `star_of_sq_eq` (2026-08-01) — `Y = (ζ+ζ⁻¹) + X` と `Y·Y^θ = α² + X·X^θ`
     から **(∗)**。入力 `θ(ζ+ζ⁻¹) = ζ+ζ⁻¹` が書籍の「`ζ ∈ W` ゆえ
     `(ζ+ζ⁻¹)^θ = ζ^σ + ζ^{-σ} = ζ+ζ⁻¹`」。

   🔶 **残り = 群↔体の橋渡し 1 本だけ**: `stepTwo_linear` を `a` と `b`
   (`b^{1+θ} = α + a^{-(1+θ)}`) の 2 通りで使って
   **`1/(a²+ζ⁻¹) = ζa⁻²/(b²+ζ⁻¹)`** を出す。書籍はここで §2 の段 (2) を座標で
   読み `f(ωs^a)‾ = ζa⁻²·f(ω(0,α+a^{-(1+θ)}))‾` を使う。
   ⚠ `a⁻²` が `X`、`α^{2τ}` が `z`、`b^{1+θ}` が `s^b` の座標に対応。
   これが出れば `sq_eq_of_one_div_eq` → `star_of_sq_eq` →
   `eq_one_and_eq_add_inv_of_star` で §3 (3) は閉じる。

   ⚠⚠ **スカラー規約に注意 (2026-08-01 に気づいた罠)**。`stepTwo_linear` は
   `(μ(a²,1) + μ(1,ζ))·X̄_a = ω̄` の形で、書籍の `(a² + ζ⁻¹)` の `ζ⁻¹` が
   こちらでは `μ(1,ζ)` になっている (書籍の `x^d = d⁻¹xd` と `coord_act` の
   向きの差)。したがって橋渡しの関係式 `X̄_a = ζa⁻²·X̄_b` の `ζ` が
   `μ(1,ζ)` なのか `μ(1,ζ)⁻¹` なのかは**必ず §2 段 (2) の座標読み下しから
   導出して確かめること** — 素朴に当てると `b² = ζ+ζ⁻¹+a⁻²` でなく
   `B = u²A⁻¹` という別物が出る (実際に机上で食い違いを確認した)。
   ⟹ 次セッションはまず **§2 段 (2) (`f_mul_conj_distinguishedInvolution`) を
   `coord` で読む補題**を作り、そこから関係式を導くこと。
3. §3 (4)。
