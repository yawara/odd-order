...
1. **§3 (1) の本体**。材料は揃った (`f_inv_eq` / `g_inv_eq` / `h_inv_eq` +
   §2 の段 (2)(3) = `f_mul_conj_distinguishedInvolution` /
   `f_conj_distinguishedInvolution_mul`)。書籍の連鎖:

       f(ω⁻¹s^{a⁻¹}) = f(f(ω⁻¹)s^a)^{a²}s^a        ← §2 段 (2) を `ω := ω⁻¹`, `a := a⁻¹` で
                     = f(s^{a⁻¹}g(ω⁻¹))^{h(ω⁻¹)^t}f(ω⁻¹)   ← §2 段 (3) を `ω := ω⁻¹` で

   ⚠ 注意点: repo の段 (3) は書籍と積の順序が逆 (docstring 参照)。`s^a ∈ Q₀` が
   `Q` の中心なので一致するが、`rw` の整合には `W_centralizes_Q0` 等で明示的に
   入れ替える必要がある。`h(ω⁻¹)^t = ζ⁻³` は `conj_t_pow_eq` で。
