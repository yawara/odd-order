...
* 🎯 **前半 `h(ω) ∈ W` は完了** (`h_mem_W`, PSU3StepEighteen.lean)。
  ...
  互いに素性で直接出る。
* 🔶 **後半** (`f(ω_i) = (ω_i⁻¹)^ζ`、すなわち `ω_i² = (0,α)`) — **末端 2 段は完了**
  (`sq_eq_of_dOrbitRel` / `f_eq_conj_inv_of_sq_eq`, PSU3StepTwenty.lean, 2026-08-01):
  (H5) の帰結 `(ω⁻¹(0,r))^{KW} = (ω(0,α+r))^{KW}` から `D` の自由性で
  `ω² = (0,α)` が出て、それが正規化と合わさって `f(ω) = (ω⁻¹)^ζ`。
  **残るのは (H5) に食わせる 3 本の軌道等式の連鎖と、軌道代表系の相異性から
  `i = k` を出す部分**。
...
1. §2 締めの Proposition の後半 (`f(ω_i) = (ω_i⁻¹)^ζ`) — (H5) を使う。
2. §3 の Proposition (pp.129-134)。p.130 のページ画像は保存済、p.131-134 は未取得
