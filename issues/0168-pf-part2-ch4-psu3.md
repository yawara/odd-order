...
**(8) の残り (これだけ)**:
1. `V = W` を明示仮説として導入。`K ∩ W = 1` と `|D| = |V|·|K|` と合わせて
   **`D = KW`** が出る (`|KW| = |K||W| = |K||V| = |D|` かつ `KW ⊆ D`)。
2. `D = KW` により「`KW`-軌道に入る」= 「`D`-共役に合同」= (上の同値で)
   既存補題が消費する形。⟸ **ここだけがモデル依存**
   (`QuotientFieldModel.coord`/`coord_act` で `Q/Q₀ ≅ E` と `KW` 作用を取る)。
3. `Σ_i m_i = |Q₀| = q` (`f` が `Q−Q₀` を保つので `f(ω₁x)‾ ∈ (Q/Q₀)^#`、
   軌道はちょうど 1 つ)。
4. `n = (q+1)/m` と 2 評価で `q = Σ m_i ≤ nm − 1 = q` の挟み込み → 全等号。
