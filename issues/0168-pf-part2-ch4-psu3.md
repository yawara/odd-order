...
**(8) の残り = モデル依存部分のみ**:
1. `V = W` を明示仮説として導入 (`|V| = |W| = m` にする)。
2. 「`f(ω₁x)‾` が `ω̄_i` の `KW`-軌道に入る」を `∃ y ∈ Q₀, ∃ a ∈ D,
   f(ω₁x) = (ω_i y)^a` に翻訳する補題 (`QuotientFieldModel.coord` /
   `coord_act` を使う)。
3. `Σ_i m_i = |Q₀| = q` (すべての `x ∈ Q₀` でちょうど 1 つの軌道; `f` が
   `Q−Q₀` を保つので `f(ω₁x)‾ ∈ (Q/Q₀)^#`)。
4. `n = (q+1)/m` と上の 2 評価で `q = Σ m_i ≤ nm − 1 = q` の挟み込み → 全等号。
