...
1. **§3 (2)**: 段 (1) を `coord` で読む。`ζ⁻¹a²·f(ωs^a)‾ = ζ⁻²·f(ωs^a)‾ + ζ⁻¹ω̄`
   から `(a² + ζ⁻¹)f(ωs^a)‾ = ω̄`。
   * ✅ **配管は完成**: `coord_conj_eq` (PSU3OrbitCount.lean, 2026-08-01) —
     `c = κv` による共役は `Q ⧸ Z(Q) ≅ E` の座標をスカラー `μ(κ,v)` 倍する
     (`conjQHom_kActor_apply_val` → `quotientKWHom_mk` → `coord_act`)。
     `centerCoord_conj` (`Q₀` 側) の `Q/Q₀` 側での対応物。
   * **導出は完全に書き下せた (2026-08-01)。次セッションはこれを実装するだけ**:

     ✅ **(a) は完了** (`f_conj_zeta_of_mem_Q0`, 2026-08-01)。
     `X := f(ω · (a s a⁻¹))` と置く。`stepOne_chain` の 2 つの `f` は `X` の共役:

     - `f((ζωζ⁻¹)z) = ζ X ζ⁻¹`,  `f(z(ζ⁻¹ωζ)) = ζ⁻¹ X ζ`  (`z = a s a⁻¹ ∈ Q₀`)
       — これが `f_conj_zeta_of_mem_Q0`。

     `coord_conj_eq` (`coord(mk(c x c⁻¹)) = μ(κ,v)·coord(mk x)`, `c = κv`) と
     `coord` の加法性で `stepOne_chain` の両辺を読むと (`s^a ∈ Q₀ = Z(Q)` は
     `mk` で消える; 右辺の `ζωζ⁻¹` は `Q₀` に無いので**積の第 2 因子として加法的に
     残る**):

         μ(a²,1)·μ(1,ζ)·x = μ(1,ζ³)·μ(1,ζ⁻¹)·x + μ(1,ζ)·ω̄

     `μ` は準同型なので `μ(1,ζ³)μ(1,ζ⁻¹) = μ(1,ζ²)`。両辺を `μ(1,ζ)` (単元) で
     割って標数 2 でまとめると

         **(μ(a²,1) + μ(1,ζ)) · x = ω̄**

     ⚠ 書籍の `(a² + ζ⁻¹)` の `ζ⁻¹` はこちらでは `μ(1,ζ)` になる (`μ` の向きと
     書籍の `x^d = d⁻¹xd` の差)。自己完結しているので問題ない。
   * ✅ **道具は全部揃った (2026-08-01)**: `coord_conj_eq` (共役 → スカラー倍) /
     `coord_mk_mul` (積 → 和) / `coord_mk_eq_zero_of_mem_Q0` (`Q₀` は消える)
     — すべて `PSU3OrbitCount.lean`。
   * 🔶 **残り = 読み下しの組み立て 1 本**。`z := a s a⁻¹ ∈ Q₀`,
     `X := f(ω z)`, `x := coord(mk X)`, `ω̄ := coord(mk ω)` と置き、
     `stepOne_chain` を `f_conj_zeta_of_mem_Q0` で `X` の共役に書き換えてから:

     - 左辺 `(a²·(ζXζ⁻¹)·(a⁻¹)²)·z`:
       `coord_mk_mul` で `z` を落とし (`coord_mk_eq_zero_of_mem_Q0`)、
       `coord_conj_eq` を `κ := a², v := 1` と `κ := 1, v := ζ` で 2 回。
       ⟹ `μ(a²,1)·μ(1,ζ)·x`
     - 右辺 `(ζ³·(ζ⁻¹Xζ)·(ζ³)⁻¹)·(ζωζ⁻¹)`:
       `coord_mk_mul` で 2 項に割り、`coord_conj_eq` を
       `κ := 1, v := ζ³` / `κ := 1, v := ζ⁻¹` / `κ := 1, v := ζ` で 3 回。
       ⟹ `μ(1,ζ³)·μ(1,ζ⁻¹)·x + μ(1,ζ)·ω̄`

     ⚠ `W` 成分は subtype で扱う: `V := (⟨ζ,hζ⟩ : ↥hyp.W)` と置いて
     `μ(1,V³)·μ(1,V⁻¹) = μ(1,V²) = μ(1,V)²` (`map_mul` / `map_pow`)。
     `⟨ζ³, _⟩ = V³` は `Subtype.ext`。
     最後に単元 `μ(1,V)` で割り標数 2 でまとめて **`(μ(a²,1)+μ(1,V))·x = ω̄`**。
2. §3 (3) の残り、(4)。
