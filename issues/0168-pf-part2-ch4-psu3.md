...
* `hOne` — **(H1)** `f(x⁻¹) = g(x)⁻¹`。定義式を反転すると
  `t x⁻¹ t = f(x)⁻¹ · t · h(x)⁻¹ g(x)⁻¹` で、まだ標準形 `a · t · b` (`b ∈ Q`) では
  ない。⚠ **要点は `h(x)⁻¹` を `t` の向こうへ押し出すこと** — `D = M ∩ M^t` なので
  `t h(x)⁻¹ t ∈ M` が使え、標準形になって `Q` 成分が `g(x)⁻¹`。一意性で
  `f(x⁻¹) = g(x)⁻¹`。追加で要った仮説は `t² = 1` と `∀ d ∈ D, t d t ∈ M` の 2 つ。

* `hThree` — **(H3)** `f(x^a) = f(x)^{a^t}` (`a ∈ D`)。定義式を `t` で共役すると
  `t x^a t = (a^t)⁻¹ (t x t) a^t` となり、`t·t = 1` の両側に現れる `a`/`a⁻¹` が
  相殺して標準形になる。`Q` 成分が `f(x)^{a^t}`。
  追加仮説: `D` が `t` で安定 (`t d t ∈ D`) と `D` が `Q` を正規化。
  ⚠ **指数が `a` でなく `a^t` であることは、この計算で先に分かり、ページ画像で確認した**。

**次**: (H2), (H4)-(H6)。(H2)(H4) は「straightforward calculations」、
