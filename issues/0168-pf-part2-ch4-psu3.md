...
**(6)** `f(ωx) = (f(ω)y)^a` (`ω ∈ Q−Q₀`, `x,y ∈ Q₀`, `x ≠ 1`, `a ∈ D`) ⟹ **`a ∉ K`**。
書籍 (p.124): `x = s^k` (`k ∈ K`) とおく。(2) より
`(f(ω)y)^a = f(f(ω)s^{k⁻¹})^{k⁻²} s^{k⁻¹}`、よって
`f(f(ω)s^{k⁻¹}) = ((f(ω)y)^a s^{k⁻¹})^{k²} = (f(ω)s^{k⁻¹}·s^{k⁻¹}y s^{k⁻¹a⁻¹})^{ak²}`。
(5) より `ak² ∉ K`、ゆえに `a ∉ K`。

⚠ **要検討**: 最後の「`ak² ∉ K` ⟹ `a ∉ K`」は `a ∈ K ∧ k ∈ K ⟹ ak² ∈ K` を要する。
`K = {x ∈ D | x^t = x⁻¹}` で `(ak²)^t = a⁻¹k⁻²` vs `(ak²)⁻¹ = k⁻²a⁻¹` なので
**`a⁻¹` と `k⁻²` の可換性**が要る。repo は `hyp.K` を部分群として持つ文脈があるので
(`OrderThreePSL.lean` の `K_le_normalizer_Q0` 等)、`K` が可換群である補題を先に探す。
これは着手前の必須調査項目。

**(7)** `ω, ω' ∈ Q−Q₀`, `x_i, y_i ∈ Q₀`, `a_i ∈ D` (i=1,2) で `x₁ ≠ x₂` かつ
`f(ωx_i) = (ω'y_i)^{a_i}` なら `a₂ ∉ a₁K`。証明は p.124 の続き (未読)。
