...
**(8) の残り = 数値部分のみ**:
1. `Σ_i m_i = |Q₀| = q`。`f` は `Q−Q₀` を保つ (`mem_Q0_of_f_mem_Q0` の対偶) ので
   すべての `x ∈ Q₀` で `f(ω₁x)‾ ∈ (Q/Q₀)^#`、属する軌道はちょうど 1 つ。
2. `n` = `(Q/Q₀)^#` 上の `D`-軌道の個数、`m = |W|` として `n·m = q+1`。
   ...
   割る形。⚠ `μ` の単射性 / `|μ(KW)|` の確定が要るはず — 0167 の
   `exists_actualKActor_mu_eq` (`K₁ = F^×`, `|F^×| = q−1`) が使えるか要確認。
