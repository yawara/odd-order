...
| **Lem 4.4** ✅ (class 2 + exponent + Φ ⊆ Z) | ✅ (2026-05-24) | `pow_mem_center_of_class_le_two_of_commutator_pow` (一般化 statement: 任意 `n`, `commutator G ≤ Z(G)` + 全交換子の `n`乗 = 1 ⇒ `x^n ∈ Z(G)`). + `isElementaryAbelian_quotient_center_of_class_le_two` ("In particular" `p`-elem abelian 帰結). + **`frattini_le_center_of_class_le_two_of_commutator_pow_eq_one`** ("and thus Φ(P) ⊆ Z(P)" 結論, Lem 4.5 forward 経由 2-step). class ≤ 2 で `⁅·, z⁆` 左 hom + `⁅x^n, z⁆ = ⁅x, z⁆^n` 経由. ~95 LOC. |
| **Lem 4.5** ✅ (双方向 P/N elem abelian ⇔ Φ(P) ⊆ N) | ✅ (2026-05-24) | **backward** `isElementaryAbelian_quotient_of_frattini_le_of_pgroup` (~170 LOC via `index_eq_prime_of_isCoatom_of_pgroup`, `commutator_le_frattini_of_pgroup`, `pow_p_mem_frattini_of_pgroup` helpers). **forward** `frattini_le_of_isElementaryAbelian_quotient_of_pgroup` (~110 LOC) via Module (ZMod p) on `Additive (P/N)` → `Projective.exists_dual_ne_zero` (vector space dual) → MonoidHom via `AddMonoidHom.toMultiplicativeRight` → range = ⊤ (`IsSimpleOrder.eq_bot_or_eq_top` on ZMod p) → ker has index p → maximal (`isCoatom_of_index_prime` helper) → Φ(P) ⊆ M ⊇ N. **iff** form `frattini_le_iff_isElementaryAbelian_quotient_of_pgroup`. 新規 imports: `Mathlib.Algebra.Field.ZMod`, `Mathlib.Algebra.Module.ZMod`, `Mathlib.LinearAlgebra.Dual.Lemmas`. 下流引用: Lem 4.4 "thus Φ(P) ⊆ Z(P)" 結論のみ; Ch.5+ では direct cite 無し. |
| **Lem 4.6** ⭐ (G' = ⁅A, ⊤⁆) | ✅ (2026-05-23) | `commutator_eq_commutator_of_normal_abelian_cyclic_quotient`. mathlib `commutative_of_cyclic_center_quotient` 経由 5-step proof: ⁅A,⊤⁆ ≤ A, lift Q→G/A, ker ⊆ Z(Q) (∵ ⁅a,g⁆∈⁅A,⊤⁆), Q abelian, commutator G ⊆ ⁅A,⊤⁆. |
| **Lem 4.6 後半** ⭐ (G' ≅ A/(A∩Z(G)) + cardinality) | ✅ (2026-05-23) | `commutatorRightHom hAb g : A →* G` (θ a := ⁅a, g⁆), `commutatorRightHom_range_eq_commutator` (= commutator G via Normal + closure induction over A ⊔ ⟨g⟩ = ⊤), `commutatorRightHom_ker_eq` (= (A ⊓ Z(G)).subgroupOf A), `card_commutator_mul_card_inf_center_eq_card_of_normal_abelian_cyclic_quotient` (\|G'\| · \|A ⊓ Z(G)\| = \|A\|). Lagrange + index_ker 経由. ~240 LOC (hom 40 + range_normal 70 + range_eq 50 + ker + cardinality 80). Thm 4.7 への直接前提. |
| **Thm 4.7 part 1** (m=1 base + lcs map helper) | ✅ (2026-05-23) | `lowerCentralSeries_map_eq_of_surjective`: 全射 hom `f : G →* H` ⇒ `(lcs G n).map f = lcs H n`. `nilpotencyClass_eq_one_of_normal_abelian_cyclic_quotient_inf_center_prime_card_p`: m = 1 base case (`|A| = |A ⊓ Z(P)| = p` ⇒ A ⊆ Z(P) ⇒ P abelian ⇒ class = 1, via `commutative_of_cyclic_center_quotient`). ~80 LOC. |
| **Thm 4.7** ⭐ (full statement, m ≥ 2 induction) | ✅ (2026-05-23) | `nilpotencyClass_eq_of_normal_abelian_cyclic_quotient_inf_center_prime_card_p_pow`: `(m : ℕ)` 明示 + `induction m generalizing P`. base case m=0 不可能 (`A` trivial vs `\|A ⊓ Z(P)\| = p`), m=1 ↑ reduce, m ≥ 2 帰納. 補助 `card_map_mk_mul_card_inf_eq_card`: `\|H.map mk'\| · \|H ⊓ N\| = \|H\|` (1st iso + Lagrange). 流れ: Lem 4.6 cardinality で `\|G'\| = p^k` + Thm 1.19 (Ch.1) で `G' ⊓ Z(P) > ⊥` + `Subgroup.card_dvd_of_le` で `\|G' ⊓ Z(P)\| = p` ⇒ `Z ≤ G'`. 商 `P̄ = P/Z`, `Ā = A.map mk'` で IH 適用 ⇒ `class P̄ = m-1`. lift back via `lowerCentralSeries_map_eq_of_surjective` + `lowerCentralSeries_succ_eq_bot`. ~190 LOC. **§4A 完全完成**. Ch.10 で 2 引用. ⚠️ commit `2a63f75` に Ch.5 cyclic_pgroup と同梱 (commit message は Ch.5 のみ言及). |
...
§4A **literally 完全完成** (2026-05-24): Lem 4.1, 4.3, **4.4 (双方向 + Φ⊆Z 結論)**, **4.5 双方向**, **4.6 前半+後半**, **Thm 4.7 (m=0/m=1/m≥2 全 case)**, **Thm 4.8(a)(b) + collection formula**. 残り無し.
