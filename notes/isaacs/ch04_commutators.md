...
| **Lem 4.5 backward** (Φ(P) ⊆ N ⇒ P/N elem abelian) | ✅ (2026-05-23) | `isElementaryAbelian_quotient_of_frattini_le_of_pgroup`. helper 群: `index_eq_prime_of_isCoatom_of_pgroup` (p-群極大 ⇒ index p; NormalizerCondition + Cauchy で k=1 を示す), `commutator_le_frattini_of_pgroup` (IsCyclic of prime card 経由), `pow_p_mem_frattini_of_pgroup` (`pow_card_eq_one'`). ~170 LOC. forward direction (P/N elem abelian ⇒ Φ ⊆ N) は構造定理経由で別途. **唯一の引用箇所**: Lem 4.4 末尾 "thus Φ(P) ⊆ Z(P)" の結論 (Isaacs L2202 "By Lemma 4.5, the proof is now complete"). Lem 4.4 の完全形式化 (Φ ⊆ Z 結論部分) を望むなら必要, 他下流 (Ch.5+, BG, Peterfalvi) では cite 無し. |
...
§4A **完全完成** (2026-05-23): Lem 4.1, 4.3, 4.4, **4.5 backward**, **4.6 前半+後半**, **Thm 4.7 (m=0 不可能 + m=1 base + m≥2 induction)**, **Thm 4.8(a)(b) + collection formula**. 残: Lem 4.5 forward (構造定理経由, Lem 4.4 "thus Φ(P) ⊆ Z(P)" 結論の完全形式化を望む場合のみ必要, 下流章では cite 無し, 保留).
