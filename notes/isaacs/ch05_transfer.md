...
**mathlib カバレッジは Ch.5 中で最も厚い**. `Mathlib/GroupTheory/Transfer.lean` (350 行) + `Focal.lean` (218 行) + `Schreier.lean` + `SpecificGroups/ZGroup.lean` で 主要結果 (transfer 定義・Burnside 5.13・cyclic Sylow 5.14・Focal Subgroup 5.21・Schur 5.7・Z-group 構造 5.15-5.16) を直接実装済. ⇒ ラッパー仕事中心、新規実装は **Thm 5.18 (abelian Sylow 強化 Burnside)・Thm 5.24 (nilpotent maximal)・Thm 5.25 (control own fusion)・Thm 5.26 Frobenius** などに限られる.
...
| 5.17 cyclic Sylow ⇒ p ∤ \|G'\|·\|G:G'\| | ✅ `isaacs_thm_5_17` (2026-05-23, ~175 LOC). **Forward dep on Ch.4 §4D Thm 4.34 (Fitting)** (axiom 化) + cyclic-pgroup-chain (axiom). Schur-Zassenhaus + Fitting + cyclic chain で C_P(K) = ⊥ vs ⁅P,K⁆ = ⊥ 場合分け, Burnside (mathlib `ker_transferSylow_isComplement'`) for case 1, P ⊆ G' for case 2 | ✅ |
...
| 5.25 controls own fusion ⇔ normal p-comp | 🟡 部分完成 (2026-05-24). `Sylow.ControlsOwnFusion` def + (⇒) `controlsOwnFusion_of_hasNormalPComplement` (~50 LOC, sorry-free) via `IsComplement'.sup_eq_top` + `mem_sup_of_normal_left` + 直接 z = q x q⁻¹ 計算 (`IsComplement'.QuotientMulEquiv` 不要). (⇐) `hasNormalPComplement_of_controlsOwnFusion` sorry (mathlib に O^p, A^p 未収載で新規定義 ~50 LOC + 本体 ~150 LOC). | 🟡 部分 |
...
| 5.28 (Lem, key Sylow 共役) | 🟡 部分完成 (2026-05-24, Steps 1-10 preview, sorry-free). Statement + 強帰納法 skeleton + Case 1 (P = Q) + Case 2 Steps 1-8 + Step 10 preview 全て sorry-free 実装. Helper 2 件 (`sylow_sup_normal_eq_top_of_quot_isPGroup`, `lt_normalizer_of_pgroup_of_lt_top`). 実装済 Steps: 1 (P ⊓ N > D), 2 (Q ⊓ N > D), 3 (S, T Sylow), 4 (R Sylow), 5 (S ⊔ C = ⊤), 6 (Sylow II), 7 (n = yC * sS 分解), 8 (T = yC • S), 10 prev (P ⊓ R > D). 残り Step 9 (Q ⊓ N ≤ yR via T → G 翻訳, ~50 LOC), Step 10/11 (二回 IH + 結合 c = x · yC⁻¹ · z, ~100 LOC). | 🟡 部分 |
...
8. **§5E (5.25, 5.26, 5.27, 5.28, 5.29, 5.30)**: **FT クリティカル**. 5.26 Frobenius normal p-complement は mathlib 未収載で新規実装の山. 5.30 (p odd) は奇数位数仮定と相性が良いため FT 経路で重要可能性.

優先度 (FT クリティカル度): **5.21 (Focal) ≫ 5.13 (Burnside) ≫ 5.26 (Frobenius), 5.30** > 5.6, 5.7 (mathlib カバー厚) > その他.
