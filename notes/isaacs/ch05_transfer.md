...
| 5.27 (Lem, easy 1⇒2⇒3) | ✅ Part 1 (1⇒2 strong): `hasNormalPComplement_of_subgroup` (2026-05-24, ~70 LOC). `def HasNormalPComplement p G` 導入 (mathlib 未収載). Sylow `card_eq_multiplicity` + `not_dvd_index` + `relIndex_dvd_index_of_normal` + `factorization_mul` で `\|N \cap H\| * \|Q\| = \|H\|` を確立し `isComplement'_of_coprime` 適用. Part 2 (2⇒3) は保留. | ✅ Part 1 |
