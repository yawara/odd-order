...
⟹ **欠けているのは「(b)(c) ⟹ `CharacterPsiDecomposition τ χ 0` が存在する」の 1 本だけ**で、
それは書籍 pp.27-28 の (5.6.1)-(5.6.3) の議論そのもの (補助等長 `n` の構成 + `Y` の係数評価)。
下流はすべて構成済なので、この 1 本を書けば書籍仮説そのままの (5.6) が出る。**~2 ページ分の
実作業**であり packaging 差ではない (= 低優先繰延の理由は「小さいから」ではない)。
