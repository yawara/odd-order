...
| (5.4) | ⬜ 要突合 (statement は在る) | `S07_Coherence/NormInequalities:28` が (5.4) の仮説を束ね、(5.4.a) `‖X‖² ≥ ‖χ‖²` と (5.4.b) の等号・`X = ∑_{α∈E} α` を docstring に明記。書籍 p.26 と未突合 |
| (5.5) | ⬜ 要突合 (statement は在る) | `S12_TypeIIColumnPin:24,51` (可約 `T2`-列に対する形) / §9 level は `AxiomsCheck:12672`。書籍 p.27 と未突合 |
| (5.6) | ⚠ **要注意・未突合** | sub-part は在る ((5.6.1) 構成 / (5.6.2) 開始評価 `‖Y‖² ≤ ‖ψ‖²` `NormInequalities:371` / (5.6.3) 分解対 `:145`)。⚠ **§8 の standalone 定理が「(5.6) break-member oracle」`h56` を仮説パラメータとして取る** (`S08_Theorem62_63_Standalone:350,388`)。これが「per-application の正当な入力」なのか「(5.6) 本体が posited」なのかを**producer まで追って確認する**こと。書籍 pp.27-28 と未突合 |
