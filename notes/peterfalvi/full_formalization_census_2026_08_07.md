...
> **これは live な scope 文書**。[issue 0172](../../issues/closed/0172-peterfalvi-full-formalization.md) の
...
| (1.7)(b) | ✅ **2026-08-08 に「未形式化」判定を撤回** (下記 §3.5 末尾) — 実体は `RepresentationTheory/InducedInvariantConstituent.lean`。以下は撤回前の記述: | 「`T/H` 可換 ⟹ `Ind_H^G θ = e ∑ χᵢ`, `n = \|T:H\|/e²`, `χᵢ(1) = \|G:T\|eθ(1)`」。repo が持つのは **coprime 版** (`induce_eq_sum_mul_linearClassFunction`, Gallagher) だけで、`GallagherDecomposition.lean:229` が「**general** Peterfalvi (1.7.b)」を未達目標と明記。必要な前提 = 可換 inertia 商への拡張定理 (`CyclicCharacterExtension` の巡回版は済、「合成列に沿って反復」が未実施 = 旧 issue 9002 の残り) |
...
| (3.8) | ✅ **2026-08-08 に個数条項を補充** (packaging 差は半減) | `S05_SigmaTrichotomy.sigmaCoeff_trichotomy` が三分岐を係数形で持つ。⚠ 書籍は結論に **`NC(ψ) = w₁` / `= w₂`** を含め、`ψ = a·Σω^σ + β` の形で述べる。repo は係数形 (「列 `j₀` が定数 `c ≠ 0`、他は 0」) で、NC 値は導けるが**述べられていない**。⟹ **2026-08-08 に packaging 差を完全解消**: (i) `NC` 条項 = `sigmaNC_eq_card_W1_of_column` / `sigmaNC_eq_card_W2_of_row`、(ii) 再構成条項 = `exists_sigmaBeta` (**Hypothesis (3.6) の分解そのもの**を定理化 — `{ω^σ}` が正規直交ゆえ `β := ψ − ∑ a_{ij} χ_{ij}` は自動で `Im σ` と直交) + `exists_sigmaBeta_column` / `exists_sigmaBeta_row`、(iii) 逐語版 `sigmaCoeff_trichotomy_book` (3 分岐それぞれに個数条項 + 再構成条項; case (a) の `ψ = β` は「全 Fourier 係数が消える」として表現)。係数形 `sigmaCoeff_trichotomy` は consumer 向け working version として存置 |
...
(3.7) の "In particular" 節の 2 件。繰延 1 件だった (3.8) の packaging 差は **2026-08-08 に完全解消** (個数条項 + 再構成条項 + 逐語版、上記)。
...
| (5.6) | ✅ **全体組み立てが在る (UNCONDITIONAL)** | ⚠ **前ターンの「全体定理が見当たらない」は誤り**。実体は `S07.retarget_isCoherent` (`S07_RetargetScaled:367`) = 「**MAIN coherence-union assembly (general (5.6), UNCONDITIONAL)**: `IsCoherent (S₁ ∪ {χ,χ̄}) A`」。拡張 `τ₂ := retarget τ₁ χ χ̄ X X̄` を**構成**し、`ℤ[S₁∪{χ,χ̄}]` 上の格子等長を証明、`extends_on_supported` を 3 本の差分生成元で discharge。**special-position 制限なし**。さらに仮説側も posited でなく構成される: `{X,X̄}` は `retargetTargetPair` が (5.5) 分解から**構成** (既約 `χ` では target pair は強制される)、`himg` は `image_eq_of_decomposition` が構成。(6.6)/(6.8) の `coherentPairChain` が呼ぶ単一エントリ (`IsCoherent τ S₁ A → IsCoherent τ (S₁∪{χ,χ̄}) A`) も在る。私が先に見つけた `isCoherent_union_pair_of_bridge` は §9 レベルの**別ルート** (bridge 経由) だった。✅ **2026-08-08 に narrow な問いも解消**: `DadeChainStep` が書籍の (b) を `famDegree_chi`、(c) を `hdeg_c` として持ち、唯一 posited に見えた `Dmem` も `decompositionPairFromDadeOfIrreducible` (coherence 仮説を取らない) で構成可能 ⟹ **元から書籍強度**だった |
...
✅ **narrow な残問だった (5.6) は 2026-08-08 に解消** (下記)。書籍仮説 (b) `χ₁(1) \| χ(1)` / (c) 次数不等式が repo では
「分解 `D₀`/`Da` の存在」に置き換わっている。

**2026-08-08 に経路を確定**。書籍 (5.6) (p.26 画像で確定) の仮説は
(a) `𝒮₁` coherent / (b) `χ₁(1) ∣ χ(1)` / (c) `2χ(1)χ₁(1) < ∑_{i=1}^n χᵢ(1)²/‖χᵢ‖²`。
repo の連鎖は

```
CharacterPsiDecomposition τ χ 0   (= (5.4) のデータ束: R(χ) / 補助等長 τ₁ / (χ−ψ)^{τ₁} = X − Y)
  → retargetTargetPair            (5.6.3) 直交対 {X, X̄} を構成
  → image_eq_of_decomposition     himg を構成
  → retarget_isCoherent_S         (5.6) 本体 (UNCONDITIONAL)
```

**さらに追跡した結果 (同日)、Dade 設定では (b)(c) を仮説に持つ入口が既に在る**:
`S07_Coherence/FamilyBundleDade.DadeChainStep` が

- **(b)** = `famDegree_chi : χ(1) = a·χ₁(1)` (`a : ℕ` なので整除性そのもの)
- **(c)** = `hdeg_c : 2·a < ∑ᵢ (aᵢ/‖χᵢ‖²)²·‖χᵢ‖²`

を**フィールドとして持ち**、`DadeChainStep.advance` が `IsCoherent τ (S₁ ∪ {χ, χ̄}) A'` を返す。
docstring によれば `R(χ)` と (2.6.b) `ZIrr` 帰属は**内部で構成**され、(5.1) 生成条件も
差分生成元経由で内部 discharge される。

**⟹ 残問も同日に解消 — (5.6) は書籍強度で完全被覆だった**。`Dmem` フィールドは
**構成可能**であって隠れた仮定ではない:

```
dadeOrthonormalCharacterImageFamily        R(χ) を構成 (χ 既約 + 非実 + χ/χ̄ が supported のみ)
decompositionPairFromDadeOfIrreducible     CharacterPsiDecomposition τ χ 0 を構成
   仮説 = 非実 / χ・χ̄・a·χ₁ が supported / χ₁ ∈ ℤ[Irr L] / 直交 3 本
   ⚠ **coherence 仮説を一切取らない**
```

つまり `Dmem` は各メンバーの局所データ (既約・非実・supported・直交) から作れる。
`DadeChainStep` がフィールドとして持つのは呼び出し側が渡す設計だからで、独立の仮定ではない。

⟹ **§5 の繰延項目はゼロ**。(5.6) は (a)(b)(c) を仮説とする書籍そのままの形が
`DadeChainStep` + `advance` として在り、その全フィールドが構成可能。

(当初「~2 ページ分の実作業」と見積もったが、実測すると新規に書くものは無かった。
このセッションで 5 回目の「無いと記録されていたが実は在った」— census の記録を鵜呑みにせず
毎回実測する運用が効いている。)
...
| (8.5) | ✅ **(a)(b)(c) 全条項を突合済 (2026-08-08、書籍 p.45 の画像で確定)** | (a) `F(M) = HC_U(H)` = `TypePData.fitting_eq` (carrier field の projection)。(b) は書籍どおり **2 文** = `S11.typeP_commutator_U_centralizes_H` (`⁅U,U⁆ ≤ C_G(H)`) + `S11.typeP_U_not_centralizes_H` (`U ≠ 1 → ¬ U ≤ C_G(H)`)、両方 AxiomsCheck 登録済。(c)「`V` は正規化群 `W` を持つ TI-subset」も 2 半分とも在る: TI = `S10.typePData_V_ti` (2026-08-08 に AxiomsCheck 登録)、正規化群 = `TypePData.normalizer_V` を `X = V` に適用 (非空性は `S12.typePData_typePV_nonempty`) — 書籍 (8.4)(e) の projection で (a) と同じ構図 |
...
✅ **§8 は全項目被覆** (2026-08-08 に (8.13)(c3) の Type I/II 両方が landing、[issue 0174](../../issues/closed/0174-peterfalvi-813-c3-support-membership.md))。
