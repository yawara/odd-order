...
⟹ **残る narrow な問いは 1 点に縮む**: `DadeChainStep` の `Dmem` フィールド
(「`S₁` の各メンバーの `ψ = 0` 分解族 ((5.5))」) が **(a) `S₁` coherent から導出可能か**、
それとも独立の仮定か。書籍では (5.5) を各 `χᵢ` に当てて得るものなので導出可能なはずだが、
repo では posited フィールド。**次の作業 = `Dmem` の producer を (a) から書けるか確認**。

(この追跡で「(b)(c) ⟹ `CharacterPsiDecomposition τ χ 0`」を新規に書く必要は無いと判明した —
Dade 設定の入口が既に (b)(c) を受けている。一般 `τ` 版が要るかは消費者次第。)
