...
| (5.6) | ⚠ **要確認 — 全体定理が見当たらない** | 書籍 p.26: 「(a) `𝒮₁` coherent、(b) `χ₁(1) \| χ(1)`、(c) `2χ(1)χ₁(1) < ∑ χ_i(1)²/‖χ_i‖²` ⟹ `𝒮₁ ∪ 𝒮₂` coherent」。repo に在るのは**各ステップ**: (5.6.1) 構成 / (5.6.2) 開始評価 `‖Y‖² ≤ ‖ψ‖²` (`NormInequalities:371`) / **(5.6.3)** union-pair 拡張 (`S07_UnionPairBridge.isCoherent_union_pair_of_bridge`、**bridge 恒等式を仮説に取る**)。⟹ **(a)(b)(c) から結論を出す全体定理**が見つからない。下流 (§15 の (9.11.7)-(9.11.8) 等) は (5.6.3) を直接使い bridge を手で供給している。**(1.3) と同じ「engine 止まり」型の可能性が高い** — 次イテレーションで結論の形 (`degree` 条件 + `IsCoherent (S₁ ∪ S₂)`) で再検索し、無ければ形式化する。⚠ 併せて (6.6) の degree-ratio 界 (`NormInequalities:986`「feeding (5.6)」) との関係も見る |
