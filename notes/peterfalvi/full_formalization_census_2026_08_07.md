...
| (3.8) | ✅ **2026-08-08 に個数条項を補充** (packaging 差は半減) | `S05_SigmaTrichotomy.sigmaCoeff_trichotomy` が三分岐を係数形で持つ。⚠ 書籍は結論に **`NC(ψ) = w₁` / `= w₂`** を含め、`ψ = a·Σω^σ + β` の形で述べる。repo は係数形 (「列 `j₀` が定数 `c ≠ 0`、他は 0」) で、NC 値は導けるが**述べられていない**。⟹ **2026-08-08: `NC` 条項を `sigmaNC_eq_card_W1_of_column` / `sigmaNC_eq_card_W2_of_row` として補充** (非零係数が列 `Ŵ₁ × {j₀}` / 行 `{i₀} × Ŵ₂` ちょうどであることから)。残る差は再構成条項 `ψ = a·∑ω^σ + β` (`β` の名指し) のみ |
...
(3.7) の "In particular" 節の 2 件。繰延 1 件だった (3.8) の packaging 差は **2026-08-08 に個数条項を補充して半減** (上記)。
