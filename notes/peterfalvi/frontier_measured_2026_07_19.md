...
| §6 | (6.6) | 部分 (**次の frontier**) | 教科書形: `1 ≠ Z ⊴ L`, `Z ⊆ Z(K)`, `X = S − S(Z) ⊆ Irr L` ⟹ (a) `X = {χ ∈ Irr L | Z ⊄ Ker χ}` かつ (b) `X` は coherent。**(a) は Z-generic 済** (`Xset_eq_irreducible_not_subset_characterKernel`, S08_DegreeSums/CoherenceGlue.lean:491)。**(b) が特殊化**: `Xset_centralCommutator_isCoherent_of_c2_caseA` (S08_CoherenceBasic.lean:1280) は `Z = hyp.centralCommutator` に固定。⚠ **2026-07-19 に signature を実測して訂正** — 旧記述の 「`Coprime H.index p`」は**存在しない**。実際の追加仮説は `cert : S06.CertainTypeHypothesis (sharpImage H) L` + `hK : cert.K = H` + `hW1 : cert.W1 = hyp.W1` + `hA : center ↥H ⊓ cert.W2.subgroupOf H = ⊥` + `hHnonab : commutator ↥H ≠ ⊥` + `hp : p.Prime` + `hp3 : 3 ≤ p` + `hHp : IsPGroup p ↥H`。着手時は「`centralCommutator` 固有の補題 (`centralCommutator_ne_bot`, `isIrreducibleCharacter_of_mem_Xset_c2_caseA`) が任意の `Z ⊆ Z(K)` に一般化するか」を先に判定する。 |
