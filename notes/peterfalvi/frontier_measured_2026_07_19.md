...
| §10 | (10.11) | **✅ 2026-07-21 完了 (issue 1048)** | 第 1 主張 (|W1|,|W2| prime) は一般 (`theorem88_caseB_prime_orders`)。第 2 主張 (型 II) = `S11.typeII_sSet_coherent`: 型 II の M で (9.2)/(9.5) の記法のまま「H は位数 p^q (p=|W₂|) の elementary abelian + 𝒮 coherent」。書籍の証明どおり `H₀ = 1` (`typeII_chiefFactor_H0_trivial`、\|W₂\|^q = \|H\| = p^q·\|H₀\| の一意分解) → H elementary abelian (N=⊥ で mk' 単射) → `C′ = 1` (`typeII_cprimeSub_eq_bot`、型 II の U abelian) → `𝒮(H₀C′) = 𝒮` (`sOf_bot_eq_sSet`) → (9.11) 型 II 版 (`typeII_nineEleven_coherent`, issue 1045)。副産物: `sOf_bot_eq_sSet` を S15 から ChiefFactorCore へ再層化。全 axiom-clean。 |
...
8. (8.15)/(8.18)/(9.7)/(9.11)/(10.11) は前提の作り直しを伴うので最後 (特に (8.15) は
   ...
   **§15 経由でなく (9.2)/(9.5) の記法で直接述べるのが書籍どおり**で、その最後の一歩が
   まさに本日 landed した (9.11) 型 II 版 ⟹ **issue 1048 で 2026-07-21 に完了** (`typeII_sSet_coherent`)。
   ⚠ §10 章の `pdftotext` は §9 と同じく文字バラけして使用不能 — 原文は PDF 画像で読むこと。
   **⟹ Pf 本文 (S01–S16) の型仮定/特殊化債務は実測上すべて解消。次は Isaacs 側 or 付録の残 sorry。**
