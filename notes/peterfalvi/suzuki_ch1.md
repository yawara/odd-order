...
9. **Section 3 Lemma 1, Sz(q) target in progress**.
   ...
   - Next upstream layer: take the subgroup generated by these permutations, prove its
     standard action is doubly transitive and the generated group is simple, then feed
     its concrete degree into Lemma 1. After Sz(q), construct the PSU(3,q) target of
     degree `q^3 + 1`.
