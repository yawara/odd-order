...
- **次 = per-step `XAdjoinStepInput` builder** (T8.11, 最重・残): `∀ i<N, IsCoherent τ (pairUnion S₀ pair i) A → XAdjoinStepInput ... (χs i)` を構成。残 field: (6.6)(b)/(c) からの divisibility・degree-gap 証明 (`a`/`hDeg` と family `hdvd`) + `hSgen`/`hgen`。`Dmem`/`crux1` はこれらの field から `xAdjoinStep` 内部で組み立つ。新 pair の `hreal`/`hdiffsupp`/row-ortho/prefix-ortho は T8.11b、accumulator member-family data は T8.11c、scaled support conversion は T8.11d/T8.11f/g、supported-diff ZIrr wiring は T8.11e、family ratio extraction は T8.11h で解決済み。
