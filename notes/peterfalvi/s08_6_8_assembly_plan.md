...
- ✅ **T8.11d (2026-06-04, codex)** `sMember_scaledDiffSupport_of_charValue_eq`: `S`-member の degree-ratio 等式 `χ(1)=aχ₁(1)` から `χ-aχ₁` の support ⊆ `H^#` を直接作る scaled support bridge を landed。degree ratios が入れば `hmemdegdiffsupp`/`hdiffasuppχ` はこの補題で放電でき、`htau1_memaχ` は supported 差 + `ZIrr` closure へ落ちる。
- **次 = per-step `XAdjoinStepInput` builder** (T8.11, 最重・残): `∀ i<N, IsCoherent τ (pairUnion S₀ pair i) A → XAdjoinStepInput ... (χs i)` を構成。残 field: degree-ratio data (`deg`/`a`/`i₁`/`hi₁`/`ha1`/`hDeg`) + supported-diff ZIrr wiring (`htau1_memaχ`) + `hSgen`/`hgen`。`Dmem`/`crux1` はこれらの field から `xAdjoinStep` 内部で組み立つ。新 pair の `hreal`/`hdiffsupp`/row-ortho/prefix-ortho は T8.11b、accumulator member-family data は T8.11c、scaled support conversion は T8.11d で解決済み。
