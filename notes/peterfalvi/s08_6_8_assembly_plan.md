...
- **次 = per-step `XAdjoinStepInput` builder** (T8.11, 最重・残): `∀ i<N, IsCoherent τ (pairUnion S₀ pair i) A → XAdjoinStepInput ... (χs i)` を構成。残 field: degree-ratio data (`deg`/`a`/`i₁`/`hi₁`/`ha1`/`hDeg`) + `hSgen`/`hgen`。`Dmem`/`crux1` はこれらの field から `xAdjoinStep` 内部で組み立つ。新 pair の `hreal`/`hdiffsupp`/row-ortho/prefix-ortho は T8.11b、accumulator member-family data は T8.11c、scaled support conversion は T8.11d、supported-diff ZIrr wiring は T8.11e で解決済み。
