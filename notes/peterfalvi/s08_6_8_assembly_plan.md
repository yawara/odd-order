...
- **次 = per-step `XAdjoinStepInput` builder** (T8.11, 最重・残): `∀ i<N, IsCoherent τ (pairUnion S₀ pair i) A → XAdjoinStepInput ... (χs i)` を構成。残 field: degree-ratio/support data (`deg`/`a`/`i₁`/`hi₁`/`ha1`/`hmemdegdiffsupp`/`hdiffasuppχ`/`htau1_memaχ`/`hDeg`) + `hSgen`/`hgen`。`Dmem`/`crux1` はこれらの field から `xAdjoinStep` 内部で組み立つ。新 pair の `hreal`/`hdiffsupp`/row-ortho/prefix-ortho は T8.11b、accumulator member-family data は T8.11c で解決済み。
