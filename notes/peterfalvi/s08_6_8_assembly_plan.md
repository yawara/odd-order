...
- **残 T6 (Y coherent の family 構成側)**: `coherentEqualDegree_fromDade hyp.dade hyp.hconj hn η hηinj hηdeg
  ...
  (3) **`inertia(θ)=H` (自由作用) = T6 の律速・深い blocker (未着手)**: (c1) Frobenius / (c2) から θ≠1 の
      ...
      **W₁(⟨g⟩)-共役 permutation を character table 上に構成して instantiate** すれば良いが substantial (新 infra)。
      ...
      `card_fixedPoints_conjByPermIrr_eq_card_fixedPoints_conjClassPerm`)。⟹ 残 = **Layer C**:
      fixed conjugacy classes が単位類のみであることから nontrivial Irr は固定されないと示し、
      `inertia(θ)=H` へ戻す。これが (6.8)/§9-§16 の Frobenius-induced irreducibility 全体の鍵。
  ...
  → **T6 の律速 = (3) general Brauer for conjugation Layer C**。Layer B は landed 済みなので、
    fixed class count = 1 から nontrivial Irr の非固定性を出せば 6.34 + (4)/(5) + engine
