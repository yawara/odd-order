...
- **次 = per-step `XAdjoinStepInput` builder** (T8.11, 最重・残): `∀ i<N, IsCoherent τ (pairUnion S₀ pair i) A → XAdjoinStepInput ... (χs i)` を構成。field: `hreal`/`hχχ`/ortho 系(T8.1 系から)+ `hdiffsupp`(T8.2)+ **member family enum** of accumulator + **Dmem/crux1 payload** via `decompositionDaFromDadeOfDiff`/`crux1_of_memberFamily` + **degree gap**(5.6 次数不等式) + `hSgen`/`hgen` + `hχ_S1`/`hχbar_S1`(=T8.10 disjoint から)。
