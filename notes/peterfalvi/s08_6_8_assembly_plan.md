...
| **T4** | **(1.9)Galois作用+(5.9.a)coherence不変** (cyclotomic field-of-values) — case B 専用。**✅ (1.9) 完了 (2026-06-02 R1, `CyclotomicGaloisAction.lean` 全 axiom-clean)**: `exists_complexRingEquiv_mapRingEquiv_eq_pow` (=(1.9.b), \|G\|=ab で一様 σ: ord g∣a ⇒ (σ∘φ)(g)=φ(g^k), ord g⊥a ⇒ =φ(g), ∀φ∈ℤ[Irr G]) + `exists_complexRingEquiv_pow_and_fixed` (=(1.9.a) CRT 形) + `exists_complexRingEquiv_extends` (部分体自己同型の ℂ 延長; 超越基底+MvPolynomial 係数 twist+`IsAlgClosure.equivOfEquiv`) + `map_trace_of_pow_eq_one` (有限位数自己準同型の trace 公式)。前提 infra: `IsIrreducibleCharacter.mapRingEquiv` (χ^σ∈Irr 無条件)/`galoisPerm`/`mapRingEquiv_mem_ZIrr_iff` (GaloisCharacter.lean)。**残 = (5.9.a) τ₁-u 可換** (χ^{τ₁u}=χ^{uτ₁}): ⚠️ **設計知見 — `IsCoherent.galoisTransport`/`mapRingEquiv_inner` の `hσ : ∀ z, σ(star z)=star(σ z)` は global には σ∈{id,conj} しか満たさない** (σ star-可換 ⟺ σ(ℝ)=ℝ)。wild cyclotomic σ では「指標値 (∈ℚ_n, Galois abelian で conj と可換) 上でのみ star-可換」に弱化が必須 (証明は値でしか hσ を使っていないので安全な厳密一般化)。+ 𝒮^u⊆𝒮 (galoisPerm+induction 可換) + Dade τ∘u=u∘τ (「by definition of τ」部分) | ~200-300 | (1.9)✅ | 🔄 (5.9.a) のみ |
