...
Isaacs: 1 章 = 1 ディレクトリ, 入口は `Main.lean`. BG / Peterfalvi: 1 節 = 1 ファイル.

**分割は行数でなく「編集局所性 + 再ビルドレイテンシ」基準** (2026-05-27 改訂; 旧「1500-2000 行で
分割」を置換). `lake build` はファイル単位で全再 elaboration (≈ 5s 固定 + ~2ms/行, 証明密度で変動)
するので、1 edit-cycle のレイテンシが本質。トリガー = 「**今伸ばしている章** かつ **leaf 再ビルドが
痛い** (目安 >~4000 行 or >~12-15s)」。**休眠中の巨大ファイルは行数だけでは割らない** (キャッシュ済で
無害, 今割れば先回り分割). 割るときは active frontier を小さな leaf `Main.lean` に残し、凍結 subsection
を上流ファイルへ押し出す (線形 import チェーンで上流を編集すると下流全部が再ビルドされる点に注意).
細分化 (<~800-1000 行乱立) は固定 5s/ファイルで逆効果. 詳細は ROADMAP「育ってから分割」+ memory
`build-perf-bottleneck`.
