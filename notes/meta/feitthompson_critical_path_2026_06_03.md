# FeitThompson 実 critical path — 訂正版 roadmap (2026-06-03)
...
```
feitThompson (odd ⇒ solvable)                      [FeitThompson.lean:21 — 🔴 body なし裸 sorry]
  └─ (欠落) minimal-counterexample 還元: odd 非可解 ⇒ ∃ minimal simple odd G
  └─ noMinimalSimpleOdd_of_section16 (hG : BG.IsMinimalSimpleOdd G) (hyp : S16.Hypothesis G) : False
       = BG.AppC.final_contradiction hG hyp                       [FeitThompson.lean:29 — 配線済]
...
**入力 `BG.IsMinimalSimpleOdd G` と `S16.Hypothesis G` は仮説のまま** — それらを**構成**する wiring
(BG §7-16 局所解析 + Pf §10-15 型解析) が未構築。これが「~214 sorry が closure に未登場」の理由
(closure は IsMinimalSimpleOdd/S16.Hypothesis を hypothesis 取りするので現状小さい)。
...
| **最上位還元** | 1 sorry (+構成) | odd⇒minimal-simple + IsMinimalSimpleOdd/S16.Hypothesis 構成 | 裸 sorry、未着手の大 assembly |
