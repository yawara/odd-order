...
> 横断運用ドキュメント。**標準監視ペース = 15 分間隔** (cron `7,22,37,52 * * * *`、:00/:30 回避・15 分均等、**ユーザー 2026-07-05: Opus 切替で 15 分に復帰**; 同日 Fable 使用中は速度考慮で一時 30 分 `13,43`、2026-07-02〜07-05 は 15 分、2026-06-29〜07-02 は 30 分、それ以前は 15 分)。cron は session-only ([[cron-dies-on-model-switch]]; CronCreate `durable:true` は本環境で disk 永続せず session-only 扱い) ゆえ、**再作成時は必ずこのペース `7,22,37,52` で**作る。main worktree = `/home/ywr/odd-order`。
