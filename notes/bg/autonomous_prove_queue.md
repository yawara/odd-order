...
| 8.8 | **precursor(2)** minimal ψ-inv ⇒ special exp p (G Thm 3.7/3.8/3.10) | `isSpecial_expP_of_minimal_pprime_action` | Thm 3.6/3.7/3.10 + 🔴 **G Thm 2.2 (indecomp⇒homocyclic)** | 🟡 **IN PROGRESS** (2026-05-31): `IsSpecial` def + 設計ノート `s04_precursor2_special_expP_design.md` + ✅ **G Thm 2.4 (Ω₁-extension)** `actionCommutator_eq_bot_of_omega1_le_fixedPoints`@CoprimeAbelianPGroup.lean (commit a2780ec, sorry-free; **Thm 2.2 非依存と判明**)。**次 = G Thm 2.2 (homocyclic, 最難)** = Lemma 2.1 + coprime Maschke (Thm 3.3.2) → Thm 3.7 → 3.8/3.10。**【大】複数セッション** |
