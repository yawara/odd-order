...
| 8.8 | **precursor(2)** minimal ψ-inv ⇒ special exp p (G Thm 3.7/3.8/3.10) | `isSpecial_expP_of_minimal_pprime_action` | 🔴 **G Thm 2.2 (indecomp⇒homocyclic) / 2.4 欠落** + Thm 3.6/3.7/3.10 | 🟡 **SCOPED** (2026-05-31): `IsSpecial` def 着地 (IsExtraspecial.lean) + 設計ノート `notes/bg/s04_precursor2_special_expP_design.md`。**【大】複数セッション規模**。次 leaf = **G Thm 3.6 (`[P,A,A]=[P,A]`)** (present `fixedPoints_sup_actionCommutator_eq_top` から assemblable、独立着手推奨)。本当の壁 = Thm 2.2 homocyclic + Thm 2.4 (coprime 表現論、repo 不在) |
