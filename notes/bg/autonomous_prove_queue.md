...
| 8.8 | **precursor(2)** minimal ψ-inv ⇒ special exp p (G Thm 3.7/3.8/3.10) | `isSpecial_expP_of_minimal_pprime_action` | 全 kernel ✅ (新規インフラ不要) | 🟢 **ASSEMBLY-READY** (2026-05-31 大発見): **G Thm 2.2 (homocyclic) 不要と確定** (Thm 3.7 は Thm 2.4 を ⟨ψ⟩ に適用して elem-ab を出す; homocyclic は Gorenstein の remark のみ)。**全 kernel が既存**: Thm 2.4✅(`CoprimeAbelianPGroup`)/Thm 3.5✅(`fixedPoints_sup_actionCommutator_eq_top`)/Thm 3.6✅(`actionCommutator_restrict_self_map_subtype_eq`@OperatorQuotientAction、既存!)/Maschke✅(`OperatorMaschke.exists_aInvariant_complement_in_omega1_quotient` を S=P' で再利用)/three-subgroups✅/stability✅。**残 = Thm 3.7/3.8/3.10/precursor(2) の純 assembly (~150-250行, precursor(1) と同型)**。詳細 `s04_precursor2_special_expP_design.md`。 |
