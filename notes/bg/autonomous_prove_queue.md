...
| 8.7 | **precursor(1)** `pRank_le_two_of_scn3_empty` (=G Thm4.15(i)) | `pRank_le_two_of_scn3_empty` | gate: 8.6✅ + GL橋✅。**§5 も開く** | 🟡 **IN PROGRESS** (2026-05-31 直接証明): easy/translation half ✅ (`normalAbelian_pRank_le_two_of_scn3_empty`@S04 + companions `exists_maximalAbelianNormal_ge`/`exists_maxRank_maximalAbelianNormal`@SCN) + **GL-squeeze kernel ✅** (`card_le_prime_of_isPGroup_of_not_sq_dvd` + `card_pSubgroup_mulAut_le_prime_of_card_le_prime_sq`@PRank, 全 sorry-free)。**残 = G Thm4.15(i) 本体 assembly のみ (mechanical, kernels 全部手元)** = `pRank_le_two_of_normalAbelian_pRank_le_two` (Lemma4.14 + conjNormal index + rank arithmetic, ~130行)。assemble = G415i ∘ translation。詳細 issue 0051 |
