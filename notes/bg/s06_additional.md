...
> **2026-05-28 追記**: §6 → App.A+B(Puig)で Gorenstein 依存を捨てる経路の設計決定・依存閉包・J→L 大域置換の検証は [`notes/meta/log/bg_s6_appAB_route_2026_05_28.md`](../meta/bg_s6_appAB_route_2026_05_28.md) に集約。要点: Isaacs は Z(J) を省くため 6.2 は **App.B Thm B.4(Puig L(S))**で代替、ゲートは **A.4(b)+A.4(c)**(7.3 reduction、最深部 7.3 は repo 既証)。
