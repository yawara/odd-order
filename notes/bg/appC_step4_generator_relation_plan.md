...
- **🆕 (2026-06-06) Step3 bad branch through `P char PU` done**:
  ...
  giving `step3_badBranch_t_sq_normalizes_P : t² ∈ N_G(P)`.  The remaining mathematical
  chunk is `P₁` normalizes `P`, then the `P₀=P₁` contradiction.
...
5. **w_i=1 / (C.10)**: ✅ Step3 が `s₁w₃^{p-1}s₁⁻¹∈U` を与え、C.6 の
   ...
   Lean 化済。bad branch は `step3_badBranch_t_sq_normalizes_P` で `t² ∈ N_G(P)` まで進んだ。
   残りは BG Step3 後半の `P₁` normalizes `P` → `P₀=P₁` 矛盾と、global C.6 非零 fact の接続。
...
| Step3 適用 | `S₁W₃^(p-1)S₁⁻¹∈U` + C.6 `c₁,c₃≠0` ⟹ `w₁=w₂=w₃=1` | ✅ consumer landed; ✅ U-branch/dichotomy connected by `relationC9_w_eq_one_and_relationC10_of_w3_step3_inf_eq_U` and `relationC9_w_eq_one_and_relationC10_or_step3_inf_eq_P_sup_U`; bad branch pushed to `t²∈N_G(P)` by `step3_badBranch_t_sq_normalizes_P` |
| **(C.10)** | `t²s₁t⁻¹s₂t⁻¹s₃=1` | ✅ included once the Step3 dichotomy is in the `U` branch; otherwise the remaining task is BG Step3's `P₁≤N_G(P)` and `P₀=P₁` contradiction from `t²∈N_G(P)` |
...
3. **(C.7)–(C.10)** (Frobenius 置換 + Step3):
   ...
   - Step3 の `U`-branch producer は接続済み。bad branch は `t²∈N_G(P)` まで Lean 化済み。
     残る producer は BG Step3 後半の `P₁≤N_G(P)`/Sylow 矛盾と、global `(C.6)` の `c₁,c₃≠0` 配線。
...
- 🔴 残り (Step 4 の残作業、(X)/(XI) infra は完了):
  1. **Step3 bad-branch 排除**: exact `(C.9)` の membership と Step3 dichotomy は
     ...
     `t²∈N_G(P)` までは `step3_badBranch_t_sq_normalizes_P` で landed。残る producer は
     BG Step3 後半の `P₁` normalizes `P` → `P₀=P₁` 矛盾を形式化し、global C.6 の
