...
6. **Frobenius 量子化**: μ_c := λ_c − λ_c³ が 1 つでも非零なら: 同符号パターン辺
   ...
   ⟹ **λ₊, λ₋ ∈ 𝔽₃**。
7. 𝔽₃ 内で Δ, Σ̄ ≠ 0 の残候補 = **(±1, 0), (0, ±1) の 4 本**。各候補: 候補依存の
   一方の同符号パターン辺 ((±1,0) は (−,−)、(0,±1) は (+,+)) が `K(p) = 0` を強制して
   即死し、他方の同符号辺はその swap がこの致死パターンになる ⟹ **同符号辺 1 本で死**;
   混合パターン ((+,−): K(p) = ±δ₀^e / (−,+): K(p) = ∓δ₁^e) は e-冪単射性で p ごとに
   r を一意に pin ⟹ **out-degree |T|−1 ≤ 2 ⟹ |T| ≤ 3、|T| ≥ 4 と矛盾**
   (|T| = (Q−3)/4 は無条件に ≥ 4、`card_paleySet_lower`)。
   パターン人口仮説・G-定数型残渣は**不要**。

**使用仮説の総目録**: χ(−1) = −1 (Q ≡ 3 mod 4)、e 奇・`z^{e³} = z`・gcd(e, Q−1) = 1、
`K ≠ 0`、|T| ≥ 4、swap の成分反転。equidistribution・Weil 評価・Davenport・鳩の巣すら
**最終版では不要** (κ は singleton slot でも自明に well-defined、可換子 loop は
辺再利用で組めるため)。
...
**✅ 最終 assembly 監査完了 (2026-08-13、effort max、CONFIRMED・fatal 0)**:
量化子構造・循環なし・全退化世界の walk・e²-エンドポイント glue の end-to-end 数値
検証・独立スポットチェック 45/45×2 指数。重み恒等式は体論の恒等式として紙上再導出済
(数値依存なし)。(EX)⟹master の一般 q 一意性も anchor 論法で明示的に再導出された
(統合文書に書き下すこと)。監査 nit 6 件は本節に反映済 (step 7 の候補依存パターン・
仮説目録から鳩の巣を削除)。

残作業 (数学は完結、記録と形式化):
1. 統合された完全証明文書 (§1–§6.2 を 1 本の定理として書き下し; anchor 論法の
   uniqueness を明示化)。
