...
| `fitting_not_ti_cases` (Thm 15.7) | L4234 | ✅ **issue 3022 完了 (2026-07-19)**。逐条精査 (07-18) では (b) 準恒真・(d) 3/4 欠落・(e) 恒真だったが、全条項を修復済: (a) 元から faithful / (b) `X = F(M)∩F(M)^g` を束縛し `∀ g ∉ M` 形へ / (c) `=`→`≤` は**正当**（MathComp 準拠、原文が overstatement）/ (d) `sigmaComplement_structure_of_not_fittingIsTI` + `quotientE2MulEquivE1` で 4 条項すべて / (e) 新 leaf `S16_MainResults/FittingNonTITrichotomy.lean` の **`fitting_not_ti_structure_e`** が Coq `nonTI_Fitting_structure` (:947-950) と同形で (e1)/(e2)/(e3) 全条項（共通 conjunct を括り出し (e2) ∨ (e3) を内側 disjunction に）。⚠ 恒真性チェック済: 第2枝は `¬可換 O_p(M_F)` であって `¬可換 M_F` ではないので (e1) の否定にならない。`p` は `|X|` に束縛（`∃ p` で decoupled ではない）|
