...
**次ステップ**:
- q=p 分岐の `C_G(W) ∩ C_G(V/W)` を表す subgroup/predicate を定義し、上の subgroup helper に渡せる形にする.
