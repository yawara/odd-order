...
- **2026-05-25**: q=p 分岐用に scalar character helper 群 (`monoidHom_units_eq_one_of_isPGroup_charP` ほか) と、`C_G(W) ∩ C_G(V/W)` の可換性に対応する線型代数補題 `end_commute_of_fixed_on_submodule_and_quotient` を追加済み. 次はこれを `ρ` の image/subgroup statement へ接続する.
...
**次ステップ**:
- `end_commute_of_fixed_on_submodule_and_quotient` を使い、representation image 上で `C_G(W) ∩ C_G(V/W)` が可換になる subgroup-level helper を追加する.
