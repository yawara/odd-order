...
theorem isSimpleGroup_of_isPerfect_of_isQuasiPreprimitive_of_isSolvable_stabilizer
    ...
    (base : Ω) [Group.IsSolvable (stabilizer G base)] :
    IsSimpleGroup G := by
  ...
  · right
    ...
    let : Group.IsSolvable (G ⧸ N) := Group.isSolvable_of_surjective hf
    have hsub : Subsingleton (G ⧸ N) := by
      by_contra h
      let : Nontrivial (G ⧸ N) := not_subsingleton_iff_nontrivial.mp h
