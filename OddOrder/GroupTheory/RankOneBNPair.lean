...
* `IsFGH.dOrbitRel_f`, `IsFGH.dOrbitRel_fj_cube` — `⟨f, j⟩` acts on the `D`-orbits of
  `Q^#` as a quotient of the dihedral group of order `6`; `IsFGH.dOrbitRel_f_of_le` and
  `IsFGH.dOrbitRel_fj_cube_of_mem` are the same for a `t`-stable subgroup of `D`, which
  is what Ch. IV §2 needs (its orbits are those of `K W`).
...
/-- **`f` descends to the orbit set of a `t`-stable subgroup of `D`** (Peterfalvi
Part II, Ch. IV §2, p. 124: the orbits the section works with are those of `K W`, not of
all of `D`).

The proof of `IsFGH.dOrbitRel_f` only ever conjugates by `a^t`, so any subgroup of `D`
closed under `a ↦ a^t` carries the same statement. -/
theorem IsFGH.dOrbitRel_f_of_le (hS : Setup M Q D t) (H : IsFGH M Q D t f g h)
    (hxQ : x ∈ Q) (hx1 : x ≠ 1) {S : Subgroup L} (hSD : S ≤ D)
    (hSt : ∀ a ∈ S, t * a * t ∈ S) {a : L} (haS : a ∈ S) :
    dOrbitRel S (f x) (f (a⁻¹ * x * a)) :=
  ⟨t * a * t, hSt a haS, (hThree hS H hxQ hx1 (hSD haS)).1⟩

/-- **(H5) on the orbit set of a subgroup of `D`** — the conjugator is `h(x)⁻¹`, so the
statement holds for any subgroup containing it. -/
theorem IsFGH.dOrbitRel_fj_cube_of_mem (hS : Setup M Q D t) (H : IsFGH M Q D t f g h)
    (hxQ : x ∈ Q) (hx1 : x ≠ 1) {S : Subgroup L} (hhS : h x ∈ S) :
    dOrbitRel S x (f ((f ((f x⁻¹)⁻¹))⁻¹)) :=
  ⟨(h x)⁻¹, S.inv_mem hhS, by rw [hFive hS H hxQ hx1]; group⟩
