...
theorem centerIn_ne_bot [Finite G] {p : ℕ} [Fact p.Prime] {H : Subgroup G}
  ...
  have : Nontrivial ↥H := (Subgroup.nontrivial_iff_ne_bot H).mpr hne
  have := hH.center_nontrivial
...
theorem sylow_fitting_map_le_oPiCore [Finite G] {p : ℕ} [Fact p.Prime]
  ...
  have hSn : (S : Subgroup ↥(Ch01.fitting G)).Normal := Ch01.Sylow.normal_of_isNilpotent S
  have hSc : (S : Subgroup ↥(Ch01.fitting G)).Characteristic :=
    Sylow.characteristic_of_normal S hSn
  have : ((S : Subgroup ↥(Ch01.fitting G)).map (Ch01.fitting G).subtype).Normal :=
...
theorem not_commute_of_coprime_orderOf_card_fitting [Finite G] [Group.IsSolvable G]
    (hCN : ∀ z : G, z ≠ 1 → Group.IsNilpotent ↥(Subgroup.centralizer ({z} : Set G)))
  ...
  have hCnil : Group.IsNilpotent ↥(Subgroup.centralizer ({x'} : Set G)) := hCN x' hx'1
  ...
  have hcentF : ∀ f ∈ Ch01.fitting G, Commute y' f := by
    ...
    rintro ⟨r, hr⟩ z hz
    have : Fact (r : ℕ).Prime := ⟨Nat.prime_of_mem_primeFactors hr⟩
...
theorem exists_sylow_eq_oPiCore_of_normal_pPrime_le_fitting
  ...
  have hOpF : Ch03.oPiCore ({p} : Set ℕ) G ≤ Ch01.fitting G := by
    have : Group.IsNilpotent ↥(Ch03.oPiCore ({p} : Set ℕ) G) := hOpPG.isNilpotent
    exact Ch01.nilpotent_normal_le_fitting
  have hNnil : Group.IsNilpotent ↥N := isNilpotent_of_le_of_isNilpotent hNF inferInstance
  ...
  have hz1 : (z : G) ≠ 1 := fun h => hzne (Subtype.ext h)
  have hCnil : Group.IsNilpotent ↥(Subgroup.centralizer (N : Set G)) :=
  ...
  have hPn : ((P.subtype hPC : Sylow p ↥(Subgroup.centralizer (N : Set G))) :
      Subgroup ↥(Subgroup.centralizer (N : Set G))).Normal :=
    Ch01.Sylow.normal_of_isNilpotent _
  have hPc : ((P.subtype hPC : Sylow p ↥(Subgroup.centralizer (N : Set G))) :
