...
/-! ## Commutators and the square filtration -/
...
/-- If `[P,A] ≤ A²`, then the commutator of a square with `A` lies in
`A⁴`. This is the element-level collection step used before passing to the
subgroup generated by squares. -/
theorem commutatorElement_sq_mem_agemo_two_map
  ...
  have hP_A2 : ⁅(⊤ : Subgroup P), (Agemo A 2 1).map A.subtype⁆ ≤
    ...
    commutator_agemo_one_map_le_agemo_two_map hAcomm hPA
  have hdA2 : ⁅g, a⁆ ∈ (Agemo A 2 1).map A.subtype :=
    hPA (Subgroup.commutator_mem_commutator (Subgroup.mem_top g) ha)
  have hgdA4 : ⁅g, ⁅g, a⁆⁆ ∈ (Agemo A 2 2).map A.subtype :=
    hP_A2 (Subgroup.commutator_mem_commutator (Subgroup.mem_top g) hdA2)
  obtain ⟨d, hd, hdval⟩ := Subgroup.mem_map.mp hdA2
  have hd2A : d ^ 2 ∈ Agemo A 2 2 :=
    mem_agemo_succ_iff.mpr ⟨d, hd, rfl⟩
  have hd2 : ⁅g, a⁆ ^ 2 ∈ (Agemo A 2 2).map A.subtype := by
    rw [← hdval]
    exact Subgroup.mem_map_of_mem A.subtype hd2A
  have heq : ⁅g ^ 2, a⁆ = ⁅g, ⁅g, a⁆⁆ * ⁅g, a⁆ ^ 2 := by
    simp only [commutatorElement_def, pow_two]
    group
  rw [heq]
  exact ((Agemo A 2 2).map A.subtype).mul_mem hgdA4 hd2

/-- If `[P,A] ≤ A²` and `A` is normal abelian, then the subgroup generated
by squares of `P` centralizes `A` modulo `A⁴`. -/
theorem commutator_agemo_two_one_le_agemo_two_map
  ...
  haveI hA4normal : A4.Normal := by
    ...
    infer_instance
  let K : Subgroup P :=
    { carrier := {x | ∀ a : P, a ∈ A → ⁅x, a⁆ ∈ A4}
      one_mem' := by simp
      mul_mem' := by
        intro x y hx hy a ha
        rw [commutatorElement_mul_left_eq_conj_mul]
        exact A4.mul_mem (hA4normal.conj_mem _ (hy a ha) x) (hx a ha)
      inv_mem' := by
        intro x hx a ha
        rw [commutatorElement_inv_left]
        have hax : ⁅a, x⁆ ∈ A4 := by
          rw [← commutatorElement_inv]
          exact A4.inv_mem (hx a ha)
        simpa using hA4normal.conj_mem _ hax x⁻¹ }
  have hsquare (g : P) : g ^ 2 ∈ K := by
    intro a ha
    exact commutatorElement_sq_mem_agemo_two_map hAcomm hPA g ha
  have hagemoK : Agemo P 2 1 ≤ K := by
    rw [Agemo, Subgroup.closure_le]
    rintro _ ⟨g, rfl⟩
    simpa using hsquare g
  rw [Subgroup.commutator_le]
  intro x hx a ha
  exact hagemoK hx a ha
