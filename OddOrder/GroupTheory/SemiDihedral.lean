...
/-- The canonical element outside the cyclic subgroup generated by `c 1` is not in it. -/
theorem ca_zero_not_mem_zpowers_c_one (k : ℕ) :
    ca (0 : ZMod (2 ^ k)) ∉ Subgroup.zpowers (c 1 : SemiDihedralGroup k) := by
  intro h
  have h' := (mem_zpowers_iff_mem_range_orderOf
    (x := (c 1 : SemiDihedralGroup k)) (y := ca (0 : ZMod (2 ^ k)))).mp h
...
theorem zero_isCyclic : IsCyclic (SemiDihedralGroup 0) := by
  ...
  have hne : (ca 0 : SemiDihedralGroup 0) ≠ 1 := by
    ...
    injection h
  rw [Nat.card_eq_fintype_card, card]
  exact (orderOf_eq_prime ca_zero_sq hne).trans (by norm_num)
