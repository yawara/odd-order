...
theorem RegularOperatorSetup.commutator_eq_bot [Finite R] [Finite B]
  ...
  have hSinv : IsAInvariant hyp.act S := by
    have : S.Characteristic := Omega.characteristic
  ...
  have : IsCyclic (↥S ⧸ T) := isCyclic_of_prime_card hcardq
...
theorem RegularOperatorSetup.commutator_le_centralizer [Finite R] [Finite B]
  ...
  have : IsCyclic (↥(Omega R p 1) ⧸ Subgroup.centralizer
...
/-- **BG `(E.25)`'s witness, contrapositive form**: if `A` is generated by `a` over `A'`,
`B` by `b` over `B'`, the cross terms `⁅A', B⁆` and `⁅A, B'⁆` lie in `K`, and `⁅a, b⁆ ∈ K`,
then all of `⁅A, B⁆` lies in `K`.

Applied to `A = Hᵢ`, `B = Hⱼ`, `A' = Hᵢ₊₁`, `B' = Hⱼ₊₁`, `K = H_k`, this says that
`⁅Hᵢ, Hⱼ⁆ ⊄ H_k` forces `⁅wᵢ, wⱼ⁆ ∉ H_k` — BG's *"`⁅wᵢ,wⱼ⁆ ∈ H_{k-1} − H_k`"* in `(E.25)`
(the `∈ H_{k-1}` half is `⁅T,T⁆ ≤ H_{k-1}`, from the minimality of `k`).

In `G/K` every element of `A` is `ā^m·ā'` with `ā'` central against `B̄`, and symmetrically,
so `commutatorElement_zpow_mul_zpow_mul` collapses the commutator to `⁅ā, b̄⁆^{mn} = 1`. -/
theorem commutator_le_of_generators {G : Type*} [Group G] {K : Subgroup G} [K.Normal]
    {A B A' B' : Subgroup G} [A'.Normal] [B'.Normal] {a b : G}
    (haA : a ∈ A) (hbB : b ∈ B) (hB'B : B' ≤ B)
    (hA : A ≤ Subgroup.zpowers a ⊔ A') (hB : B ≤ Subgroup.zpowers b ⊔ B')
    (hA'B : ⁅A', B⁆ ≤ K) (hAB' : ⁅A, B'⁆ ≤ K) (hab : ⁅a, b⁆ ∈ K) :
    ⁅A, B⁆ ≤ K := by
  -- Membership in `K` is commutation in `G/K`.
  have hcomm : ∀ x y : G, ⁅x, y⁆ ∈ K →
  ...
  intro x hx y hy
  -- Decompose `x = a^m · a'` and `y = b^n · b'`.
  obtain ⟨s, hs, a', ha', rfl⟩ : x ∈ (Subgroup.zpowers a : Set G) * (A' : Set G) := by
    rw [← Subgroup.mul_normal]; exact hA hx
  obtain ⟨m, rfl⟩ := hs
  obtain ⟨t, ht, b', hb', rfl⟩ : y ∈ (Subgroup.zpowers b : Set G) * (B' : Set G) := by
    rw [← Subgroup.mul_normal]; exact hB hy
  obtain ⟨n, rfl⟩ := ht
  -- The three commutation facts, in `G/K`.
  have h1 : Commute ((QuotientGroup.mk' K) a') ((QuotientGroup.mk' K) b) :=
    hcomm _ _ (hA'B (Subgroup.commutator_mem_commutator ha' hbB))
  have h2 : Commute ((QuotientGroup.mk' K) a') ((QuotientGroup.mk' K) b') :=
    hcomm _ _ (hA'B (Subgroup.commutator_mem_commutator ha' (hB'B hb')))
  have h3 : Commute ((QuotientGroup.mk' K) b') ((QuotientGroup.mk' K) a) :=
    (hcomm _ _ (hAB' (Subgroup.commutator_mem_commutator haA hb'))).symm
  have hz : ⁅(QuotientGroup.mk' K) a, (QuotientGroup.mk' K) b⁆ = 1 := by
    rw [← map_commutatorElement]; exact (QuotientGroup.eq_one_iff _).mpr hab
  -- Collapse the commutator in `G/K`.
  have hzc : ⁅(QuotientGroup.mk' K) a, (QuotientGroup.mk' K) b⁆ ∈ Subgroup.center (G ⧸ K) := by
    rw [hz]; exact Subgroup.one_mem _
  have hgoal : (QuotientGroup.mk' K) ⁅a ^ m * a', b ^ n * b'⁆ = 1 := by
    rw [map_commutatorElement, map_mul, map_mul, map_zpow, map_zpow,
      commutatorElement_zpow_mul_zpow_mul h1 h2 h3 hzc m n, hz, one_zpow]
  rw [QuotientGroup.mk'_apply] at hgoal
  exact (QuotientGroup.eq_one_iff _).mp hgoal

/-- **BG's maximal `i` is never `0`** — the diagonal case of `commutator_le_of_generators`.

If both cross terms `⁅A', A⁆` and `⁅A, A'⁆` land in `K` and `A` is generated by `w` over
`A'`, then `⁅A, A⁆ ≤ K`, because the remaining generator pairing is `⁅w, w⁆ = 1`.

Applied with `A = H₀ = T`, `A' = H₁`, `K = H_k`, this says: if BG's maximal `i` were `0`
then (`j ≤ i` forces `j = 0` too, so both maximality statements read as the hypotheses here)
`T' ≤ H_k`, contradicting the minimality of `k`.  Hence `i ≥ 1`, and with `i + 2 ≤ k` from
`range_of_max_commutator_indices`, **`k ≥ 3`**.

That lower bound is what rules out the alternative recursion `t_{i+1} = tᵢ·t₀` in `(E.23)`:
that alternative forces `H₁ = T'·H₂`, hence `H₁ ≤ H₂` once `k ≥ 3`, contradicting
`|H₁ : H₂| = p`. -/
theorem commutator_self_le_of_generator {G : Type*} [Group G] {A A' K : Subgroup G}
    [K.Normal] [A'.Normal] {w : G} (hw : w ∈ A) (hA'A : A' ≤ A)
    (hgen : A ≤ Subgroup.zpowers w ⊔ A')
