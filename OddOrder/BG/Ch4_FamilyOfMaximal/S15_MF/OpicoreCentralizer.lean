...
import OddOrder.BG.Ch4_FamilyOfMaximal.S15_MF.OpicoreCentralizerBasic
...
⚠ **This is the S15-layer partial**, deliberately kept at this import position.
`OpicoreCentralizer` sits *above* `WitnessPGroup` and `S16` in the import order
...
2. **(e2) vs (e3).**  BG refines the non-abelian case by type: for `M ∈ ℳ_F` the exponent of `M/H`
   divides `q − 1` for every `q ∈ π(H)` (`typeF_exponent_dvd_sub_one_of_invariant_card`,
   also used by `S16`'s `isTypeI_of_isTypeF`); for `M ∈ ℳ_{P₁}` (the Singer case) one gets
   `|O_p(H)| = p³` and `|M/H| ∣ p + 1`
...
theorem maxNilpotentNormalHall_derivedInG_eq_Msigma_of_isTypeP2_of_tau2_empty [Finite G]
  ...
  · -- `⊆`
    have : Group.IsNilpotent ↥(maxNilpotentNormalHall (derivedInG M)) :=
...
theorem not_prime_mem_tau2_of_centralizer_kappaCompl_not_le [Finite G] (hG : IsMinimalSimpleOdd G)
  ...
  have hrU : r ∈ piSet U := by
    rwa [piSet, Set.mem_ofPred_eq, ← Nat.card_congr (Subgroup.subgroupOfEquivOfLe hUM).toEquiv]
...
theorem exists_sylow_eq_opiCore_of_mem_sigma_of_msigma_nilpotent [Finite G]
  ...
  have : Group.IsNilpotent ↥Mσ := hMσ ▸ hnil
...
theorem mem_sigma_and_le_Msigma_of_rank2_centralizer_kappa [Finite G]
  ...
  have hApiσ : Ch03.Subgroup.IsPiGroup (OddOrder.BG.Ch3.S10.sigma Mstar) (A.subgroupOf Mstar) := by
    ...
    exact hp ▸ hq1σ
  have : ((OddOrder.BG.Ch3.S10.Msigma Mstar).subgroupOf Mstar).Normal := by
    rw [OddOrder.BG.Ch3.S10.Msigma_subgroupOf]; infer_instance
...
theorem partner_opiCore_nonabelian [Finite G]
  ...
  have hObarN : Obar.Normal := hObardef ▸ Ch03.oPiCore.normal _ _
