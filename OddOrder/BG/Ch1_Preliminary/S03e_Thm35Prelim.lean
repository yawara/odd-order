...
/-!
# BG §3 — Theorem 3.5, the step infrastructure (mmd L925-951)

The public machinery for BG Theorem 3.5: the sub-Frobenius constructor, the step-(3.5)
dichotomy `U ⊆ C_V(N)` or `W ⊆ C_V(N)`, the step-9 Clifford hard core (general field),
the weight-space/orbit-count layer for an abelian normal subgroup, and the step-9
assembly bridges.

Split from `OddOrder.BG.Ch1_Preliminary.S03e_Thm35` (issue 0149, the longFile-1500
campaign); `S03e_Thm35` imports this leaf, so downstream imports are unchanged.
-/
