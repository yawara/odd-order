...
/-!
# BG §2F — the odd two-dimensional induction machinery

The strong-induction web behind Theorem 2.6 (`odd_two_dim_abelian`): the
determinant-kernel spine, the `O_p`-core branches, the base-change/algebraic-closure
transport, up to the core-spine commutativity layer.  Two formerly `private` spine
lemmas are public here because the assembly layer in the parent consumes them
(CLAUDE.md: no cross-file `private`).
...
theorem determinantKernel_hind_of_odd_two_dim_induction_outputs
...
theorem commutative_of_determinantKernel_core_spine_algebraicClosure_charAway
    {F : Type*} [Field F]
    {G : Type*} [Group G] [Finite G]
    {V : Type*} [AddCommGroup V] [Module F V] [Module.Finite F V]
    (ρ : Representation F G V) (hfaithful : Function.Injective ρ)
