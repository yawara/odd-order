...
**ファイル分割 (issue 0149)**: (3.29)–(3.38) は IH を消費しないので、本ファイルからは
`S03f_Endgame.endgame_contradiction` として切出し済み — ここには IH を消費する
(3.6)–(3.28) と最終呼び出しだけが残る。
...
-- 単一の巨大宣言 (thm36_aux) — IH-free な末尾 ((3.29)–(3.38)) は `S03f_Endgame` に切出済み
-- (issue 0149)。残りは IH を消費する Phase A–D 前半で、誘導の内側から切り出せない。
...
(3.29)–(3.38) (IH-free) は `S03f_Endgame.endgame_contradiction` に分離 (Phase F はさらに
`S03f_OrbitParity.orbit_parity_contradiction`) — ここでは (3.28) までの確立事実を渡して
`False` を受け取るだけ.
...
private theorem thm36_aux : ∀ (n : ℕ)
  ...
  | _ n IH =>
    ...
    have hR₀_le_NPG : R₀ ≤ Subgroup.normalizer ((P.map H.subtype : Subgroup G) : Set G) :=
      fun _g hg => mem_normalizer_map_subtype_of_isAInvariant (hφ ▸ hP_inv) (hR₀R hg)
