...
* **Phase F — 最終矛盾** (3.38–): `V = ⊕ Vᵢ` (`Vᵢ = C_V(Kᵢ) ≠ 1`, index-`q` `Kᵢ`; **Prop 1.16**);
  ...
  even from the orbit structure) yields the final contradiction.
...
Phase F = `Kᵢ` 族 (index-`q`, `Vᵢ := C_{V_G}(Kᵢ) ≠ ⊥`) を averaging 射影 `eᵢ = avgConj` で
直交化し (`V = V₁×⋯×Vₙ` の代替), Prop 1.16(2) で spanning, (3.11) で `PR`-推移性,
ノルム写像 `avgConj R₀` の単射性+全射性 (`|Vᵢ| = p` = (3.19)) で「非固定 index は単一
`R₀`-軌道」, 固定 index は `Kⱼ = ⁅K_G,R₀⁆` (巡回 Aut 可換) で高々 1 個 ⟹ `n = r ∨ n = r+1`;
`n = r` は `P`-固定点 + (3.24) + (3.14) で, `n = r+1` は偶奇 (`n ∣ |G|` odd vs `r+1` even) で矛盾.

(`maxHeartbeats`: equations (3.6)–(3.38) を 1 つの minimal-counterexample 証明で運ぶため宣言が
巨大で、既定の 200000 を超える。ファイル分割で解消予定。) -/
private theorem thm36_aux : ∀ (n : ℕ)
  ...
  | _ n IH =>
    ...
    -- ===== Phase F ((3.38)): `V = V₁ × ⋯ × Vₙ` and the orbit-parity contradiction =====
    -- `K` is elementary abelian of order `> q²` acting faithfully (3.28) and fixed-point-freely
    -- (3.14) on the elementary abelian `p`-group `V`.  Let `K₁, …, Kₙ` be the index-`q` subgroups
    -- of `K_G` with `Vᵢ := C_{V_G}(Kᵢ) ≠ ⊥`; they exist by Prop 1.16(2), the `Vᵢ` are independent
    -- (averaging projections), `PR` permutes them transitively ((3.11)), and the norm map over
    -- `R₀` pins `|Vᵢ| = p` ((3.19)), forcing `n = r` or `n = r + 1` — both impossible.
    classical
    ...
    have horbitSup_normal : ∀ i₀ : {Ki : Subgroup G // Pfam Ki},
      ...
      · -- `P_G ≤ A`
        intro g hg
        exact hApart g (Subgroup.mem_sup_left hg)
      · -- `R₀ ≤ A`
        intro g hg
        exact hApart g (Subgroup.mem_sup_right hg)
    ...
    have htrans : ∀ i j : {Ki : Subgroup G // Pfam Ki}, j ∈ MulAction.orbit (↥A) i := by
      ...
      have hS₁_le_H : S₁ ≤ H :=
        (iSup₂_le fun j _ => hVfam_le j).trans (Subgroup.map_subtype_le V)
      have hS₂_le_H : S₂ ≤ H :=
        (iSup₂_le fun j _ => hVfam_le j).trans (Subgroup.map_subtype_le V)
    ...
    have hnotallfix : ¬ ∀ i : {Ki : Subgroup G // Pfam Ki}, xA • i = i := by
      ...
      haveI hsub : Subsingleton {Ki : Subgroup G // Pfam Ki} := by
        ...
        rcases h311 (Vfam i) (Vfam j) ((hVfam_le i).trans (Subgroup.map_subtype_le V))
          ((hVfam_le j).trans (Subgroup.map_subtype_le V)) (hVfam_disj i j hij) with h7 | h7
        · exact hVfam_ne i h7
        · exact hVfam_ne j h7
