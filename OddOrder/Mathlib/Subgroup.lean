...
theorem AddSubgroup.toZModSubmodule_closure {M : Type*} [AddCommGroup M] (n : ℕ)
    [Module (ZMod n) M] (S : Set M) :
    AddSubgroup.toZModSubmodule n (AddSubgroup.closure S) = Submodule.span (ZMod n) S := by
  refine le_antisymm ?_ ?_
  · intro x hx
    have hx' : x ∈ AddSubgroup.closure S := hx
    have hle : AddSubgroup.closure S ≤ (Submodule.span (ZMod n) S).toAddSubgroup :=
