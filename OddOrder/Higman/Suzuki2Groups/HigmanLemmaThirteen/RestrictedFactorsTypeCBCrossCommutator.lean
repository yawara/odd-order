...
/-!
# Higman's Lemma 13: the cross commutator of restricted type-C/B factors

G. Higman, “Suzuki 2-groups,” *Illinois Journal of Mathematics* 7 (1963),
Lemma 13, p. 92.

For a restricted type-C right factor and a restricted type-B right factor,
this file fixes their explicit canonical ambient eigenfamilies.  The type-C
source equation is supplied by the coordinate data itself.  The type-B
family's square nondegeneracy then gives the cross-commutator inclusion in
the ambient square Frattini subgroup.
-/
...
local instance restrictedFactorsTypeCBCrossLayerIsMulCommutative
    (P : Type uP) [Group P] (i : Nat) :
    IsMulCommutative (lowerCentralLayer P i) :=
  lowerCentralLayerIsMulCommutative P i

local instance restrictedFactorsTypeCBCrossLayerCommGroup
    (P : Type uP) [Group P] (i : Nat) :
...
noncomputable local instance restrictedFactorsTypeCBCrossLayerZModTwoModule
    (P : Type uP) [Group P] (i : Nat) :
    Module (ZMod 2) (Additive (lowerCentralLayer P i)) :=
  lowerCentralLayerZmodModule P i

/-- **Higman Lemma 13 (p. 92), restricted type-C/type-B cross commutator.**

The explicit canonical ambient right-factor families of a restricted type-C
subgroup and a restricted type-B subgroup have cross commutator contained in
`Φ(P)²`.  The restricted actor and Singer conjugacy needed on the type-B side
are reconstructed from the ambient data. -/
theorem commutator_le_frattiniSquare_of_restricted_typeC_typeB_factors
  ...
  exact commutator_le_frattiniSquare_of_typeCB_eigenfamily_spans
    hP hxi hPhiComm hfour hexists hn hr h2r1 c eSquare nu
    hnuPrimitive hconj rightX.lambda rightX.theta
    rightX.kernel_eigenvalue_eq hrightThetaX
    f g hcanonicalX.1 hg hsquare X Z hcanonicalX.2 hcanonicalZ.2
