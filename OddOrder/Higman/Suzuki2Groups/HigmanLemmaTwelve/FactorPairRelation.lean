...
two complementary type-A factors supplied by the caller.  After normalizing
their Frobenius parameters, the result records one of five oriented
relations: B, C in either orientation, or D in either orientation.

This is deliberately finer than the public disjunction
`IsTypeB P ∨ IsTypeC P ∨ IsTypeD P`.  It is the prescribed-pair input for a
later coherent-coordinate theorem; the existential coordinates returned here
alone are not identified across separate pairwise joins.
-/
...
The normalized B/C/D parameter relation is returned for the specified
complementary type-A factors, rather than for factors chosen internally by
...
theorem XiLengthThreeTypeAFactorData.exists_normalizedFactorPairRelation
  ...
  · rcases hRcase with hthetaR1 | ⟨rR, hrR0, hrRhalf, hthetaRfrob, -⟩
    · exact ⟨c, ePhi, nu, dL, dR,
        .typeB (hthetaL1.trans hthetaR1.symm)⟩
  ...
  · rcases hRcase with hthetaR1 | ⟨rR, hrR0, hrRhalf, hthetaRfrob, -⟩
    ...
    · by_cases hre : rL = rR
      · have hthetaEq : dL.theta = dR.theta := by
        ...
        exact ⟨c, ePhi, nu, dL, dR, .typeB hthetaEq⟩
      · have hleftNorm : dL.lambda ^ (1 + 2 ^ rL) = nu := by
          have htheta :
              dL.theta dL.lambda = dL.lambda ^ 2 ^ rL := by
            rw [hthetaLfrob, frobeniusEquiv_pow_apply]
