...
/-!
# Isaacs Problems 1E (pp. 37–38) — Sylow 計数の道具立てと 1E.1–1E.5

Isaacs, *Finite Group Theory* (AMS GSM 92, 2008), Problems 1E の形式化
(campaign issue 1055) の前半。§1E は「与えられた位数の群が単純になれない」ことを Sylow の
個数 `n_q = |Syl_q(G)|` の合同・整除条件から導く演習からなる。本ファイルはそこで繰り返し
使う道具立てと, 一般形の 1E.1 / 1E.2 および具体的位数の 1E.3 / 1E.4 / 1E.5 を扱う
(残りの 1E.6–1E.8 は `ProblemsNonSimple.lean`)。
...
- `mul_comm_of_card_eq_prime_sq` / `le_normalizer_of_card_eq_prime_sq` — 位数 `p²` の
  部分群は可換で, その部分群を正規化する。
-/
...
theorem mul_comm_of_card_eq_prime_sq {G : Type*} [Group G] {P : Subgroup G} {p : ℕ}
...
theorem le_normalizer_of_card_eq_prime_sq {G : Type*} [Group G] {P D : Subgroup G} {p : ℕ}
