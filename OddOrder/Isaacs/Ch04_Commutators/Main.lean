...
/-- **BG Lemma 1.7(d) (⊇ direction)**: in a finite `p`-group `R`, the subgroup
generated by commutators and `p`-th powers lies in `Φ(R)`. -/
theorem commutator_sup_pow_closure_le_frattini
    {p : ℕ} [Fact p.Prime] {R : Type*} [Group R] [Finite R]
    (hR : IsPGroup p R) :
    commutator R ⊔ Subgroup.closure (Set.range (fun x : R => x ^ p)) ≤ frattini R := by
  apply sup_le
  · exact commutator_le_frattini_of_pgroup hR
  · rw [Subgroup.closure_le]
    rintro _ ⟨x, rfl⟩
    exact pow_p_mem_frattini_of_pgroup hR x

/-- **BG Lemma 1.7(d) (⊆ direction)**: in a finite `p`-group `R`, `Φ(R)` lies
in the subgroup generated by commutators and `p`-th powers.

Let `K = R' ⊔ ⟨x^p | x ∈ R⟩`. Since `R/K` is elementary abelian, Isaacs Lemma 4.5
gives `Φ(R) ≤ K`. -/
theorem frattini_le_commutator_sup_pow_closure
  ...
  haveI hPowsChar : Pows.Characteristic := by
    dsimp [Pows]
    exact Subgroup.pow_closure_characteristic p
  ...
  have hle : frattini R ≤ K :=
    (frattini_le_iff_isElementaryAbelian_quotient_of_pgroup
      (P := R) (p := p) (N := K) hR).mpr hElem
  simpa [K, Pows] using hle

/-- **BG Lemma 1.7(d)**: for a finite `p`-group `R`,
`Φ(R)` is generated by `R'` and the `p`-th powers. -/
...
If `C_G(F(G))` had a normal subgroup `K` minimal among those not contained in `F(G)`,
then `[K,K] < K` by solvability and minimality forces `[K,K] ≤ F(G)`. Since
`K ≤ C_G(F(G))`, `K ∩ F(G)` is central in `K`, and `K/(K ∩ F(G))` is abelian;
hence `K` is nilpotent, contradicting maximality of `F(G)`. -/
...
This combines the Fitting self-centralizing theorem with Isaacs Ch.4's action-commutator
decomposition. It is the practical form needed later: the kernel of the action on the
Fitting subgroup is also the kernel of the action on the whole group. -/
theorem actionCommutator_eq_bot_of_fitting_le_fixedPoints
  ...
  classical
  set F : Subgroup G := OddOrder.Isaacs.Ch01.fitting G with hF_def
  have hAC_le_cent : actionCommutator φ ≤ Subgroup.centralizer (F : Set G) := by
    ...
    exact hxconj.symm
  have hAC_le_F : actionCommutator φ ≤ F :=
    hAC_le_cent.trans (by
      rw [hF_def]
      exact centralizer_fitting_le_fitting)
  have htriv : ∀ a : A, ∀ h ∈ actionCommutator φ, (φ a) h = h := by
    intro a h hh
