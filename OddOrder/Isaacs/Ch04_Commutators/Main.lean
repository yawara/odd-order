...
/-! **Isaacs Lemma 4.6 後半 part 2** (range = commutator G): For `A ⊴ G` abelian,
`G ⧸ A` cyclic generated by `g · A`, `range(commutatorRightHom hAb g) = commutator G`.

`≤` direction: `commutatorRightHom_range_le_commutator` (上記, 自明).
`≥` direction: 未完. アイデア: `⁅⊤, ⊤⁆ = ⁅A ⊔ ⟨g⟩, A ⊔ ⟨g⟩⁆` を bilinearity で分解し
各部分 (`⁅A,A⁆=⊥`, `⁅⟨g⟩,⟨g⟩⁆=⊥`, `⁅A, ⟨g⟩⁆ ⊆ range`) を示す.
`⁅A, ⟨g⟩⁆ ⊆ range`: 任意 `a ∈ A, k ∈ ℤ` で `⁅a, g^k⁆ ∈ range` を `k`-induction で示す.
`⁅a, g^(k+1)⁆ = ⁅a, g⁆ · (g · ⁅a, g^k⁆ · g⁻¹)` (identity) で右側は range normal で OK. -/
