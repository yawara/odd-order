...
/-- **§7D Step 4 axiom** (Isaacs L4001-4019) — *`q`-central does not normalize a
`p`-subgroup generated by `p`-central elements*.

If `y` is `q`-central and normalizes a `p`-subgroup `V`, then `V` contains no
`p`-central element.  (Equivalently, the subgroup `V^⋆` generated by the
`p`-central elements of `V` is trivial.)

Textbook proof: choose `W` maximal among `p`-subgroups normalized by `y` and
generated by `p`-central elements; if `W > 1`, the `N_G(S) ⊄ N` argument with
`g ∈ G - N`, `S^g = S` produces a strictly larger such subgroup
`(W(W^b ∩ N))^⋆ > W`, contradicting maximality of `W`. -/
axiom step4_qCentral_normalizes_no_pCentral
    ...
    ∀ x : H, IsPCentral p x → x ∉ V

/-- **§7D Step 6 axiom** (Isaacs L4031-4037) — *a `q`-central element cannot
normalize a nontrivial `p`-subgroup*.

This is the symmetric capstone of Steps 4-5: combining "every `p`-subgroup is
centralized by a `p`-central element" (landed `exists_isPCentral_centralizing`)
with Step 4, no `q`-central element lies in `N_G(V)` for a nontrivial
`p`-subgroup `V`.  Proven in the textbook via Step 5 (a maximal `M ⊇ N_G(V)`
must be `p`-type, hence contains no `q`-central element). -/
axiom step6_qCentral_not_normalizes_nontrivial_pSubgroup
    {H : Type*} [Group H] [Finite H] [IsSimpleGroup H] {p q : ℕ}
    [Fact p.Prime] [Fact q.Prime] (hpq : p ≠ q)
    {a b : ℕ} (hH_card : Nat.card H = p ^ a * q ^ b)
    (hSubgroupsSolvable : ∀ K : Subgroup H, K ≠ ⊤ → IsSolvable K)
...
theorem step7_p_ne_two_and_q_ne_two
  ...
  have key : ∀ (r s : ℕ) [Fact r.Prime] [Fact s.Prime], r ≠ s →
    ...
    exact step6_qCentral_not_normalizes_nontrivial_pSubgroup (p := s) (q := 2)
      (Ne.symm hrs) hcard hSubgroupsSolvable ht_2central hX_ne_bot hX_pgroup ht_norm
  refine ⟨?_, ?_⟩
