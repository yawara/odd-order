...
`p`-central element; then **Step 4** (Isaacs L4001-4019: a `q`-central element
normalizing a `p`-subgroup `V` forces `V` to contain no `p`-central element —
proven via the `W`-maximality argument on `p`-central-generated subgroups) shows
no `q`-central element lies in `N_G(V₀) = M`.  Discharging this axiom therefore
also requires formalizing Step 4. -/
