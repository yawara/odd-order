  ...
  | succ n ih =>
    ...
    · -- L ⊄ C(P): C_L(P) = L ⊓ C_G(P) は L の真部分群. q | |L:C_L(P)| が取れる.
      ...
      have hPnorm_LP : P ≤ Subgroup.normalizer (LP_comm : Set _) := by
        -- P normalizes L (hypothesis); P normalizes P (trivially). So P normalizes ⁅L, P⁆.
        rw [hLP_comm_def]
        intro a ha
        rw [Subgroup.mem_normalizer_iff]
        intro y
        -- ⁅L, P⁆ is generated by ⁅l, p⁆. Conjugation by a maps generators to generators.
        sorry  -- TODO: 補助補題 (commutator subgroup P-invariance), 別 commit
      ...
      · -- Case [L, P] < L: IH 適用 → P ≤ C([L, P]) → Lem 4.29 で [L, P] = ⊥ → P ≤ C(L).
        ...
        -- Lem 4.29: coprime + solvable で `⁅⁅L, P⁆, P⁆ = ⁅L, P⁆`.
        -- P ≤ C([L, P]) ⇒ ⁅⁅L,P⁆, P⁆ = ⊥. 合わせて ⁅L,P⁆ = ⊥, 即ち P ≤ C(L).
        -- TODO: Lem 4.29 の Γ-semidirect 形式から G 上の subgroup commutator 形式への翻訳が必要.
        --   Ch.4 §4D の `iterCommutator_inl_inr_two_eq_one` + `actionCommutator_map_inl` から
        --   `comm_iterate_eq_comm_of_coprime_normalizer` (本ファイル内 helper or Ch.4 corollary) を
        --   別 commit で実装. それまでこの分岐は sorry.
        sorry
