...
/-- **§7D Step 5 (second half) axiom** (Isaacs L4027-4030) — *a `p`-type maximal
subgroup contains no `q`-central element*.

Stated symmetrically in the two primes `p, q` (the hypothesis `hH_card` is the
`p^a q^b` form; to apply it with the roles swapped, instantiate with `p := q`,
`q := p` and `hH_card` in `q^? p^?` form).

Textbook proof: for `M` `p`-type, `V₀ := O_p(M) > 1` with `O_q(M) = 1`.  By
Hall-Higman 1.2.3 (`hall_higman_opCore`), `C_M(V₀) ⊆ V₀`, so `V₀` contains a
`p`-central element; by Step 4, no `q`-central element lies in `N_G(V₀) = M`. -/
axiom step5b_pType_no_qCentral
    {H : Type*} [Group H] [Finite H] [IsSimpleGroup H] {p q : ℕ}
    [Fact p.Prime] [Fact q.Prime] (hpq : p ≠ q)
    {a b : ℕ} (hH_card : Nat.card H = p ^ a * q ^ b)
    (hH_nsol : ¬ IsSolvable H)
