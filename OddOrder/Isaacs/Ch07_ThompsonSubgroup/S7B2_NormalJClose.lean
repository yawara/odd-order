...
/-!
# Isaacs FGT Ch.7 — the normal-J theorem, closing steps (Thm 7.6, pp. 209-214)

Steps 4-8 of the normal-J argument and the unconditional `normal_J`: the Abar
centralizing bridges (Steps 3-5), the `|Ā| = p` count, the Step-8 action setup
`Ḡ →* MulAut V` with its kernel identification, and the final assembly.

Split from `OddOrder.Isaacs.Ch07_ThompsonSubgroup.S7B2_NormalJ_PComplement` (issue
0149, the longFile-1500 campaign); that file imports this leaf, so downstream imports
are unchanged.
-/
