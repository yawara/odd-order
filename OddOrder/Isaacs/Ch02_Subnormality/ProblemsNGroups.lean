...
`S` が可解なら (a) の `G ⧸ S` 可解と合わせて `G` が可解 (`solvable_of_ker_le_range`)。 -/
theorem not_isSolvable_of_isMinimalNormal_of_isNGroup [Finite G] (hG : IsNGroup G)
    (hns : ¬ IsSolvable G) {S : Subgroup G} (hS : IsMinimalNormal S) : ¬ IsSolvable ↥S := by
  haveI := hS.1
  intro h
  haveI := h
  haveI : IsSolvable (G ⧸ S) := isSolvable_quotient_of_isNGroup hG hS.2.1
  exact hns (solvable_of_ker_le_range S.subtype (QuotientGroup.mk' S)
    (by rw [QuotientGroup.ker_mk', Subgroup.range_subtype]))

/-- **Isaacs Problem 2C.1(b)** (存在). 非可解群は非自明ゆえ minimal normal subgroup をもつ。 -/
theorem exists_isMinimalNormal_of_not_isSolvable [Finite G] (hns : ¬ IsSolvable G) :
...
theorem isMinimalNormal_unique_of_isNGroup [Finite G] (hG : IsNGroup G)
    (hns : ¬ IsSolvable G) {S₁ S₂ : Subgroup G}
  ...
  have hinf : S₁ ⊓ S₂ = ⊥ := by
    ...
    · rcases h₂.2.2 S₁ h₁.1 (h ▸ inf_le_right) with h' | h'
      ...
      · exact absurd h' hne
  haveI : IsSolvable (G ⧸ S₁) := isSolvable_quotient_of_isNGroup hG h₁.2.1
  haveI : IsSolvable (G ⧸ S₂) := isSolvable_quotient_of_isNGroup hG h₂.2.1
  refine hns (solvable_of_solvable_injective
...
theorem isSimpleGroup_of_isMinimalNormal_of_isNGroup [Finite G] (hG : IsNGroup G)
    (hns : ¬ IsSolvable G) {S : Subgroup G} (hS : IsMinimalNormal S) : IsSimpleGroup ↥S := by
  classical
  haveI := hS.1
  have hSns : ¬ IsSolvable ↥S := not_isSolvable_of_isMinimalNormal_of_isNGroup hG hns hS
  haveI : Nontrivial ↥S := (Subgroup.nontrivial_iff_ne_bot S).mpr hS.2.1
  ...
  haveI : Nontrivial ↥T := (Subgroup.nontrivial_iff_ne_bot T).mpr hT.2.1
  ...
  haveI : IsSolvable ↥(Subgroup.normalizer (P : Set G)) := hG _ ⟨p, hp, P, hPne, hPpg, rfl⟩
  ...
  haveI hTsol : IsSolvable ↥T := hVsolT (by
    haveI : IsSolvable ↥(V.map S.subtype) := solvable_of_solvable_injective
  ...
  have hcomm_bot : ⁅T, T⁆ = ⊥ := by
    ...
    · exfalso
      ...
      exact absurd htop (IsSolvable.commutator_lt_top_of_nontrivial (G := ↥T)).ne
  haveI hTcomm : IsMulCommutative ↥T :=
    Subgroup.le_centralizer_iff_isMulCommutative.mp
      (Subgroup.commutator_eq_bot_iff_le_centralizer.mp hcomm_bot)
  haveI hTnilp : Group.IsNilpotent ↥T :=
...
theorem not_isMulCommutative_of_isMinimalNormal_of_isNGroup [Finite G] (hG : IsNGroup G)
    (hns : ¬ IsSolvable G) {S : Subgroup G} (hS : IsMinimalNormal S) :
    ¬ IsMulCommutative ↥S := fun h =>
  not_isSolvable_of_isMinimalNormal_of_isNGroup hG hns hS
    (isSolvable_of_comm fun a b => h.is_comm.comm a b)
