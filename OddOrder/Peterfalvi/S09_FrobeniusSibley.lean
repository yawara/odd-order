...
`S08.SibleyDadeHypothesis`.  Its Frobenius case (c1) is the intended Peterfalvi (7.10) producer.
This
leaf wires the `FrobeniusFamily` (7.10) hypothesis to a `SibleyDadeHypothesis`, unlocking `ν` for
the
...
`sharpImage ((H_i).subgroupOf L_i) = (H_i)^#`: since `H_i ≤ L_i`, mapping `(H_i).subgroupOf L_i`
back
through `L_i.subtype` recovers `H_i`, and `sharpImage`/`sharp` both remove the identity.  This
aligns
...
subgroups are trivial), and the kernel-nilpotency `hnilp` (available from
`frobeniusKernelIsNilpotent`
...
of the (6.8) `coherence`; it is the `ν` argument of `hypothesis78OfDade`, with `hnu_isometry`
supplied
