...
/-!
# Peterfalvi (10.10.3)–(10.10.4): type-V case (c) — column characters and the coherence glue
...
`S` in case (c) — their degree `μ_j(1) = d·w₁` ((10.10.2)), the numeric pins
`δ = −1`, `n = 2`, the (10.10.3) Dade-image identity
`α_{ij}^τ = δ(ω_{ij}^σ − ω_{i0}^σ) − n·ζ^{τ₁}` over the `S₁`-coherent `τ₁`, and the (10.10.4)
column identity `∑_i α_{ij} = μ_j − δ·μ_0 − (d−δ)·ζ` — the source-side algebra of the
coherent-extension glue `(μ_j − d·ζ)^τ = δ·(μ_0 − ζ)^τ + ∑_i α_{ij}^τ` — assembled into
`typeV_caseC_coherence_engine`.

The `p³` structural inputs of (10.10.2) (the `S₁`-count, `W₂ = H′ = M″`, the degree pin
`d = p`, the `hstruct` dichotomy) live downstream in `S12_TypeVCaseC`.
-/
