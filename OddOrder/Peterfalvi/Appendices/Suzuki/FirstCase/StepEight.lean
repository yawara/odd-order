...
/-- **Artin's degree theorem for a single automorphism** (step (8), p. 110): for
a ring automorphism `σ` of a finite field `F`, the order of `σ` is the degree of
`F` over its fixed field, so `|F| = |C_F(σ)|^{orderOf σ}`.

The cyclic group `⟨σ⟩ = zpowers σ` acts on `F` by ring automorphisms; its fixed
subfield is `C_F(σ) = {x : σ x = x}` and `FixedPoints.finrank_eq_card` gives
`[F : C_F(σ)] = |⟨σ⟩| = orderOf σ`. -/
theorem card_eq_card_fixedPoints_pow_orderOf {F : Type*} [Field F] [Finite F]
    (σ : F ≃+* F) :
  ...
  have hset : Nat.card (FixedPoints.subfield G F) = Nat.card {x : F // σ x = x} := by
    ...
    · intro hx g
      have hσstab : σ ∈ MulAction.stabilizer (RingAut F) x := hx
      have hle : Subgroup.zpowers σ ≤ MulAction.stabilizer (RingAut F) x :=
        (Subgroup.zpowers_le).mpr hσstab
      exact hle (hG ▸ g.2)
  rw [Nat.card_eq_fintype_card, hcard, hfr, hGcard, ← Nat.card_eq_fintype_card, hset]

/-- **The ring automorphism group of a finite field is cyclic** (step (8),
p. 110): `RingAut F ≅ Gal(F/𝔽_q)`, which is cyclic (generated by the Frobenius).

A ring automorphism of `F` fixes the prime field `𝔽_q = ZMod q`, so it is a
`ZMod q`-algebra automorphism; the forgetful map `RingAut F → (F ≃ₐ[ZMod q] F)`
...
theorem isCyclic_ringAut_of_charP {F : Type*} [Field F] [Finite F] {q : ℕ}
  ...
  have hφinj : Function.Injective φ := fun a b hab => by
    ext x; exact DFunLike.congr_fun hab x
  exact isCyclic_of_injective φ hφinj

/-- **A cyclic group whose nonidentity elements all have the same order has prime
order** (step (8), p. 110): if `G` is cyclic with `|G| > 1` and every `g ≠ 1` has
`orderOf g = m`, then `|G|` is prime.

The generator has order `|G|`, so `m = |G|`; hence every nonidentity element is a
generator, so `φ(|G|) = |G| - 1` (`IsCyclic.card_orderOf_eq_totient`), which
forces `|G|` prime (`Nat.totient_eq_iff_prime`). -/
theorem card_prime_of_isCyclic_forall_ne_one_orderOf {G : Type*} [Group G]
    [Fintype G] [IsCyclic G] {m : ℕ} (hcard : 1 < Fintype.card G)
    (h : ∀ g : G, g ≠ 1 → orderOf g = m) : (Fintype.card G).Prime := by
  classical
  set ℓ := Fintype.card G with hℓ
  obtain ⟨g₀, hg₀⟩ := IsCyclic.exists_generator (α := G)
  have hord0 : orderOf g₀ = ℓ := by
    rw [hℓ, ← Nat.card_eq_fintype_card]
    exact orderOf_eq_card_of_forall_mem_zpowers hg₀
  have hg0ne : g₀ ≠ 1 := fun h1 => by rw [h1, orderOf_one] at hord0; omega
  have hmℓ : m = ℓ := (h g₀ hg0ne).symm.trans hord0
  have hcount := IsCyclic.card_orderOf_eq_totient (α := G) (d := ℓ) dvd_rfl
  have hfilter : Finset.univ.filter (fun x : G => orderOf x = ℓ) = Finset.univ.erase 1 := by
    ext x
    simp only [Finset.mem_filter, Finset.mem_univ, true_and, Finset.mem_erase]
    constructor
    · intro hx
      exact ⟨fun h1 => by rw [h1, orderOf_one] at hx; omega, trivial⟩
    · rintro ⟨hx, -⟩; rw [← hmℓ]; exact h x hx
  rw [hfilter, Finset.card_erase_of_mem (Finset.mem_univ 1), Finset.card_univ,
    ← hℓ] at hcount
  exact (Nat.totient_eq_iff_prime (by omega : 0 < ℓ)).mp hcount.symm
...
/-- **Peterfalvi Part II, Ch. II, step (8)** (p. 110): assume `Q₁ ≠ 1` and let
`ℓ = |Σ| = |C_W(P)|`.  If `ℓ ≠ 1`, then `ℓ` is prime and `F` is a field of order
`3^ℓ`, `5^ℓ`, or `9^ℓ`.

Each nonidentity `w ∈ C_W(P)` acts on `F` as `σ_w = dAut [w]`, and
`ψ : C_W(P) → RingAut F` is an injective homomorphism (the composite of
`C_W(P) ≅ Σ` and `dAut`).  By Artin, `|F| = |C_F(w)|^{orderOf σ_w}`, and
`|C_F(w)| = f^{a_w}` with `a_w ∈ {1,2}` (step (C)); since `orderOf σ_w = orderOf w`
is odd (`Σ` has odd order), the parity of the fixed exponent `a_w · orderOf σ_w`
determines `a_w`, so `|C_F(w)|` is independent of `w`.  Hence `orderOf σ_w` is
constant, `RingAut F` is cyclic (finite field), so `C_W(P)` is cyclic with all
...
theorem card_prime_and_card_field_of_Q1_ne_bot
    ...
    (model : letI := fc.toHypothesis.centralizerQuotientMulAction fc.P_le_V
      NearFields.AffineNearFieldModel fc.rankOneQuotient F)
    (hQ1 : fc.toHypothesis.Q1 ≠ ⊥)
    (hℓ : Nat.card ↥(fc.toHypothesis.W ⊓ Subgroup.centralizer (fc.P : Set G)) ≠ 1) :
    letI := fc.toHypothesis.centralizerQuotientMulAction fc.P_le_V
    (Nat.card ↥(fc.toHypothesis.W ⊓ Subgroup.centralizer (fc.P : Set G))).Prime ∧
      (Nat.card F =
          3 ^ Nat.card ↥(fc.toHypothesis.W ⊓ Subgroup.centralizer (fc.P : Set G)) ∨
        Nat.card F =
  ...
  -- `ψ`'s `C_W(P) → Σ` part, `w ↦ [w]`
  have hmem : ∀ w : ↥CW,
      ((QuotientGroup.mk' N).comp (Subgroup.inclusion hCWL)) w ∈ fc.rankOneQuotient.D :=
    fun w => Subgroup.mem_map_of_mem _ (Subgroup.mem_subgroupOf.mpr
      (fc.toHypothesis.V_le_D (fc.toHypothesis.W_le_V w.2.1)))
  set f : ↥CW →* ↥fc.rankOneQuotient.D :=
    ((QuotientGroup.mk' N).comp (Subgroup.inclusion hCWL)).codRestrict
      fc.rankOneQuotient.D hmem with hfdef
  -- `f w = [w]` (the step (C) element)
  have hfw : ∀ w : ↥CW, f w = fc.sigmaElt w.2.1 w.2.2 := fun w => rfl
  -- `f` injective (kernel meets `W ∩ N = 1`)
  have hNW : fc.kernelN ⊓ fc.toHypothesis.W = ⊥ := fc.kernelN_inf_W_eq_bot ind
  have hfinj : Function.Injective f := by
    intro a b hab
    have h1 : QuotientGroup.mk' N (⟨(a : G), hCWL a.2⟩ : ↥L) =
        QuotientGroup.mk' N ⟨(b : G), hCWL b.2⟩ := congrArg Subtype.val hab
  ...
  -- `dAutHom` injective
  have hdauinj : Function.Injective (fc.dAutHom hcomm model) := fun a b hab =>
    model.dAut_injective (AddEquiv.ext (fun x => DFunLike.congr_fun hab x))
  -- `ψ = dAut ∘ [·]`, injective
  ...
  have hgne : g ≠ 1 := fun h1 => by rw [h1, orderOf_one] at hgord; omega
  have hgG : (g : G) ≠ 1 := fun h => hgne (Subtype.ext h)
  -- `char = |st| ∈ {3, 5}`
  have hchar := fc.orderOf_st_eq_char model
  have hcharval : model.char = 3 ∨ model.char = 5 := by
    have hst := (fc.st_mem_and_cQ_isPGroup_of_mem_centralizer_W ind g.2.1 g.2.2 hgG).1
    rcases hst with h | h
    · exact Or.inl (hchar.symm.trans h)
    · exact Or.inr (hchar.symm.trans h)
  -- `Σ = C_W(P)` has odd order, so every element has odd order
  have hCWodd : Odd (Nat.card ↥CW) := by
    obtain ⟨e⟩ := fc.sigma_mulEquiv_centralizer_W ind
    rw [← Nat.card_congr e.toEquiv]
    exact fc.rankOneQuotient.D_odd
  have hoddw : ∀ w : ↥CW, Odd (orderOf w) := fun w => by
    obtain ⟨c, hc⟩ := orderOf_dvd_natCard w
    exact (Nat.odd_mul.mp (hc ▸ hCWodd)).1
  -- per-nonidentity-`w` facts
  have hArtin : ∀ w : ↥CW,
      Nat.card F = Nat.card {x : F // ψ w x = x} ^ orderOf (ψ w) :=
    fun w => card_eq_card_fixedPoints_pow_orderOf (ψ w)
  have hordw : ∀ w : ↥CW, orderOf (ψ w) = orderOf w :=
    fun w => orderOf_injective ψ hψinj w
  have hpsiC : ∀ w : ↥CW, w ≠ 1 →
      Nat.card {x : F // ψ w x = x} = model.char ∨
      Nat.card {x : F // ψ w x = x} = 9 := by
    intro w hw
    exact fc.cardFixedField_char_or_nine ind model hQ1 w.2.1 w.2.2
      (fun h => hw (Subtype.ext h))
  -- `|C_F(ψ w)| = char^a` with `a ∈ {1,2}` and `|F| = char^(a · orderOf (ψ w))`
  have hkey : ∀ w : ↥CW, w ≠ 1 → ∃ a : ℕ, (a = 1 ∨ a = 2) ∧
      Nat.card {x : F // ψ w x = x} = model.char ^ a ∧
      Nat.card F = model.char ^ (a * orderOf (ψ w)) := by
    intro w hw
    obtain ⟨a, ha⟩ := exists_card_fixedSet_eq_char_pow model.char_prime (ψ w)
    have ha12 : a = 1 ∨ a = 2 := by
      rcases hpsiC w hw with hc | hc
      · left
        exact Nat.pow_right_injective (by omega)
          (by rw [← ha, hc, pow_one] : model.char ^ a = model.char ^ 1)
      · rcases hcharval with h3 | h5
        · right
          have he : model.char ^ a = model.char ^ 2 := by
            rw [← ha, hc, h3]; norm_num
          exact Nat.pow_right_injective (by omega) he
        · exfalso
          have h95 : (9 : ℕ) = 5 ^ a := by rw [← hc, ha, h5]
          rcases Nat.lt_or_ge a 2 with hlt | hge
          · interval_cases a <;> norm_num at h95
          · have h25 : 5 ^ 2 ≤ 5 ^ a := Nat.pow_le_pow_right (by norm_num) hge
            norm_num at h25; omega
    exact ⟨a, ha12, ha, by rw [hArtin w, ha, ← pow_mul]⟩
  -- `|C_F(ψ w)|` is independent of the nonidentity `w`
  have hconst : ∀ w : ↥CW, w ≠ 1 →
      Nat.card {x : F // ψ w x = x} = Nat.card {x : F // ψ g x = x} := by
    intro w hw
    obtain ⟨aw, haw12, hawc, hawF⟩ := hkey w hw
    obtain ⟨ag, hag12, hagc, hagF⟩ := hkey g hgne
    have hNeq : aw * orderOf (ψ w) = ag * orderOf (ψ g) :=
      Nat.pow_right_injective (by omega) (hawF.symm.trans hagF)
    have haeq : aw = ag := by
      have hw2 : orderOf (ψ w) % 2 = 1 := Nat.odd_iff.mp (hordw w ▸ hoddw w)
      have hg2 : orderOf (ψ g) % 2 = 1 := Nat.odd_iff.mp (hordw g ▸ hoddw g)
      rcases haw12 with rfl | rfl <;> rcases hag12 with rfl | rfl <;> omega
    rw [hawc, hagc, haeq]
  -- base of the power is `> 1`
  have hbase : 1 < Nat.card {x : F // ψ g x = x} := by
    rcases hpsiC g hgne with hc | hc
    · rw [hc]; rcases hcharval with h | h <;> omega
    · rw [hc]; norm_num
  -- every nonidentity `w` has `orderOf w = orderOf g = ℓ`
  have hallord : ∀ w : ↥CW, w ≠ 1 → orderOf w = orderOf g := by
    intro w hw
    have h1 : Nat.card {x : F // ψ g x = x} ^ orderOf (ψ w) =
        Nat.card {x : F // ψ g x = x} ^ orderOf (ψ g) := by
      rw [← hArtin g, hArtin w, hconst w hw]
    have : orderOf (ψ w) = orderOf (ψ g) := Nat.pow_right_injective hbase h1
    rw [← hordw w, ← hordw g]; exact this
  -- `ℓ` is prime
  have hℓprime : (Nat.card ↥CW).Prime := by
    rw [Nat.card_eq_fintype_card]
    exact card_prime_of_isCyclic_forall_ne_one_orderOf
      (by rw [← Nat.card_eq_fintype_card]; exact hℓpos) hallord
  -- `|F| = |C_F(ψ g)|^ℓ`
  have hgcard : Nat.card F = Nat.card {x : F // ψ g x = x} ^ Nat.card ↥CW := by
    rw [hArtin g, hordw g, hgord]
  refine ⟨hℓprime, ?_⟩
  rcases hpsiC g hgne with hc | hc
  · rcases hcharval with h | h
    · exact Or.inl (by rw [hgcard, hc, h])
    · exact Or.inr (Or.inl (by rw [hgcard, hc, h]))
  · exact Or.inr (Or.inr (by rw [hgcard, hc]))
