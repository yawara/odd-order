...
/-!
# Peterfalvi Appendix IV: coherence of `𝒮(Q')` (pp. 145–146)

T. Peterfalvi, *Character Theory for the Odd Order Theorem* (LMS LNS 272, 2000),
Appendix IV, pp. 145–146.  Prefix-split from
`OddOrder.Peterfalvi.Appendices.FeitSibleyTheorem` (longFile 1500, issue 0149):
the derived subgroups `S' = [S,S]`, `Q' = [Q,Q]` of the "Hypotheses and
Notation" block (p. 145) with their elementary facts, the `H = Q ⋊ D` counting
(`[H : Q] = d`), the degree-`d` Remark on `𝒮(Q')` and its coherence
(`ssetOf_Qder_coherent`), and the degree-square bound of step (1)–(2)
(`exists_deg_sq_le_of_mem_SsetOf`).
-/
