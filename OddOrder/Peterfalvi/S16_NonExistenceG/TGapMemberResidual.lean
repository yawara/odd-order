...
The member-level (11.8) core itself lives upstream in §11 = S13
(`S13.zeta_residual_not_orthogonal_H0C_of_refuter`, the book's `∀ ζ` form);
this leaf keeps only the S16-specific transport layers (specified-`TypePData`
carrier, arbitrary-σ column identity, transposed-grid reindexing).  The former
member-pinned duplicates (`exists_charParameters_full_for_member`,
`exists_s13Hypothesis_for_member`, `member_residual_not_orthogonal_H0C_of_refuter`)
were removed in issue 1040 — their statements are subsumed by
`S12.Hypothesis.exists_charParameters_full_of_zeta` and the S13 theorem.
-/
...
theorem member_residual_not_orthogonal_of_transposed_alignment [Finite G]
    ...
    ¬ ∀ (i : Fin q) (j : Fin p),
      ClassFunction.inner
        ...
        (eta i j) = 0 := by
  have hnot := OddOrder.Peterfalvi.S13.zeta_residual_not_orthogonal_H0C_of_refuter
    hG hyp htype hM2 hHcard hrefute hzetaS hzetairr hzeta1
  intro horth
  apply hnot
  intro i j
  have hsum :
