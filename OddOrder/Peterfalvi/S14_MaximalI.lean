...
/-- **Peterfalvi (1.7)(b) precondition: the inertia quotient `I(θ)/H` is abelian** for a type-`F`
group.  Building on `typeF_inertia_inf_U_isMulCommutative` (the inertia complement `I(θ) ∩ U` is
abelian) and the coprime factorization `Γ = H U` (`H ⊔ U = ⊤`, `H ⊴ Γ`): since `H ≤ I(θ)`
(`ClassFunction.subgroup_le_inertia`), the Dedekind modular law factors every `s ∈ I(θ)` as
`s = h · u` with `h ∈ H` and `u = h⁻¹ s ∈ I(θ) ∩ U`.  Passing to `Γ ⧸ H`, the images of any two
`s, t ∈ I(θ)` are images of `u, u' ∈ I(θ) ∩ U`, which commute (`I(θ) ∩ U` abelian), so
`⁅s, t⁆ ∈ H`.  Equivalently `⁅I(θ), I(θ)⁆ ≤ H`, i.e. the inertia quotient `I(θ)/H`
(`ClassFunction.inertiaQuotient`) is abelian.

This is exactly the abelian-inertia-quotient hypothesis of Peterfalvi (1.7)(b) that makes
`Ind_H^L θ` multiplicity-free with equal-degree constituents (per PFsection1
`cfInd_central_Inertia`, `abelian` not `cyclic` suffices), feeding the general type-I
`typeI_induced_char_constituents`. -/
theorem typeF_inertia_commutator_le {Γ : Type*} [Group Γ] [Finite Γ]
    {H U U1 : Subgroup Γ} [H.Normal]
    (hHU : H ⊔ U = ⊤)
    (hUHcop : Nat.Coprime (Nat.card ↥U) (Nat.card ↥H))
    (hcent : ∀ x ∈ H, x ≠ 1 → U ⊓ Subgroup.centralizer ({x} : Set Γ) ≤ U1)
    (hU1comm : IsMulCommutative ↥U1)
    (θ : IrreducibleCharacter ↥H) (hθ : θ ≠ trivialIrreducibleCharacter ↥H) :
    ⁅ClassFunction.inertia (θ : ClassFunction ↥H ℂ),
        ClassFunction.inertia (θ : ClassFunction ↥H ℂ)⁆ ≤ H := by
  set T := ClassFunction.inertia (θ : ClassFunction ↥H ℂ) with hTdef
  have hHT : H ≤ T := ClassFunction.subgroup_le_inertia (θ : ClassFunction ↥H ℂ)
  have hTUcomm : IsMulCommutative ↥(T ⊓ U) :=
    typeF_inertia_inf_U_isMulCommutative hUHcop hcent hU1comm θ hθ
  ...
  have hcommU : u₁ * u₂ = u₂ * u₁ := by
    have hc := (isMulCommutative_iff.mp hTUcomm) ⟨u₁, hu₁⟩ ⟨u₂, hu₂⟩
  ...
  have hq : (QuotientGroup.mk' H) ⁅s, t⁆ = 1 := by
    ...
    exact hcm
  have hmem : ⁅s, t⁆ ∈ (QuotientGroup.mk' H).ker := MonoidHom.mem_ker.mpr hq
