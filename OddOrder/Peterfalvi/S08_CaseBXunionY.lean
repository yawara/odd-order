...
/-- **(6.8.2.3) the anchored column difference `μ_j − a·η₁` is `H^#`-supported.**

For a nontrivial column `μ_j = columnSum h46 χ₂` (`χ₂ ≠ 1`) and a `Y`-anchor `η₁ ∈ Y` with the
degree match `μ_j(1) = a·η₁(1)`, the difference `μ_j − a·η₁` vanishes off `A = H^#`:

* at `1` it vanishes by the degree match (`μ_j(1) − a·η₁(1) = a·η₁(1) − a·η₁(1) = 0`);
* `μ_j` vanishes off `A ∪ {1}` (Peterfalvi (4.7), `columnSum_support_subset`);
* `η₁ ∈ S` is `η₁ = Ind_H^L θ` (`S_eq`), so it vanishes off `H` (`support_induce_subset_of_normal`),
  and off `A ∪ {1} = H` together with `g ≠ 1` lands in `A = H^#`.

This is the column instance of the `hSdiff`/`hμη₁supp` (5.4)-supportedness input of the per-`φ`
column-branch decomposition `columnDecompositionTau` (S08_CaseBCoherence2), one of the two mechanical
`(6.8.2.3)` inputs the assembly plan flagged as "to construct" (the other being `tau_columnSum_sub_smul_mem_ZIrr`).
The conjugate-difference half `μ_j − μ̄_j` of `hSdiff` is the existing `columnDiff_support_subset`. -/
theorem columnSum_sub_smul_Yset_support
    (hyp : SibleyDadeHypothesis G L H) [H.Normal]
    (h46 : OddOrder.Peterfalvi.S06.Hypothesis46 (sharpImage H) L)
    [NeZero (Nat.card h46.W1)] [Invertible (Nat.card ↥h46.K : ℂ)]
    [Fintype ↥(h46.W1 ⊔ h46.W2)] [Invertible (Nat.card ↥(h46.W1 ⊔ h46.W2) : ℂ)]
    ...
    (ha : (OddOrder.Peterfalvi.S06.columnSum h46 χ₂ : ClassFunction ↥L ℂ) 1
      = (a : ℂ) * η₁ 1) :
    (OddOrder.Peterfalvi.S06.columnSum h46 χ₂ - a • η₁).support ⊆
      OddOrder.Peterfalvi.S04.supportInSubgroup (sharpImage H) L := by
  classical
  -- value at `1` is `0` (degree match `ha`), so `1` is not in the support.
  have hzero : (OddOrder.Peterfalvi.S06.columnSum h46 χ₂ - a • η₁ : ClassFunction ↥L ℂ) 1 = 0 := by
    have hsm : (a • η₁ : ClassFunction ↥L ℂ) 1 = (a : ℂ) * η₁ 1 := by
      rw [← Nat.cast_smul_eq_nsmul ℂ a η₁, ClassFunction.smul_apply]
    rw [ClassFunction.sub_apply, ha, hsm, sub_self]
  -- `η₁ = Ind_H^L θ ∈ S` has support in the normal `H`.
  have hη₁S : η₁ ∈ hyp.S := hyp.Yset_subset_S hη₁
  rw [hyp.S_eq] at hη₁S
  obtain ⟨θ, -, hη₁eq⟩ := hη₁S
  have hηsupp : η₁.support ⊆ (H : Set ↥L) := by
    rw [hη₁eq]
  ...
  have hg1 : g ≠ 1 := by
    ...
    exact hg hzero
  rcases ClassFunction.support_sub_subset _ _ hg with hgμ | hgη
  · -- column support ⊆ `supportInSubgroup ∪ {1}`; `g ≠ 1` ⟹ in `supportInSubgroup`.
    rcases OddOrder.Peterfalvi.S06.columnSum_support_subset h46 hχ₂ hgμ with hgA | hg1'
    · exact hgA
    · exact absurd (Set.mem_singleton_iff.mp hg1') hg1
  · -- `g ∈ (a•η₁).support ⊆ η₁.support ⊆ H`, and `g ≠ 1` ⟹ in `sharpImage H`.
    rw [← Nat.cast_smul_eq_nsmul ℂ a η₁] at hgη
    have hgH : g ∈ H := hηsupp (ClassFunction.support_smul_subset _ _ hgη)
...
/-- **(6.8.2.3) the anchored column image `(μ_j − a·η₁)^{hyp.tau}` is a virtual character of `G`.**

The Sibley–Dade map `hyp.tau = dadeIntegralCharacterMap hyp.dade …` sends a *supported* virtual
character of `L` to a virtual character of `G` (`dadeIntegralCharacterMap_mem_ZIrr_of_supported`).
The anchored column difference `μ_j − a·η₁` is supported on `H^#` (`columnSum_sub_smul_Yset_support`)
and is a virtual character of `L` (`μ_j = ∑_i μ_{ij}` a sum of irreducibles, `η₁` irreducible).

This is the `htau1_mema` input of the per-`φ` column-branch decomposition `columnDecompositionTau`
(S08_CaseBCoherence2), the second of the two mechanical `(6.8.2.3)` inputs the assembly plan flagged
as "to construct". -/
theorem tau_columnSum_sub_smul_mem_ZIrr
    (hyp : SibleyDadeHypothesis G L H) [H.Normal]
    (h46 : OddOrder.Peterfalvi.S06.Hypothesis46 (sharpImage H) L)
    [NeZero (Nat.card h46.W1)] [Invertible (Nat.card ↥h46.K : ℂ)]
    [Fintype ↥(h46.W1 ⊔ h46.W2)] [Invertible (Nat.card ↥(h46.W1 ⊔ h46.W2) : ℂ)]
  ...
  classical
  have hZ : (OddOrder.Peterfalvi.S06.columnSum h46 χ₂ - a • η₁ : ClassFunction ↥L ℂ) ∈ ZIrr (↥L) := by
    refine Submodule.sub_mem _ ?_ (nsmul_mem ?_ a)
    · rw [OddOrder.Peterfalvi.S06.columnSum_def]
      exact Submodule.sum_mem _ fun i _ => IrreducibleCharacter.mem_ZIrr _
    · exact IsIrreducibleCharacter.mem_ZIrr (hyp.isIrreducibleCharacter_of_mem_Yset hη₁)
  exact OddOrder.Peterfalvi.S07.dadeIntegralCharacterMap_mem_ZIrr_of_supported hyp.dade hyp.hconj
    (columnSum_sub_smul_Yset_support hyp h46 hχ₂ hη₁ ha) hZ
