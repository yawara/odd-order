    ...
    · -- **`two_le`** (discharged): at least two conjugacy classes of maximal subgroups (7.10).
      ...
      · -- one class: `⋃ᵢ thickenedA1(Mᵢ) = thickenedA1(M)`, of cardinality `< |G#|`.
        ...
        have hcard_eq : Nat.card ↥(data.cover i₀) = Nat.card G - 1 := by
          ...
          change ((↑(⊤ : Subgroup G) : Set G) \ {1}).ncard = Nat.card G - 1
...
theorem not_all_maximal_typeI (hG : OddOrder.BG.IsMinimalSimpleOdd G)
  ...
  -- Kernel nilpotence is no longer supplied here: (7.11) derives it from condition (7.10)(a)
  -- via Thompson's theorem (`FrobeniusFamily.isNilpotent_kernel`), as the book does.
...
`P₁`, which is Type V (`M_F = M_σ`) or Type III/IV (`M_F ≠ M_σ`); unequal gives `P₂` = Type II.
This
