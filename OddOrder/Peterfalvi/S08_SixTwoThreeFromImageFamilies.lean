...
/-! ### The (5.3.a) signed pair as an orthonormal image family

Peterfalvi's (5.2.d) datum `R(χ)` appears in two shapes here: the **signed pair**
`CharacterDifferenceImage` (`τ(χ − χ̄) = ε·(μ − ν)` with `μ ≠ ν` irreducible and `ε = ±1`, the
(5.3.a) conclusion) and the **finite orthonormal family** `OrthonormalCharacterImageFamily`
(`τ(χ − χ̄) = ∑_{α ∈ R(χ)} α`, the shape the adjoining engines consume).  The bridge below is
`R(χ) = {ε·μ, −ε·ν}`.

Its point is that (5.2.d)/(5.2.e) become **derivable** rather than assumed for a subfamily of
*irreducible* members: `characterDifferenceImage_of_irreducible` (`S08_SixFiveGeneral`) already
builds the signed pair out of the (5.2.b) isometry alone (issue 0156). -/

section SignedPairFamily

variable {L G : Type*} [Group L] [Group G] [Fintype G] [Invertible (Nat.card G : ℂ)]
variable {τ : IntegralCharacterMap L G} {χ : ClassFunction L ℂ}

/-- The first member `ε·μ` of the orthonormal form of a (5.3.a) signed pair. -/
noncomputable def CharacterDifferenceImage.orthoFst
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) : ClassFunction G ℂ :=
  R.sign • (R.mu : ClassFunction G ℂ)

/-- The second member `−ε·ν` of the orthonormal form of a (5.3.a) signed pair. -/
noncomputable def CharacterDifferenceImage.orthoSnd
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) : ClassFunction G ℂ :=
  -(R.sign • (R.nu : ClassFunction G ℂ))

theorem CharacterDifferenceImage.inner_orthoFst_self
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) :
    ClassFunction.inner R.orthoFst R.orthoFst = 1 := by
  rw [orthoFst, inner_zsmul_irreducible_eq, if_pos rfl]
  rcases R.sign_eq with h | h <;> rw [h] <;> norm_num

theorem CharacterDifferenceImage.inner_orthoSnd_self
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) :
    ClassFunction.inner R.orthoSnd R.orthoSnd = 1 := by
  rw [orthoSnd, ClassFunction.inner_neg_left, ClassFunction.inner_neg_right, neg_neg,
    inner_zsmul_irreducible_eq, if_pos rfl]
  rcases R.sign_eq with h | h <;> rw [h] <;> norm_num

theorem CharacterDifferenceImage.inner_orthoFst_orthoSnd
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) :
    ClassFunction.inner R.orthoFst R.orthoSnd = 0 := by
  rw [orthoFst, orthoSnd, ClassFunction.inner_neg_right, inner_zsmul_irreducible_eq,
    if_neg R.distinct]
  ring

theorem CharacterDifferenceImage.inner_orthoSnd_orthoFst
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) :
    ClassFunction.inner R.orthoSnd R.orthoFst = 0 := by
  rw [orthoFst, orthoSnd, ClassFunction.inner_neg_left, inner_zsmul_irreducible_eq,
    if_neg (Ne.symm R.distinct)]
  ring

omit [Fintype G] in
/-- The two members are distinct — otherwise their inner product would be both `0` and `1`.
(`Finite G` suffices: the finiteness is used only in the proof, not in the statement.) -/
theorem CharacterDifferenceImage.orthoFst_ne_orthoSnd [Finite G]
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) : R.orthoFst ≠ R.orthoSnd := by
  haveI := Fintype.ofFinite G
  intro h
  have h0 := R.inner_orthoFst_orthoSnd
  rw [← h, R.inner_orthoFst_self] at h0
  exact one_ne_zero h0

omit [Fintype G] [Invertible (Nat.card G : ℂ)] in
/-- `ε·μ + (−ε·ν) = ε·(μ − ν) = τ(χ − χ̄)`. -/
theorem CharacterDifferenceImage.orthoFst_add_orthoSnd
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) :
    R.orthoFst + R.orthoSnd = τ (χ - χ.conj) := by
  rw [orthoFst, orthoSnd, R.image_eq, smul_sub]
  abel

open scoped Classical in
/-- **The (5.3.a) signed pair `τ(χ − χ̄) = ε·(μ − ν)` as an orthonormal image family**
`R(χ) = {ε·μ, −ε·ν}`.

Orthonormality is `inner_zsmul_irreducible_eq` together with `μ ≠ ν` and `ε² = 1`; the sum
telescopes to `ε·(μ − ν)` by `image_eq`.  This is the converter that lets a family of irreducible
members satisfy (5.2.d) with no data beyond the (5.2.b) isometry (issue 0156). -/
noncomputable def CharacterDifferenceImage.toOrthonormalFamily
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) :
    OrthonormalCharacterImageFamily (L := L) (G := G) τ χ where
  imageSet := {R.orthoFst, R.orthoSnd}
  mem_ZIrr := by
    intro α hα
    simp only [Finset.mem_insert, Finset.mem_singleton] at hα
    rcases hα with rfl | rfl
    · exact zsmul_mem R.mu.2.mem_ZIrr _
    · exact neg_mem (zsmul_mem R.nu.2.mem_ZIrr _)
  orthonormal := by
    intro α hα β hβ
    simp only [Finset.mem_insert, Finset.mem_singleton] at hα hβ
    rcases hα with rfl | rfl <;> rcases hβ with rfl | rfl
    · rw [if_pos rfl]; exact R.inner_orthoFst_self
    · rw [if_neg R.orthoFst_ne_orthoSnd]; exact R.inner_orthoFst_orthoSnd
    · rw [if_neg (Ne.symm R.orthoFst_ne_orthoSnd)]; exact R.inner_orthoSnd_orthoFst
    · rw [if_pos rfl]; exact R.inner_orthoSnd_self
  image_eq := by
    rw [Finset.sum_pair R.orthoFst_ne_orthoSnd]
    exact R.orthoFst_add_orthoSnd.symm

@[simp] theorem CharacterDifferenceImage.toOrthonormalFamily_imageSet
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ) :
    letI := Classical.decEq (ClassFunction G ℂ)
    R.toOrthonormalFamily.imageSet = {R.orthoFst, R.orthoSnd} := rfl

/-- **(5.2.e) transports across the converter.**  The orthonormal members are `±ε` multiples of
the `μ`, `ν` of the signed pairs, so `CharacterDifferenceImage.Orthogonal` — which says the four
irreducibles share nothing — gives orthogonality of the converted families verbatim. -/
theorem CharacterDifferenceImage.toOrthonormalFamily_orthogonal {ψ : ClassFunction L ℂ}
    (R : CharacterDifferenceImage (L := L) (G := G) τ χ)
    (R' : CharacterDifferenceImage (L := L) (G := G) τ ψ) (h : R.Orthogonal R') :
    R.toOrthonormalFamily.Orthogonal R'.toOrthonormalFamily := by
  classical
  have hmn : ∀ (a b : IrreducibleCharacter G), (a : ClassFunction G ℂ) ∈ R.imageSet →
      (b : ClassFunction G ℂ) ∈ R'.imageSet → ∀ m n : ℤ,
      ClassFunction.inner (m • (a : ClassFunction G ℂ)) (n • (b : ClassFunction G ℂ)) = 0 := by
    intro a b ha hb m n
    rw [inner_zsmul_irreducible_eq]
    have hne : a ≠ b := by
      intro hab
      have := h ha hb
      rw [hab, irreducibleCharacter_inner_eq_ite, if_pos rfl] at this
      exact one_ne_zero this
    rw [if_neg hne, mul_zero]
  intro α hα β hβ
  simp only [toOrthonormalFamily_imageSet, Finset.mem_insert, Finset.mem_singleton] at hα hβ
  have hRmu := R.muClassFunction_mem_imageSet
  have hRnu := R.nuClassFunction_mem_imageSet
  have hR'mu := R'.muClassFunction_mem_imageSet
  have hR'nu := R'.nuClassFunction_mem_imageSet
  rcases hα with rfl | rfl <;> rcases hβ with rfl | rfl <;>
    simp only [orthoFst, orthoSnd, ClassFunction.inner_neg_left, ClassFunction.inner_neg_right]
  · exact hmn R.mu R'.mu hRmu hR'mu _ _
  · rw [hmn R.mu R'.nu hRmu hR'nu _ _]; ring
  · rw [hmn R.nu R'.mu hRnu hR'mu _ _]; ring
  · rw [hmn R.nu R'.nu hRnu hR'nu _ _]; ring

end SignedPairFamily
