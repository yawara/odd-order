...
/-- **`hgen` from the difference supports alone** (issue 1050; the rational-ratio
replacement for `zSupportedSpan_adjoinPair_subset_span_of_anchorGeneration`).
The `A`-supported part of `ℤ[S₁ ∪ {χ, χ̄}]` is generated by `ℤ[S₁, A]` together
with the two adjoined differences, provided both differences are themselves
`A`-supported.  No lattice generation `ℤ[S₁] ≤ ℤ[ℤ[S₁,A] ∪ {χ₁}]` is needed
(that inclusion fails when member/anchor degree ratios are non-integral): write
`φ = p·χ + (q·χ̄ + ξ₀)` along `ℤ[S₁ ∪ {χ,χ̄}] = ℤχ + ℤχ̄ + ℤ[S₁]`, regroup as
`φ = (p+q)·(χ − a·χ₁) − q·(χ − χ̄) + ξ` with `ξ := ξ₀ + (p+q)·a·χ₁ ∈ ℤ[S₁]`, and
read off `ξ.support ⊆ A` from the three `A`-supported summands. -/
theorem zSupportedSpan_adjoinPair_subset_span_of_diffSupports
    {S₁ : Set (ClassFunction L ℂ)} {A : Set L}
    {χ chibar chi1 : ClassFunction L ℂ} {a : ℕ}
    (hchi1S₁ : chi1 ∈ S₁)
    (hdiffconj : (χ - chibar).support ⊆ A)
    (hdiffratio : (χ - a • chi1).support ⊆ A) :
    zSupportedSpan (L := L) (S₁ ∪ {χ, chibar}) A ⊆
      Submodule.span ℤ (zSupportedSpan (L := L) S₁ A ∪ {χ - chibar, χ - a • chi1}) := by
  have hzsmul_supp : ∀ (n : ℤ) (g : ClassFunction L ℂ), (n • g).support ⊆ g.support := by
    intro n g
    rw [← Int.cast_smul_eq_zsmul ℂ n g]
    exact ClassFunction.support_smul_subset _ _
  intro φ hφ
  obtain ⟨hφspan, hφsupp⟩ := hφ
  have hset : S₁ ∪ {χ, chibar} = insert χ (insert chibar S₁) := by
    ext y
    simp only [Set.mem_union, Set.mem_insert_iff, Set.mem_singleton_iff]
    tauto
  rw [show zSpan (L := L) (S₁ ∪ {χ, chibar})
    = Submodule.span ℤ (insert χ (insert chibar S₁)) from by rw [zSpan, hset]] at hφspan
  obtain ⟨p, φ₁, hφ₁, rfl⟩ := Submodule.mem_span_insert.mp hφspan
  obtain ⟨q, ξ₀, hξ₀, rfl⟩ := Submodule.mem_span_insert.mp hφ₁
  -- Regroup around the two adjoined differences.
  set ξ : ClassFunction L ℂ := ξ₀ + ((p + q) * (a : ℤ)) • chi1 with hξdef
  have hξS₁ : ξ ∈ Submodule.span ℤ S₁ :=
    Submodule.add_mem _ hξ₀ (Submodule.smul_mem _ _ (Submodule.subset_span hchi1S₁))
  have hφeq : p • χ + (q • chibar + ξ₀)
      = (p + q) • (χ - a • chi1) - q • (χ - chibar) + ξ := by
    rw [hξdef, ← Nat.cast_smul_eq_nsmul ℤ a chi1]
    module
  have hξsupp : ξ.support ⊆ A := by
    have hξeq2 : ξ = (p • χ + (q • chibar + ξ₀))
        - (p + q) • (χ - a • chi1) + q • (χ - chibar) := by
      rw [hφeq]; abel
    rw [hξeq2]
    intro x hx
    rcases ClassFunction.support_add_subset _ _ hx with hx1 | hx2
    · rcases ClassFunction.support_sub_subset _ _ hx1 with hxφ | hxu
      · exact hφsupp hxφ
      · exact hdiffratio (hzsmul_supp _ _ hxu)
    · exact hdiffconj (hzsmul_supp _ _ hx2)
  rw [hφeq]
  refine Submodule.add_mem _ (Submodule.sub_mem _ ?_ ?_)
    (Submodule.subset_span (Set.mem_union_left _ ⟨hξS₁, hξsupp⟩))
...
omit [Finite L] [Fintype L] [Fintype G] [Invertible (Nat.card L : ℂ)] in
...
theorem CharacterDifferenceImage.nu_eq_mu_conj
    {τ : IntegralCharacterMap L G} {χ : ClassFunction L ℂ} [Finite G]
    (hχ : CharacterDifferenceImage (L := L) (G := G) τ χ)
