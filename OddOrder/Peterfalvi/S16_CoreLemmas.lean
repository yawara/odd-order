...
theorem W2_isPGroup {hyp : Hypothesis (G := G)} (data : FieldNormalizerData hyp) :
    IsPGroup hyp.base.p data.W2 := by
...
noncomputable def normOneUnitsToU {hyp : Hypothesis (G := G)}
    (data : FieldNormalizerData hyp) : fieldNormalizerNormOneUnits hyp →* data.U :=
  letI : Fact hyp.base.p.Prime := ⟨hyp.base.p_prime⟩
  { toFun := fun u =>
...
theorem normOneUnitsToU_surjective {hyp : Hypothesis (G := G)}
  ...
  letI : Fact hyp.base.p.Prime := ⟨hyp.base.p_prime⟩
  intro u
  have hu : (u : G) ∈ (fnComplement hyp).map data.sigma := by
    rw [data.sigma_U_eq_U]
    exact u.property
...
theorem P_inf_U_eq_bot {hyp : Hypothesis (G := G)} (data : FieldNormalizerData hyp) :
  ...
  apply le_antisymm ?_ bot_le
  intro x hx
  have hxP : x ∈ (fnKernel hyp).map data.sigma := by
    rw [data.sigma_P_eq_P]
    exact hx.1
  have hxU : x ∈ (fnComplement hyp).map data.sigma := by
    rw [data.sigma_U_eq_U]
    exact hx.2
  ...
  have hpu : p = u := data.sigma_injective (by
    rw [hp, hu])
  have hp_inter : p ∈ fnKernel hyp ⊓ fnComplement hyp := by
...
theorem P_sup_U_eq_sigma_top {hyp : Hypothesis (G := G)} (data : FieldNormalizerData hyp) :
  ...
  calc
    data.P ⊔ data.U =
        (fnKernel hyp).map data.sigma ⊔
          (fnComplement hyp).map data.sigma := by
      rw [data.sigma_P_eq_P, data.sigma_U_eq_U]
    _ = (fnKernel hyp ⊔ fnComplement hyp).map data.sigma := by
      rw [Subgroup.map_sup]
...
theorem fieldNormalizerKernel_pow_p_eq_one {hyp : Hypothesis (G := G)}
    {x : fieldNormalizerFrobeniusGroup hyp} (hx : x ∈ fnKernel hyp) :
    x ^ hyp.base.p = 1 := by
  letI : Fact hyp.base.p.Prime := ⟨hyp.base.p_prime⟩
  haveI : CharP (GaloisField hyp.base.p hyp.base.q) hyp.base.p := by
    rw [← Algebra.charP_iff (ZMod hyp.base.p) (GaloisField hyp.base.p hyp.base.q)
...
theorem mem_P_of_mem_P_sup_U_of_pow_p_eq_one
    {hyp : Hypothesis (G := G)} (data : FieldNormalizerData hyp)
    {x : G} (hxPU : x ∈ data.P ⊔ data.U) (hxp : x ^ hyp.base.p = 1) :
    x ∈ data.P := by
...
theorem subgroup_eq_P_sup_U_of_U_le_of_le_P_sup_U_of_ne_U
  ...
  have hXrange : X ≤ data.sigma.range := by
    ...
    simpa using hxPU
  have hUleH : fnComplement hyp ≤ XH := by
    intro g hg
  ...
  have hUleH' :
    ...
    simpa [fieldNormalizerComplement,
      OddOrder.BG.AppC.NormSet.normOneFrobeniusComplement] using hUleH
  have hneH : XH ≠ fnComplement hyp := by
    intro hXH
    apply hne
    have hmapX : XH.map data.sigma = X := Subgroup.map_comap_eq_self hXrange
    calc
      X = XH.map data.sigma := hmapX.symm
      _ = (fnComplement hyp).map data.sigma := by rw [hXH]
  ...
  have htopH : XH = ⊤ :=
    OddOrder.BG.AppC.NormSet.normOneFrobeniusSubgroup_eq_top_of_inr_range_le_of_ne_inr_range
      ...
      XH hUleH'
      (by simpa [fieldNormalizerComplement,
        OddOrder.BG.AppC.NormSet.normOneFrobeniusComplement] using hneH)
  have hmapX : XH.map data.sigma = X := Subgroup.map_comap_eq_self hXrange
  calc
    X = XH.map data.sigma := hmapX.symm
...
theorem generatorRelation_step2_primeLine {hyp : Hypothesis (G := G)}
    ...
    (hmem : (fieldNormalizerPrimeLineElement hyp c *
        (SemidirectProduct.inr u : fieldNormalizerFrobeniusGroup hyp) *
          fieldNormalizerPrimeLineElement hyp d) ∈ fnComplement hyp) :
  ...
  have hmem_original :
    ...
    simpa [fieldNormalizerPrimeLineElement, fieldNormalizerComplement,
      OddOrder.BG.AppC.NormSet.normOneFrobeniusComplement] using hmem
  exact
...
theorem generatorRelation_step2_primeLine_of_sigma_mem_U
    ...
    (hmem : data.sigma (fieldNormalizerPrimeLineElement hyp c) *
        data.sigma (SemidirectProduct.inr u : fieldNormalizerFrobeniusGroup hyp) *
          data.sigma (fieldNormalizerPrimeLineElement hyp d) ∈ data.U) :
    (c = 0 ∧ d = 0) ∨ (u = 1 ∧ c + d = 0) := by
  ...
  have hg_eq : g = prodH := data.sigma_injective (by
    rw [hg, hprod_sigma])
  have hmemH : prodH ∈ fnComplement hyp := by
...
theorem s_not_normalizes_U {hyp : Hypothesis (G := G)}
  ...
  have hmem_step :
      data.sigma (fieldNormalizerPrimeLineElement hyp 1) *
          data.sigma (SemidirectProduct.inr u : fieldNormalizerFrobeniusGroup hyp) *
            data.sigma (fieldNormalizerPrimeLineElement hyp (-1)) ∈ data.U := by
