...
matrix algebras with nil kernel.**
...
theorem exists_algHom_pi_matrix_of_isAlgClosed (k : Type*) [Field k] [IsAlgClosed k]
  ...
  refine ⟨n, d, hd, e.toAlgHom.comp (Ideal.Quotient.mkₐ k (Ring.jacobson A)), ?_, ?_⟩
  · exact e.surjective.comp Ideal.Quotient.mk_surjective
  · intro x hx
    -- The kernel is `J(A)` because `e` is injective, and `J(A) ^ m = ⊥`.
    have hxJ : x ∈ Ring.jacobson A := by
      have h0 : e (Ideal.Quotient.mk (Ring.jacobson A) x) = 0 := hx
      exact (Ideal.Quotient.eq_zero_iff_mem).mp (e.injective (by rw [h0, map_zero]))
    refine ⟨m, ?_⟩
    have hpow := Ideal.pow_mem_pow hxJ m
    rw [hm] at hpow
    simpa using hpow
