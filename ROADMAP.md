...
| 2a | **BG** Ch.I–IV + 補助 Appendices の Lean 化 | 🔄 §1 大半 + §2 (Thm 2.6) + §3 + App.A (A.1–A.5) + App.B (Puig L(S)) + Thm 6.2 一般形 sorry-free・配線済。**現フロンティア §4 Blackburn** (Thm 4.12/Prop 4.8/4.11/Lem 4.9/Gorenstein Lem 4.14 等 ✅、Thm 4.16 apex は未着地)。§5–§16 未着手 |
...
- **2026-05-31** **(catch-up: 05-29→05-31 の集約記録)** BG ローカル解析と Peterfalvi が大きく前進。**正確な live 状況は memory `ft-master-roadmap` + `notes/meta/ft_master_roadmap_2026_05_29.md`(冒頭 05-31 訂正ヘッダ付き) + 各 `notes/` が正本**、本エントリは要約:
    ...
    - **BG §4 Blackburn 大半完成**: Thm 4.12(a)(b)(c) / Prop 4.3(a)(b) / Prop 4.8(a)(b) / **Prop 4.11 (Huppert)** / Lem 4.9 / Lem 4.15 / GL(n,p) 橋 / Gorenstein Lem 4.12–4.14 が sorry-free。**残 = Gorenstein Thm 4.15(i) precursor (SCN₃=∅⇒pRank≤2) → BG Lem 4.13 → Thm 4.16 apex (未 statement)**。自走 `bg-prove` キュー = `notes/bg/autonomous_prove_queue.md`、tracker = issue 0051。
...
**Appendices** (17 結果)
- [ ] App.A Prerequisites and p-Stability (p.135) — 5 結果 (Thm A.1-A.5). §6 Thm 6.2 の証明前提. *前提: Isaacs Ch.7 + BG §2 (Thm 2.6 ✅実装済 = S02 `odd_two_dim_sylow_abelian`)* — [調査メモ](notes/bg/appA_pstability.md). **⚠️ 2026-05-28 spike 訂正 ([設計ノート §0](notes/meta/bg_s6_appAB_route_2026_05_28.md))**: 真のゲートは **A.2 = Gorenstein 3.8.1 の次元縮約 (char-p 二次既約 ⇒ dim 2)** で、新規 char-p 加群表現論が要る(7.3 流用不可)。base A.1 は部品済(S02 Thm 2.6 + `PGroupFixedVector`)、連鎖 A.5 ⟸ A.4(c) ⟸ A.3 ⟸ A.2、A.5 が App.B Puig L(S) の中核前提。**A.2 次元縮約パートの別セッション引き継ぎ = [issue #0041](issues/0041-bg-appa-a2-dim-reduction.md)**。旧「A.4(b) ≡ Isaacs 7.6 / Ch.7 import ~50%」は楽観で外れ(A.4(b) は 7.6 の系で同値でなく、route は A.2 経由)。
