...
- **分割の粒度・形式も mathlib 準拠 (2026-07-09 明文化; 上限は 2026-07-24 に mathlib 素の値へ切替)**:
  ...
  - **本リポジトリの上限 = mathlib と同じ 1500 行** (ユーザー裁定 2026-07-24「mathlib の基準に合わせる」; 旧 2000 上限 + defValue ハックは廃止)。lakefile の `linter.style.longFile = 1500` が gate で、`--strict` lint gate により超過は CI 赤。目安は **1 ファイル ≈ 300–1,500 行・1 トピック**。
  - **分割 campaign は 2026-07-25 完了** ([issue 0149 closed](issues/closed/0149-longfile-1500-split-campaign.md)): 切替時点の超過 58 file (+境界 2) を全て ≤1500 に分割し stamp 除去済み。**現存する per-file 例外 stamp は `AxiomsCheck.lean` (機械列挙、意図的恒久例外) のみ**。新たに 1500 を超えそうな file は stamp を足すのでなく分割する (例外機構の文法: `set_option linter.style.longFile N`、N は linter が許容する candidate = `(行数/100)*100+200` か candidate−100 のみ)。分割手順の実務知見 (prefix-split の gen 手順・走査の罠 6 種) は closed issue 0149 の消化記録が正本。
  ...
  - leaf の 1500 行超過は **flat な兄弟 prefix-split** (先頭クラスタを新 sibling module へ、元 file が import) で解消してよい (module 名不変・下流不変)。
  - **leaf 命名は mathlib 互換の記述的英語** (`TypePDuality`, `NormEstimates` 等)。`Part1`/`Part2` 等の無内容な名前は不可。
  - **⚠ 新 leaf は同じ commit で `OddOrder.lean` に配線する (2026-07-20 明文化、実害 2 回)**。root aggregator から到達できない leaf は `lake build OddOrder` の import closure に入らず、**hub の合流フルビルド gate が一切検証しない** — jobs 数不変のまま数秒で green が返り、新規数百行が一度も elaborate されない。`OddOrder.lean` は territory 表で**全レーン編集可**の共有ファイルなので、leaf を作ったレーンが自分で追加すること。既発生 2 件 = c の `FormalCommutator`/`FormalCollection`/`PolynomialSequences` (2 tick 素通し) と `AppE_SemidirectFrattini` (289 行、issue 0135)。hub 側の全数監査スクリプトは `notes/meta/merge_monitor.md`。
...
backlog 230 件は 2026-07-22〜24 の wave で**完済** (issue 0138 closed、0123 統合) し、gate は
**純ゼロ (`bin/check-warnings --strict`)** に切替済み — **非 sorry 警告が 1 件でも CI
(`lean_action_ci.yml`) が赤**になる。sorry 警告は常に許容 (statement-first spine の正常系)。
...
- ⚠ `flexible` / `import` 絞り込み / instance in type 系の修正は leaf build で検出不能な cascade を
  ...
  2026-07-24 wave で 3 件連鎖)。⚠ longLine census は **codepoint** で数える (`awk 'length>100'` は
  バイト数で日本語/unicode 記号を過大計上; 判定は `bin/check-warnings`)。
  ⚠ flexible の linter 提示 Try-this は不完全なことがある (unfold 系 simp 引数を欠く) —
...
- 採番 + scaffold: `bin/new-issue [--base N] <slug> "<title>"` → `issues/NNNN-<slug>.md` を作って `git add`。並行セッションは `--base`/`ODD_ISSUE_BASE` で採番レンジを分けて衝突回避 (hub/main=0、**lane 別に 1000 の倍数 base** — 現行 a=1000/b=2000/c=3000; **shared-infra claim はレーン別サブバンド a=9200/b=9300/c=9400/hub=9500** (幅 100、2026-07-19 に共有 9000 から移行); 正本 = merge_monitor.md レーン表; 既定 0)
...
- 並行 worktree には **issue 採番レンジ**を割り当てる (`export ODD_ISSUE_BASE=N`、base は 100 の倍数 — 1000 の倍数なら幅 1000、それ以外は幅 100; hub/main=0、現行 a=1000/b=2000/c=3000、shared-infra claim = a:9200/b:9300/c:9400/hub:9500)。採番衝突を予防 ([`notes/meta/issue_management.md`](notes/meta/issue_management.md) 「並行セッションの採番レンジ」)
...
- **レーンの `/loop` self-pacing wakeup = 60s (最短固定、ユーザー 2026-07-06)**。各レーンが `/loop` で自走する (self-pacing、interval 無指定 → ScheduleWakeup dynamic mode) とき、次 iteration の wakeup は **必ず `delaySeconds: 60`** (runtime clamp 下限) にする。理由: レーンは actively 本物の数学を証明中ゆえ**即再開**が正しく、60s は prompt cache を warm に保つ (300s+ は cache miss)。**⚠ 間違えやすい典型ミス**: ScheduleWakeup の「idle polling default 1200–1800s」を active-work レーンに適用すること — それは**外部状態待ち polling 専用**で、証明を進めるレーンには不適 (無駄な待機で throughput 激減)。レーンが止まってよいのは STOP 条件 (unsound/新 axiom/想定外) と context 枯渇時のみ。context 枯渇なら subagent handoff or 停止+報告 (最小 iteration での compaction 待ち空転は誤り)。**⚠ turn 終了の順序規則 (issue 0131、2026-07-25 明文化)**: report・commit・main 同期・マイルストーン報告など**どんな区切りの turn でも、turn を終える前に必ず `ScheduleWakeup(delaySeconds: 60)` を呼ぶ**。「issue を閉じた」「main と同期した」「定理が landing した」は**継続の合図**であって停止理由でない — 停止障害 4 例は全て「達成感のある turn の終わり」で wakeup 予約が抜けた (48 回連続で正しく予約できていたレーンですら抜けた)。進捗不良の検知も同じ: **報告し、そのうえで実装に移って続行する** (報告≠停止)。hub 監視 cron (**現行ユーザー指定 = 15分 `7,22,37,52` (2026-07-15)**。明示指定がない場合はモデル依存: Fable=30分 `13,43` / Opus=15分 `7,22,37,52`; 正本 = merge_monitor.md 冒頭) とは別物 (hub は合流ポーリングゆえ長間隔でよい)。正本 = [[feedback-loop-short-wakeup]]。
...
- Lean: [`lean-toolchain`](lean-toolchain) (現状 `leanprover/lean4:v4.33.0`、2026-08-20 に v4.32.2 から bump; 手順と API 変更一覧 [`notes/meta/mathlib_v433_migration.md`](notes/meta/mathlib_v433_migration.md) — **Lean 4.33 の `isDefEq` が transparency を尊重するようになった**のが最大の破壊的変更で、mathlib と同じ per-declaration 互換フラグ `set_option backward.isDefEq.respectTransparency false in` で追従 (269 箇所)。過去の bump 記録: v4.32.0-rc1 → v4.32.2 (2026-08-04, `MonoidAlgebra` structure 化) は [`notes/meta/mathlib_v4322_migration.md`](notes/meta/mathlib_v4322_migration.md)、v4.30.0-rc2 → v4.32.0-rc1 (2026-07-09) は [`notes/meta/mathlib_v432_migration.md`](notes/meta/mathlib_v432_migration.md)、v4.29.1 → v4.30.0-rc2 (2026-05-27) は [`notes/meta/mathlib_rc2_migration.md`](notes/meta/mathlib_rc2_migration.md))
  - **rc には当てない (v4.33 で実践)**: v4.33.0-rc2 の時点で「import 面 407 module 中 260 (64%) を触る」と実測できていたので final (2026-08-10) を待ち、2026-08-20 に一気に上げた。移行の実務で効いた知見は v433 note の「⚙ 必要箇所の特定手順」(逐次ビルドでは 1 ラウンド 1 module しか進まないので、一時グローバルフラグ → 全 file 個別 elaborate → per-declaration 化)。
  - ⚠ **新 linter の有無は `mathlibStandardSet` の所属だけで判断しない** — `linter.style.haveILetI` はセット外だが `defValue := true` で既定 ON だったため、12,169 箇所の `haveI`/`letI` 置換が必要になった。
