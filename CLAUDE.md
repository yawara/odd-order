...
- **最小 import (mathlib の肝)**: 各ファイルは必要な module だけ import する。これが import DAG を浅くし **full build の並列化 + 編集の再ビルド fan-out 縮小**を生む (**速度の本命はここ**)。分割しても import を絞らないと full build は速くならない。
