...
- **「quick win / shallow entry / 難易度」は着手判断の基準でない (ユーザー 2026-06-30)**。各レーンは**その目的 (= 担当クラスタの FT 経路を閉じる) にかなう work** を、難しくても・clean な近道が無くても・1 iteration で landing しなくても**正面から進める**。「quick win が無い」「shallow entry が無い」「deep char で多反復」は**手を止めて flag/ask する理由にならない**(調査偏重に逃げるのも同断)。frontier が deep なら deep なまま engage し、本物の数学 (norm estimate・coherence・char body) を実際に**証明**する。止めてよいのは「想定違反 (unsound carrier・新 axiom・signature 無断変更)」「真に判断を要する設計分岐」のみ ([[scaffold-sorry-free-not-done]] [[feedback-no-avoiding-hard-parts]])。
  - **「次に何をやるか」は規約から導出し、ユーザーに訊かない (ユーザー 2026-07-01)**。機械手順 ([`ft_path_policy.md`](notes/meta/ft_path_policy.md) §0.4): (1) on-path 未証明 piece を「上流優先 → 文書順」で並べ先頭を選ぶ (難易度・深さは無関係)、(2) それが **lane 内 clean-additive** (自 lane ファイルへの新規宣言追加＋既存 cite) で進むなら深くても着手、(3) 進めない = STOP 条件 or **他 active session の共有ファイル (coherence-core S07/S08 等) 改変**を要するなら、coordinated issue で flag し**文書順で次の in-lane piece** へ移る。判定軸は「難易度」でなく「cross-lane / STOP」——深いだけなら engage。"どの piece" は設計分岐でなく AskUserQuestion 対象外。焦点が自動決定できない/曖昧なら訊く前に**規約側を直す** ([[feedback-quick-win-not-a-criterion]])。
