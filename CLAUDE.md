...
- **分割の粒度・形式も mathlib 準拠 (2026-07-09 明文化; 上限は 2026-07-24 に mathlib 素の値へ切替)**:
  ...
  - 切替時点の超過 58 file は mathlib 自身の例外機構 per-file `set_option linter.style.longFile N` で grandfather 済み (**N は linter が許容する candidate = `(行数/100)*100+200` か candidate−100 のみ** — 過大値は警告)。stamp は **ratchet**: 伸ばすには stamp の明示 bump が必要で diff に出る。**分割が済んだ file から stamp を除去**していく — 正本 = [issue 0149](issues/0149-longfile-1500-split-campaign.md) (worklist 全 58 file)。意図的恒久例外は `AxiomsCheck.lean` (機械列挙) のみ。
